...
pub fn load_api_key_from_env_or_config(env_key: &str, file_name: &str) -> Option<String> {
    ...
    // MiniMax profiles historically stored their Token Plan key as
    // OPENAI_API_KEY in minimax.env. Keep that file-level compatibility, but do
    // not consult the ambient OPENAI_API_KEY process variable: a long-lived
    // shared server commonly has a real OpenAI credential there, and using it
    // against MiniMax produces a misleading 401.
    if env_key == "MINIMAX_API_KEY" && file_name == "minimax.env" {
        let legacy_prefix = "OPENAI_API_KEY=";
        for line in content.lines() {
            if let Some(key) = line.strip_prefix(legacy_prefix)
                && let Some(key) = clean_loaded_value(key, "OPENAI_API_KEY")
            {
                return Some(key);
            }
        }
...
mod tests {
    ...
    #[test]
    fn loads_legacy_minimax_file_key_without_using_ambient_openai_key() {
        let temp = tempfile::tempdir().expect("tempdir");
        let _guard = EnvGuard::new(&["JCODE_HOME", "MINIMAX_API_KEY", "OPENAI_API_KEY"]);
        jcode_core::env::set_var("JCODE_HOME", temp.path());

        save_env_value_to_env_file("OPENAI_API_KEY", "minimax.env", Some("legacy-minimax-key"))
            .expect("save legacy MiniMax key");
        jcode_core::env::set_var("OPENAI_API_KEY", "ambient-openai-key");
        jcode_core::env::remove_var("MINIMAX_API_KEY");

        assert_eq!(
            load_api_key_from_env_or_config("MINIMAX_API_KEY", "minimax.env").as_deref(),
            Some("legacy-minimax-key")
        );
    }
