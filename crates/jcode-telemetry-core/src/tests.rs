// sonar-file-size: waive=350 reason=pre-existing telemetry test module; this mission updates one aggregation regression test and splitting the suite is out of scope
use super::*;
use std::{
    ffi::OsString,
...
fn test_record_token_usage_aggregates_session_and_turn() {
    ...
    begin_session_with_mode("openai", "gpt-5.4", None, false);
    record_turn();
    // OpenAI-style subset accounting: the 80 cache-read tokens are already
    // included in the 100 raw input tokens.
    record_token_usage(100, 25, Some(80), None, 100);
    // Anthropic-style split accounting: effective input includes both caches.
    record_token_usage(50, 5, Some(200), Some(2), 252);

    {
        ...
        assert_eq!(state.cache_read_input_tokens, 280);
        assert_eq!(state.cache_creation_input_tokens, 2);
        assert_eq!(state.total_tokens, 382);
        let turn = state.current_turn.as_ref().expect("current turn");
        assert_eq!(turn.input_tokens, 150);
        assert_eq!(turn.output_tokens, 30);
        assert_eq!(turn.cache_read_input_tokens, 280);
        assert_eq!(turn.cache_creation_input_tokens, 2);
        assert_eq!(turn.total_tokens, 382);
    }
    if let Ok(mut session) = SESSION_STATE.lock() {
        *session = None;
    }
...
#[test]
fn test_record_todo_tool_aggregates_session_and_turn() {
    let _guard = lock_telemetry_test_state();
    if let Ok(mut session) = SESSION_STATE.lock() {
        *session = None;
    }
