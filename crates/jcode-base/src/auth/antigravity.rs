...
const ANTIGRAVITY_CLIENT_ID: &str = ""; // gitleaks:allow
const ANTIGRAVITY_CLIENT_SECRET: &str = ""; // gitleaks:allow
...
fn require_distribution_oauth_client() -> Result<()> {
    if antigravity_client_id().trim().is_empty() || antigravity_client_secret().trim().is_empty() {
        anyhow::bail!(
            "This distribution does not embed a Google OAuth application credential. Configure the existing ANTIGRAVITY OAuth client environment overrides before using this login, or select another provider."
        );
    }
    Ok(())
}
...
async fn refresh_tokens_uncoordinated(tokens: &AntigravityTokens) -> Result<AntigravityTokens> {
    require_distribution_oauth_client()?;
