...
const GEMINI_CLIENT_ID: &str = ""; // gitleaks:allow
const GEMINI_CLIENT_SECRET: &str = ""; // gitleaks:allow
...
fn require_distribution_oauth_client() -> Result<()> {
    if gemini_client_id().trim().is_empty() || gemini_client_secret().trim().is_empty() {
        anyhow::bail!(
            "This distribution does not embed a Google OAuth application credential. Configure the existing GEMINI OAuth client environment overrides before using this login, or select another provider."
        );
    }
    Ok(())
}
