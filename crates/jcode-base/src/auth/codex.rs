...
/// Load credentials for one specific stored account, ignoring the global
/// active-account override.
///
/// This is the entry point for per-session account binding: a session pinned to
/// a subscription must keep using it even when another session switches the
/// process-wide active account.
pub fn load_credentials_for_label(label: &str) -> Result<CodexCredentials> {
    let auth = load_auth_file()?;
    let account = auth
        .openai_accounts
        .iter()
        .find(|account| account.label == label)
        .with_context(|| format!("No OpenAI account with label '{label}' found"))?;
    Ok(credentials_from_account(account))
}

/// The stored account label that owns `credentials`.
///
/// Resolution is by refresh token first, then by account id. Both identify the
/// account that actually issued the credentials, which is what a token refresh
/// must write back to.
///
/// This exists because refreshes happen on a provider instance that may be
/// pinned to a non-active account. Resolving the write-back target from the
/// process-global active label would persist rotated tokens onto the wrong
/// account and silently log the pinned subscription out.
pub fn account_label_for_credentials(credentials: &CodexCredentials) -> Option<String> {
    if let Some(label) = account_label_for_refresh_token(&credentials.refresh_token) {
        return Some(label);
    }
...
/// The stored account label holding `refresh_token`.
///
/// A refresh token is unique to one account, so this identifies the account a
/// token rotation must be written back to, independent of which account is
/// globally active.
...
/// Identities of every logged-in OpenAI account, for account-binding rules.
pub fn account_identities() -> Vec<crate::auth::account_binding::AccountIdentity> {
    list_accounts()
        .unwrap_or_default()
        .into_iter()
        .map(|account| {
            crate::auth::account_binding::AccountIdentity::new(
                account.label,
                account.email,
                account.account_id,
            )
