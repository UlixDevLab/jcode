...
///
/// The account to write back to is resolved from the refresh token itself, not
/// from the process-global active account. A session pinned to a specific
/// subscription refreshes on its own credentials, and resolving the write-back
/// target globally would persist those rotated tokens onto whichever account
/// happens to be active, logging the pinned account out.
