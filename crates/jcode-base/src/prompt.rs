...
/// Maximum character length for a summarized skill description in the catalog.
/// Kept in sync with the role summary cap so both lazy catalogs use the same budget.
const MAX_SKILL_SUMMARY_CHARS: usize = 160;

/// Normalize a skill's frontmatter description and retain only the discovery gist.
/// The full skill body remains available through lazy skill loading.
pub(crate) fn summarize_skill_description(description: &str) -> String {
    let collapsed: String = description.split_whitespace().collect::<Vec<_>>().join(" ");
    if collapsed.chars().count() <= MAX_SKILL_SUMMARY_CHARS {
        return collapsed;
    }

    let clipped: String = collapsed
        .chars()
        .take(MAX_SKILL_SUMMARY_CHARS.saturating_sub(1))
        .collect();
...
fn build_available_skills_section(available_skills: &[SkillInfo]) -> Option<String> {
    ...
    for skill in available_skills {
        section.push_str(&format!(
            "\n- `/{} ` - {}",
            skill.name,
            summarize_skill_description(&skill.description)
        ));
    }
    section.push_str(
...
pub struct ContextInfo {
    ...
    /// Role catalog section size (chars)
    pub roles_chars: usize,
...
impl ContextInfo {
    ...
    pub fn breakdown(&self) -> Vec<(&'static str, usize, &'static str)> {
        ...
        if self.roles_chars > 0 {
            parts.push(("roles", self.roles_chars, "🎭"));
        }
        parts
    }
}
...
pub fn build_system_prompt_full_with_capabilities(
    ...
    // Add the role catalog (names and summaries only; bodies stay lazy).
    if let Some(roles_section) =
        crate::role::build_roles_prompt(&crate::role::discover(working_dir))
    {
        info.roles_chars = roles_section.len();
        parts.push(roles_section);
    }
...
fn build_system_prompt_split_with_capabilities_and_agents_md(
    ...
    {
        info.roles_chars = roles_section.len();
        static_parts.push(roles_section);
    }
