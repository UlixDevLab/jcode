...
pub struct Sidecar {
    ...
    /// Pre-built forked provider used for the memory-sidecar generic model
    /// override path. When `Some`, [`Sidecar::complete_via_provider`] uses
    /// THIS fork (with the override model already set) instead of calling
    /// `active_provider_fork()` on every request. The fork is independent of
    /// the main/active provider, so `set_model` on it never mutates the
    /// active provider — see `with_configured_model` for construction.
    provider_override: Option<std::sync::Arc<dyn crate::provider::Provider>>,
}
...
impl Sidecar {
    ...
    fn with_configured_model(configured_model: Option<String>) -> Self {
        // Generic active-provider model override path. When `agents.memory_model`
        // is a generic provider model (e.g. the exact Stables ID
        // `minimax/M2.7-highspeed`), the sidecar must fork the active
        // provider, set that model on the fork, and use the fork for
        // completions — without ever mutating the main/active provider.
        //
        // Dedicated OpenAI/Claude models keep their existing fast paths; this
        // branch is only entered for models that are NEITHER OpenAI NOR
        // Claude. A failed `set_model` (provider does not support switching)
        // deterministically falls through to the existing auto-select path
        // with a warning, so the sidecar never silently sends a wrong model.
        let provider_override = configured_model
            .as_deref()
            .filter(|model| !Self::is_dedicated_sidecar_model(model))
            .and_then(|model| Self::try_fork_provider_with_model(model));

        if let Some((fork, model)) = provider_override {
            return Self {
                ...
                provider: Some(fork.clone()),
                provider_override: Some(fork),
            };
        }
    ...
    /// Whether a model string maps to one of the dedicated sidecar backends
    /// (OpenAI or Claude). Generic provider models — anything else —
    /// should go through the active-provider fork path instead.
    fn is_dedicated_sidecar_model(model: &str) -> bool {
        matches!(
            crate::provider::provider_for_model(model),
            Some("openai") | Some("claude")
        )
    }

    /// Fork the active provider and set `model` on the fork, returning the
    /// fork plus the model name on success. On any failure (no active
    /// provider registered, or the active provider rejects `set_model`),
    /// returns `None` after logging an explicit warning — the caller falls
    /// back to the auto-selected backend, matching the existing
    /// config-fallback semantics for unsupported memory models.
    ///
    /// The fork is independent of the main/active provider (per the
    /// [`Provider::fork`] contract), so this function never mutates the
    /// active provider's model.
    fn try_fork_provider_with_model(
        model: &str,
    ) -> Option<(std::sync::Arc<dyn crate::provider::Provider>, String)> {
        let fork = match crate::provider::active_provider_fork() {
            Some(fork) => fork,
            None => {
                crate::logging::warn(&format!(
                    "Sidecar: memory_model '{}' is generic but no active provider is registered; falling back to auto-selected backend",
                    model
                ));
                return None;
            }
        };
        match crate::provider::set_model_with_auth_refresh(&*fork, model) {
            Ok(()) => Some((fork, model.to_string())),
            Err(err) => {
                crate::logging::warn(&format!(
                    "Sidecar: memory_model '{}' was rejected by active provider '{}' ({}); falling back to auto-selected backend",
                    model,
                    fork.name(),
                    err
                ));
    ...
    /// collects the streamed `TextDelta`s into a single string.
    ///
    /// When a generic `agents.memory_model` override is in effect, the
    /// pre-built fork with the override model already set is used directly —
    /// this guarantees the sidecar never falls through to
    /// `active_provider_fork()` (which would carry the main agent's model
    /// and effectively ignore the user's override) and never mutates the
    /// main/active provider. When no override is in effect, the active
    /// provider is forked per-call (the historical behavior).
    ...
    async fn complete_claude_oauth(&self, system: &str, user_message: &str) -> Result<String> {
        ...
        let call_id = crate::provider::anthropic::new_oauth_request_id();

        // Acquire the subscription monitor lease before model transport. The
        // monitor's usage sample happens internally and does not serialize
        // parallel Claude calls. Disabled means monitoring is not configured;
        // any other denial asks the caller to use another route for this turn.
        let lease = match crate::usage::acquire_anthropic_subscription_guard(
            &creds.access_token,
            &self.model,
            &call_id,
        )
        ...
        {
            Ok(lease) => Some(lease),
            Err(crate::usage::AdmissionError::Disabled) => {
                crate::logging::info("Sidecar OAuth: subscription monitor disabled; proceeding");
                None
            }
            Err(other) => {
                return Err(anyhow::anyhow!(
                    "Sidecar OAuth monitor paused this call: {}. Ask the operator for approval when requested, or use another configured provider for this turn.",
                    other
                ));
            }
        };
        ...
        // Wrap send + parse in an async block so we can hold the lease across
        // both operations and always call finish, even on HTTP or parse errors.
        let result = async {
        ...
        // Always finish the lease if we hold one, even on HTTP or parse errors.
        if let Some(lease) = lease {
            match lease.finish(&creds.access_token).await {
                Ok(crate::usage::FinishOutcome::Killed) => {
                    crate::logging::warn(
                        "Sidecar OAuth: suspicious subscription movement detected; the completed response is preserved and future Claude calls require operator approval.",
                    );
                }
                Ok(_) => {}
                Err(e) => {
                    // State errors are safe to log — no credentials are included.
                    crate::logging::warn(&format!(
                        "Sidecar OAuth: monitor finalization failed (state error): {}",
                        e
                    ));
                }
            }
...
mod tests {
    ...
    /// Like [`StubProvider`], but additionally implements `set_model` against a
    /// mutable `current_model` so the memory-sidecar generic-override path can
    /// be exercised against a working provider. The main provider's model is
    /// also tracked so tests can assert it is NOT mutated when the sidecar
    /// forks the provider and sets the model on the fork.
    struct StubProviderWithModel {
        name: &'static str,
        reply: String,
        current_model: std::sync::Mutex<String>,
    }

    impl StubProviderWithModel {
        fn new(name: &'static str, initial_model: &str, reply: &str) -> Self {
            Self {
                name,
                reply: reply.to_string(),
                current_model: std::sync::Mutex::new(initial_model.to_string()),
            }
        }
    ...
    #[async_trait::async_trait]
    impl crate::provider::Provider for StubProviderWithModel {
        async fn complete(
            &self,
            _messages: &[crate::message::Message],
            _tools: &[crate::message::ToolDefinition],
        ...
        fn fork(&self) -> std::sync::Arc<dyn crate::provider::Provider> {
            // Crucially, the fork must carry its own `current_model` mutex so
            // `set_model` on the fork never mutates the original provider.
            std::sync::Arc::new(StubProviderWithModel {
                name: self.name,
                reply: self.reply.clone(),
                current_model: std::sync::Mutex::new(self.current_model()),
            })
        }
    ...
    // ---- Generic active-provider model override ---------------------------

    /// With a generic `agents.memory_model` like the exact Stables ID
    /// `minimax/M2.7-highspeed`, the sidecar must fork the active provider,
    /// set that model on the fork, and use the fork for completions. The
    /// dedicated OpenAI/Claude paths are NOT used for generic models.
    #[test]
    fn sidecar_generic_model_override_routes_through_forked_provider() {
        let _guard = crate::storage::lock_test_env();
        let temp = tempfile::TempDir::new().expect("create temp jcode home");
        let _home = EnvVarGuard::set_path("JCODE_HOME", temp.path());
        let _openai = EnvVarGuard::unset("OPENAI_API_KEY");

        let active = std::sync::Arc::new(StubProviderWithModel::new(
            "openrouter",
            "openrouter/auto",
            "from-fork",
        ));
        crate::provider::set_active_provider(active.clone());

        let sidecar = Sidecar::with_configured_model(Some("minimax/M2.7-highspeed".to_string()));
        assert_eq!(
            sidecar.backend_name(),
            "provider",
            "generic memory_model must route through the provider path"
        );
        assert_eq!(
            sidecar.model_name(),
            "minimax/M2.7-highspeed",
            "sidecar.model_name() must reflect the generic override"
        );

        // The sidecar must actually complete via the forked provider, and the
        // forked provider must have the override model set on it.
        let rt = tokio::runtime::Builder::new_current_thread()
            .enable_all()
            .build()
            .unwrap();
        let out = rt
            .block_on(sidecar.complete("sys", "user"))
            .expect("forked provider-backed completion should succeed");
        assert_eq!(
            out, "from-fork",
            "sidecar must use the forked provider's reply"
        );
    }

    /// Critical safety: setting the generic override model on the fork MUST
    /// NEVER mutate the main/active provider's model. This guards against a
    /// regression where `set_model` is called on the active provider directly.
    #[test]
    fn sidecar_generic_model_override_does_not_mutate_active_provider() {
        let _guard = crate::storage::lock_test_env();
        let temp = tempfile::TempDir::new().expect("create temp jcode home");
        let _home = EnvVarGuard::set_path("JCODE_HOME", temp.path());
        let _openai = EnvVarGuard::unset("OPENAI_API_KEY");
        ...
        let main_model_before = active.model();
        crate::provider::set_active_provider(active.clone());

        let _sidecar = Sidecar::with_configured_model(Some("minimax/M2.7-highspeed".to_string()));

        let main_model_after = active.model();
        assert_eq!(
            main_model_before, main_model_after,
            "the active provider's model must be unchanged after a generic override"
        );
        assert_eq!(
            main_model_after, "openrouter/auto",
            "the active provider must still report its original model"
        );
    }

    /// When no `memory_model` override is configured, the sidecar must keep
    /// using the auto-selected backend (the live provider when no OAuth creds
    /// are present). This is a regression guard for "no override ⇒ unchanged
    /// behavior".
    #[test]
    fn sidecar_without_override_uses_auto_selection() {
        let _guard = crate::storage::lock_test_env();
        let temp = tempfile::TempDir::new().expect("create temp jcode home");
        let _home = EnvVarGuard::set_path("JCODE_HOME", temp.path());
        let _openai = EnvVarGuard::unset("OPENAI_API_KEY");
        ...
        assert_eq!(
            sidecar.backend_name(),
            "provider",
            "no override must still pick the auto-selected provider backend"
        );

        // Auto-selection must NOT install a model override on the fork — the
        // active provider is used as-is, so its model is untouched.
        let rt = tokio::runtime::Builder::new_current_thread()
            .enable_all()
            .build()
            .unwrap();
        let out = rt
            .block_on(sidecar.complete("sys", "user"))
            .expect("auto-selected provider completion should succeed");
        assert_eq!(out, "auto-pick");
        assert_eq!(
            active.model(),
            "openrouter/auto",
            "auto-selection must not have mutated the active provider's model"
        );
    }

    /// A generic override whose `set_model` is rejected by the active provider
    /// MUST deterministically fall back to the auto-selected backend and MUST
    /// NOT mutate the active provider. We use the existing `StubProvider`
    /// which intentionally does NOT implement `set_model`, so the trait
    /// default returns an explicit error — that exercises the failure path.
    #[test]
    fn sidecar_generic_model_override_falls_back_when_set_model_fails() {
        let _guard = crate::storage::lock_test_env();
        let temp = tempfile::TempDir::new().expect("create temp jcode home");
        let _home = EnvVarGuard::set_path("JCODE_HOME", temp.path());
        let _openai = EnvVarGuard::unset("OPENAI_API_KEY");

        // StubProvider does NOT override set_model, so it returns the trait's
        // default error: "This provider does not support model switching".
        let active = std::sync::Arc::new(StubProvider {
            name: "openrouter",
        ...
        // The override failed, so the sidecar must fall back to the auto-
        // selected provider backend — and the resulting model name must NOT
        // be the rejected override (otherwise the sidecar would silently
        // send a request that the active provider cannot honor).
        assert_eq!(
            sidecar.backend_name(),
            "provider",
            "failed generic override must still route through the provider path"
        );
        assert_ne!(
            sidecar.model_name(),
            "minimax/M2.7-highspeed",
            "failed generic override must NOT advertise the rejected model"
        );

        // The sidecar still completes successfully via the auto-selected fork.
        let rt = tokio::runtime::Builder::new_current_thread()
            .enable_all()
            .build()
            .unwrap();
        let out = rt
            .block_on(sidecar.complete("sys", "user"))
            .expect("fallback provider-backed completion should succeed");
        assert_eq!(out, "fallback-reply");
    }
}
