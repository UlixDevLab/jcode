// sonar-file-size: {"waive":350,"owner":"jcode-maintainers","ticket":"Jcode-control-plane-policy","baseline_loc":1629,"max_loc":1629,"expires":"2026-12-01","removal_trigger":"module reaches 350 LOC","reason":"Policy extraction keeps inherited module at or below mission-base LOC."}
...
use jcode_config_types::SessionToolPolicyMode;
...
/// Maximum Unicode scalar values retained for the current mission brief.
///
/// A mission brief is a compact provider-context record, not a transcript or a
/// file reference. Keeping the cap here prevents a stale history dump from
/// quietly becoming a second conversation context after resume or compaction.
pub const MAX_CURRENT_MISSION_BRIEF_CHARS: usize = 8_000;

/// The one current, root-owned brief for a scoped piece of work.
///
/// This is deliberately a closed record rather than an extensible context bag:
/// it gives the provider the current outcome, boundary, and meaningful open
/// decisions without manufacturing approval receipts or reviving old work.
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
#[serde(deny_unknown_fields)]
pub struct CurrentMissionBrief {
    pub outcome: String,
    #[serde(default)]
    ...
    #[serde(default)]
    pub acceptance_observations: Vec<String>,
    #[serde(default)]
    pub autonomous_decisions: Vec<String>,
    #[serde(default)]
    pub unresolved_user_decisions: Vec<String>,
}

impl CurrentMissionBrief {
    pub fn validate(&self) -> Result<(), &'static str> {
        if self.outcome.trim().is_empty() {
            return Err("current mission outcome must not be blank");
        }
        let all_text = std::iter::once(&self.outcome)
            .chain(self.in_scope.iter())
            .chain(self.out_of_scope.iter())
            .chain(self.constraints.iter())
            .chain(self.acceptance_observations.iter())
            .chain(self.autonomous_decisions.iter())
            .chain(self.unresolved_user_decisions.iter());
        let mut chars = 0usize;
        for text in all_text {
            if text.trim().is_empty() {
                return Err("current mission brief entries must not be blank");
            }
            if text
                .chars()
                .any(|ch| ch.is_control() && ch != '\n' && ch != '\r' && ch != '\t')
            {
                return Err("current mission brief entries must not contain control characters");
            }
            chars += text.chars().count();
        }
        if chars > MAX_CURRENT_MISSION_BRIEF_CHARS {
            return Err("current mission brief exceeds the character limit");
        }
        Ok(())
    }

    pub fn render_for_provider(&self) -> String {
        fn list(output: &mut String, heading: &str, entries: &[String]) {
            if entries.is_empty() {
                return;
            }
            output.push_str("\n\n### ");
        ...
        list(&mut output, "In scope", &self.in_scope);
        list(&mut output, "Out of scope", &self.out_of_scope);
        list(&mut output, "Constraints", &self.constraints);
        list(
            &mut output,
        ...
        list(
            &mut output,
            "Meaningful user decisions still open",
            &self.unresolved_user_decisions,
        );
        output
    }
}
...
pub use provider_key::derive_session_provider_key;
pub use render::{
...
pub struct Session {
    ...
    /// The root-owned current brief. It is compactly injected into root and
    /// worker provider context, and persists through resume and compaction.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub current_mission_brief: Option<CurrentMissionBrief>,
    ...
    /// Explicit, persisted session tool surface. Old session files deserialize
    /// to Normal so ordinary resume behavior remains unchanged.
    #[serde(default)]
    pub tool_policy_mode: SessionToolPolicyMode,
...
struct SessionStartupStub {
    ...
    #[serde(default)]
    current_mission_brief: Option<CurrentMissionBrief>,
    #[serde(default)]
    ...
    #[serde(default)]
    tool_policy_mode: SessionToolPolicyMode,
    #[serde(default)]
...
impl Session {
    fn session_from_startup_stub(stub: SessionStartupStub) -> Self {
        ...
        session.current_mission_brief = stub.current_mission_brief;
        ...
        session.tool_policy_mode = stub.tool_policy_mode;
    ...
    fn session_from_remote_startup_snapshot(snapshot: RemoteStartupSessionSnapshot) -> Self {
        ...
        session.current_mission_brief = snapshot.current_mission_brief;
        ...
        session.tool_policy_mode = snapshot.tool_policy_mode;
    ...
    fn journal_meta(&self) -> SessionJournalMeta {
        SessionJournalMeta {
            ...
            current_mission_brief: self.current_mission_brief.clone(),
    ...
    fn apply_journal_meta(&mut self, meta: SessionJournalMeta) {
        ...
        self.current_mission_brief = meta.current_mission_brief;
    ...
    pub fn create_with_id(
        ...
        let mut session = Self {
            ...
            subagent_model: None,
            tool_policy_mode: SessionToolPolicyMode::Normal,
            improve_mode: None,
...
request in this new forked session, using the inherited conversation only as context.\n</system-reminder>",
    ...
    /// Aggregate token usage across every stored message.
    ///
    /// Equivalent to [`Session::token_usage_totals_since`] with `start_index = 0`,
    /// so the full-session aggregate stays available alongside the range helper.
    pub fn token_usage_totals(&self) -> crate::protocol::TokenUsageTotals {
        self.token_usage_totals_since(0)
    }

    /// Aggregate token usage across stored messages whose index is
    /// `>= start_index`.
    ///
    /// Per-turn observers must use this so prior-session usage is excluded
    /// from per-turn emissions (the Langfuse GENERATION bridge would otherwise
    /// re-count the same provider call once per turn and inflate usage).
    /// `start_index` past the end of `messages` simply yields zeroed totals;
    /// saturation and the cache-field-only `cache_reported_input_tokens`
    /// semantics from [`Session::token_usage_totals`] are preserved.
    pub fn token_usage_totals_since(
        &self,
        start_index: usize,
    ) -> crate::protocol::TokenUsageTotals {
        let mut totals = crate::protocol::TokenUsageTotals {
            cache_prompt_tokens: Some(0),
            ..Default::default()
        };
        for message in self.messages.iter().skip(start_index) {
