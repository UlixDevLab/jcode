// sonar-file-size: {"waive":350,"owner":"jcode-maintainers","ticket":"Jcode-control-plane-policy","baseline_loc":769,"max_loc":769,"expires":"2026-12-01","removal_trigger":"module reaches 350 LOC","reason":"Policy extraction keeps inherited module at or below mission-base LOC."}
//! Configuration file support for jcode
//!
//! Config is loaded from `~/.jcode/config.toml` (or `$JCODE_HOME/config.toml`)
//! Environment variables override config file settings.
pub use jcode_config_types::{
    AccountResolution, AccountRule, AccountsConfig, AgentsConfig, AmbientConfig, AuthConfig,
    AutoJudgeConfig, AutoReviewConfig, CompactionConfig, CompactionMode, ConsentConfig,
    ConsentNavigationPolicy, CrossProviderFailoverMode, DiagramDisplayMode, DiagramPanePosition,
    ...
    PowerConfig, ProviderConfig, ReasoningDisplayMode, RlmConfig, RlmManifestRetention, RlmMode,
    SafetyConfig, SessionPickerResumeAction, SessionToolPolicyMode, SponsorsConfig, SwarmSpawnMode,
...
pub struct Config {
    ...
    /// Per-directory account (subscription) bindings, keyed by provider prefix.
    ///
    /// Lets sessions under a given directory bill a specific logged-in account.
    /// Skipped when empty so saving config never writes an empty table.
    #[serde(default, skip_serializing_if = "AccountsConfig::is_empty")]
    ...
    /// Experimental call-scoped context assembly. Effective only in the `jcode-rlm` launcher.
    pub rlm: RlmConfig,
...
#[cfg(test)]
#[path = "config_rlm_tests.rs"]
mod rlm_tests;

#[cfg(test)]
#[path = "config_consent_tests.rs"]
mod consent_tests;
