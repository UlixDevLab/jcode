...
// ---------------------------------------------------------------------------
// Skill description summarization
// ---------------------------------------------------------------------------

#[test]
fn skill_summary_normalizes_whitespace_and_caps_unicode_safely() {
    use crate::prompt::summarize_skill_description;

    assert_eq!(
        summarize_skill_description("  concise\n\t skill   description  "),
        "concise skill description"
    );

    let long = format!("Use this skill for complex research. {}", "🔬".repeat(200));
    let summary = summarize_skill_description(&long);
...
#[test]
fn full_and_split_prompts_use_the_same_bounded_skill_summary() {
    use crate::prompt::summarize_skill_description;

    let skill = SkillInfo {
        name: "research".to_string(),
        description: format!(
            "Use for complex repository research and evidence collection. {}",
            "detail ".repeat(40)
        ),
    };
    let summary = summarize_skill_description(&skill.description);
    let expected_line = format!("- `/research ` - {summary}");

    let (full, full_info) = build_system_prompt_with_context(
        Some("active skill body"),
        std::slice::from_ref(&skill),
        false,
    );
    let (split, split_info) =
        build_system_prompt_split(None, std::slice::from_ref(&skill), false, None, None);

    assert!(full.contains(&expected_line));
    assert!(split.static_part.contains(&expected_line));
    assert!(!full.contains(&skill.description));
    assert_eq!(full_info.skills_chars, split_info.skills_chars);
}

/// The operator's own `~/.jcode/swarm-prompt.md` must actually deliver the
/// reporting rules to a session.
///
/// These rules exist because reports kept assuming context the reader did not
/// have, and the operator had to ask what a noun referred to. That failure is
/// invisible: a clarifying question reads as ordinary conversation, so nothing
/// ever surfaces as a defect. Writing the rule is therefore not evidence it
/// arrives; only loading the real file through the real loader is.
///
/// Skipped when no global override exists, so CI and fresh checkouts stay green.
#[test]
fn operator_swarm_prompt_delivers_the_reporting_rules() {
    let _guard = crate::storage::lock_test_env();
    // Resolve the path exactly as the loader does. jcode_dir() honours
    // JCODE_HOME and only falls back to ~/.jcode, so reading dirs::home_dir()
    // directly would check a file the loader may never open: on a machine with
    // JCODE_HOME set, the test would pass against rules that never reach a
    // session, which is precisely the failure this test exists to catch.
    let Ok(jcode_home) = jcode_storage::jcode_dir() else {
        return;
    };
    let global = jcode_home.join("swarm-prompt.md");
    if !global.is_file() {
        return; // no operator override on this machine
    }
    let Ok(raw) = std::fs::read_to_string(&global) else {
        return;
    };
    ...
    // Assert against what the loader actually returns, not against the file on
    // disk. Those differ: a project-level .jcode/swarm-prompt.md overrides the
    // global one, and a blank file falls through. Reading the file directly
    // would pass while a session receives something else entirely.
    let delivered = load_swarm_prompt(None);

    // Each requirement is checked individually rather than as a count, so a
    // failure names which rule went missing.
    for (requirement, needle) in [
        (
            "name the referent on first mention",
            "Name the referent on first mention",
        ),
        (
            "re-measure numbers at report time",
            "Re-measure every number at report time",
        ),
        ("cap the report length", "one page"),
        ("keep findings on disk, not in the report", "name its path"),
    ] {
        assert!(
            raw.contains(needle),
            "swarm-prompt.md no longer states the rule: {requirement}"
        );
        assert!(
            delivered.contains(needle),
            "load_swarm_prompt does not deliver the rule to a session: {requirement}"
        );
    }
}
...
/// The canonical engineering-priority policy is one global overlay, rather than
/// a role copy, so it must reach both provider prompt formats exactly once.
#[test]
fn operator_engineering_priorities_reach_full_and_split_prompts() {
    let _guard = crate::storage::lock_test_env();
    const POLICY: &str = r#"## Engineering priorities

**Purpose over procedure.** Within explicit scope and permissions, and within safety bounds, prioritize user intent, agreed scope, correctness, and maintainability. Procedures, templates, and metrics are aids, not goals.

- Choose the smallest coherent solution with clear responsibilities and dependencies, and proportionate verification. Use SOLID and clean-code principles to guide concrete design, not abstraction for its own sake.
- Do not invent tasks, tests, file splits, agents, or infrastructure merely to satisfy formality. Refactor demonstrated problems only, not toward a quota.
- If a procedure conflicts, name the conflict and propose the smallest justified simplification. Never bypass enforced safety, approval, or data-integrity protections.
- Engineering excellence does not authorize scope expansion."#;
    let previous_home = std::env::var_os("JCODE_HOME");
    let jcode_home = tempfile::tempdir().expect("temporary JCODE_HOME");
    crate::env::set_var("JCODE_HOME", jcode_home.path());
    std::fs::write(jcode_home.path().join("prompt-overlay.md"), POLICY)
        .expect("immutable engineering-priority fixture");
    let project = tempfile::tempdir().expect("empty project directory");
    let (full, _) = build_system_prompt_full(None, &[], false, None, Some(project.path()));
    ...
    for prompt in [&full, &split.static_part] {
        assert_eq!(
            prompt.match_indices(POLICY).count(),
            1,
            "the common prompt builder must deliver the exact policy exactly once"
        );
    }
}
