...
#[test]
fn minimax_token_plan_keys_keep_the_international_endpoint_unless_explicitly_overridden() {
    let _lock = crate::storage::lock_test_env();
    ...
    crate::env::remove_var("MINIMAX_API_BASE");
    ...
    let token_plan = resolve_openai_compatible_profile_with_api_key_hint(
        MINIMAX_PROFILE,
        Some("sk-cp-test-token"),
    );
    assert_eq!(token_plan.api_base, "https://api.minimax.io/v1");
    assert_eq!(
        token_plan.setup_url,
        "https://platform.minimax.io/docs/guides/text-generation"
    );

    crate::env::set_var("MINIMAX_API_BASE", MINIMAX_CHINA_API_BASE);
...
#[test]
fn minimax_default_provider_applies_provider_specific_api_key_env() {
    ...
    // MINIMAX_API_KEY / minimax.env, not an ambient OPENAI_API_KEY or the
    // generic OPENROUTER_API_KEY / openrouter.env. The earlier bug surfaced as
    ...
    assert_eq!(
        ...
        "MiniMax profile must isolate its key from OpenAI and OpenRouter credentials"
    );
    assert_eq!(
