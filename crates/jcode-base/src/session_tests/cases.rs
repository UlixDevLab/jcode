// sonar-file-size: waive=350 reason=pre-existing legacy session test cases module; targeted persistence compatibility tests belong with the existing session loader coverage and full decomposition is out of scope
use super::*;
...
#[test]
fn token_usage_totals_since_excludes_prior_session_messages() {
    // Mirrors a turn whose `start_message_index` slices past the prefix that
    // already had its own provider rounds. The range helper must not double
    // count those earlier rounds when the Langfuse bridge maps this turn's
    // emissions into a single GENERATION.
    let mut session = Session::create_with_id(
        "session_token_usage_totals_since_test".to_string(),
        None,
        Some("Token totals since".to_string()),
    );

    // Prior-session assistant rounds (must NOT be counted by the turn below).
    session.add_message_ext(
        Role::Assistant,
        vec![ContentBlock::Text {
            text: "prior-1".to_string(),
            cache_control: None,
        }],
        None,
        Some(StoredTokenUsage {
    ...
    let prior_marker_index = session.messages.len();
    assert_eq!(prior_marker_index, 2);

    // Turn emitted by the current assistant invocation: multiple provider
    // rounds with mixed cache telemetry semantics.
    session.add_message_ext(
        Role::User,
        vec![ContentBlock::Text {
            text: "user-followup".to_string(),
            cache_control: None,
        }],
        None,
        None,
    ...
    // Mixed tool-result messages without usage must not affect totals.
    session.add_message_ext(
        Role::User,
    ...
    let full_totals = session.token_usage_totals();
    assert_eq!(full_totals.messages_with_token_usage, 4);
    assert_eq!(full_totals.input_tokens, 3_100);
    assert_eq!(full_totals.output_tokens, 310);
    assert_eq!(full_totals.cache_reported_input_tokens, 3_070);
    assert_eq!(full_totals.cache_read_input_tokens, 2_440);
    assert_eq!(full_totals.cache_creation_input_tokens, 60);

    let turn_totals = session.token_usage_totals_since(prior_marker_index);
    assert_eq!(turn_totals.messages_with_token_usage, 2);
    assert_eq!(turn_totals.input_tokens, 100);
    assert_eq!(turn_totals.output_tokens, 10);
    assert_eq!(turn_totals.cache_reported_input_tokens, 70);
    assert_eq!(turn_totals.cache_read_input_tokens, 40);
    assert_eq!(turn_totals.cache_creation_input_tokens, 10);

    // Past-end start yields a fully-zero aggregate (no emission).
    let beyond = session.token_usage_totals_since(session.messages.len() + 5);
    assert_eq!(beyond.messages_with_token_usage, 0);
    assert_eq!(beyond.input_tokens, 0);
    assert_eq!(beyond.output_tokens, 0);
    assert_eq!(beyond.cache_reported_input_tokens, 0);
    assert_eq!(beyond.cache_read_input_tokens, 0);
    assert_eq!(beyond.cache_creation_input_tokens, 0);
}
...
#[test]
fn current_mission_brief_persists_through_full_and_fast_session_loads() -> Result<()> {
    let _env_lock = lock_env();
    let temp_home = tempfile::Builder::new()
        .prefix("jcode-session-current-brief-test-")
        .tempdir()
        .map_err(|e| anyhow!(e))?;
    let _home = EnvVarGuard::set("JCODE_HOME", temp_home.path().as_os_str());
    let brief = CurrentMissionBrief {
        outcome: "Persist the current brief.".to_string(),
        in_scope: vec!["resume".to_string()],
        out_of_scope: vec!["history rewrite".to_string()],
        constraints: vec!["bounded context".to_string()],
        acceptance_observations: vec!["all session loaders retain it".to_string()],
        autonomous_decisions: vec![],
        unresolved_user_decisions: vec![],
    };
    let mut session = Session::create_with_id(
        "session_current_brief_persist_test".to_string(),
        None,
        Some("current brief persistence test".to_string()),
    );
    session.current_mission_brief = Some(brief.clone());
    session.save()?;

    assert_eq!(
        Session::load("session_current_brief_persist_test")?.current_mission_brief,
        Some(brief.clone())
    );
    assert_eq!(
        Session::load_startup_stub("session_current_brief_persist_test")?.current_mission_brief,
        Some(brief.clone())
    );
    assert_eq!(
        Session::load_for_remote_startup("session_current_brief_persist_test")?
...
#[test]
fn legacy_session_json_without_current_brief_remains_absent_after_roundtrip() -> Result<()> {
    let mut encoded = serde_json::to_value(Session::create_with_id(
        "legacy_current_brief_test".to_string(),
        None,
        None,
    ))?;
    encoded
        .as_object_mut()
        .expect("session encodes as object")
        .remove("current_mission_brief");
    let legacy: Session = serde_json::from_value(encoded)?;
    assert!(legacy.current_mission_brief.is_none());
    assert!(
        !serde_json::to_value(legacy)?
            .as_object()
            .expect("session encodes as object")
            .contains_key("current_mission_brief")
    );
    Ok(())
}

#[test]
fn legacy_intent_metadata_is_ignored_by_all_session_loaders() -> Result<()> {
    let _env_lock = lock_env();
    let temp_home = tempfile::Builder::new()
        .prefix("jcode-session-legacy-intent-test-")
        .tempdir()
        .map_err(|e| anyhow!(e))?;
    let _home = EnvVarGuard::set("JCODE_HOME", temp_home.path().as_os_str());
    let session_id = "legacy_intent_metadata_test";
    ...
    let mut encoded: serde_json::Value = serde_json::from_str(&std::fs::read_to_string(&path)?)?;
    encoded
        .as_object_mut()
        .expect("session encodes as object")
        .insert(
            ...
            serde_json::json!({
                ...
                "routing": {
                    "luna": "FactsRequired",
                    "mage": "MageRequired",
                    "explicit_mage_request": false
                }
            }),
        );
    std::fs::write(&path, serde_json::to_vec(&encoded)?)?;

    assert_eq!(Session::load(session_id)?.messages.len(), 1);
    assert_eq!(Session::load_startup_stub(session_id)?.id, session_id);
    assert_eq!(Session::load_for_remote_startup(session_id)?.id, session_id);
    assert!(
        !serde_json::to_value(Session::load(session_id)?)?
            .as_object()
            .expect("session encodes as object")
            .contains_key("intent_decision")
    );
    Ok(())
}
