// sonar-file-size: waive=350 reason=pre-existing cross-provider ledger; this change adds shared token counters to the existing atomic read-modify-write path and splitting storage compatibility is out of scope
...
/// Token usage recorded by every Jcode agent process, regardless of whether
/// the serving route was a subscription, stable router, or direct API call.
#[derive(Debug, Clone, Serialize, Deserialize, Default, PartialEq, Eq)]
pub struct SharedTokenUsage {
    /// ISO week bucket (`YYYY-Www`) for the weekly counters below.
    #[serde(default)]
    pub week: String,
    #[serde(default)]
    ...
    #[serde(default)]
    pub all_time_input_tokens: u64,
    #[serde(default)]
    pub all_time_output_tokens: u64,
}
...
pub struct ProviderActivityStore {
    ...
    #[serde(default)]
    pub token_usage: SharedTokenUsage,
}
...
#[cfg(unix)]
fn acquire_write_lock() -> Option<std::fs::File> {
...
fn roll_token_usage(usage: &mut SharedTokenUsage) {
    let week = current_iso_week();
    if usage.week != week {
        usage.week = week;
        usage.week_input_tokens = 0;
        usage.week_output_tokens = 0;
    }
}
...
fn with_fresh_store(mutate: impl FnOnce(&mut ProviderActivityStore) -> bool) {
    ...
    // Multiple Jcode processes share this ledger. Serialize the read-modify-
    // write sequence where the platform exposes `flock`, otherwise two agent
    // completions can both read the same counters and the later write wins.
    let _write_lock = acquire_write_lock();
...
/// Accumulate provider-reported token usage across all Jcode sessions and
/// processes. This intentionally has no provider filter: stable-router,
/// subscription, and direct API calls all contribute to the shared totals.
pub fn record_token_usage(input_tokens: u64, output_tokens: u64) {
    if input_tokens == 0 && output_tokens == 0 {
        return;
    }
    with_fresh_store(move |store| {
        let usage = &mut store.token_usage;
        roll_token_usage(usage);
        usage.week_input_tokens = usage.week_input_tokens.saturating_add(input_tokens);
        usage.week_output_tokens = usage.week_output_tokens.saturating_add(output_tokens);
        usage.all_time_input_tokens = usage.all_time_input_tokens.saturating_add(input_tokens);
        usage.all_time_output_tokens = usage.all_time_output_tokens.saturating_add(output_tokens);
        true
    });
}

/// Shared token totals with the current ISO-week bucket rolled forward.
pub fn token_usage_snapshot() -> Option<SharedTokenUsage> {
    let mut guard = match LEDGER.lock() {
        Ok(guard) => guard,
        Err(poisoned) => poisoned.into_inner(),
    };
    ...
    roll_token_usage(&mut cached.store.token_usage);
    let usage = cached.store.token_usage.clone();
    (usage.all_time_input_tokens > 0 || usage.all_time_output_tokens > 0).then_some(usage)
}
...
mod tests {
    ...
    #[test]
    fn shared_token_usage_roundtrips_week_and_all_time_totals() {
        let _env_lock = lock_env();
        clear_ledger_cache();
        let temp = tempfile::tempdir().expect("tempdir");
        let _home = EnvVarGuard::set("JCODE_HOME", temp.path().as_os_str());

        record_token_usage(100, 25);
        record_token_usage(50, 10);

        let usage = token_usage_snapshot().expect("token usage recorded");
        assert_eq!(usage.week, current_iso_week());
        assert_eq!(usage.week_input_tokens, 150);
        assert_eq!(usage.week_output_tokens, 35);
        assert_eq!(usage.all_time_input_tokens, 150);
        assert_eq!(usage.all_time_output_tokens, 35);

        clear_ledger_cache();
        assert_eq!(token_usage_snapshot(), Some(usage));
    }
