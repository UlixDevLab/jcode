...
pub fn resolve_openai_compatible_profile_with_api_key_hint(
    ...
    apply_profile_endpoint_overrides(profile, &mut resolved);
...
fn apply_profile_endpoint_overrides(
    ...
    // Token Plan keys use the same `sk-cp-` prefix in both MiniMax regions, so
    // the key cannot safely select an endpoint. The previous prefix heuristic
    // sent valid international keys to api.minimaxi.com and produced a 401.
    // Keep the official international endpoint by default and require an
    // explicit override for China or a custom gateway.
    let Some(raw_base) = load_env_value_from_env_or_config("MINIMAX_API_BASE", profile.env_file)
    else {
        return;
    };
    let Some(api_base) = normalize_api_base(&raw_base) else {
        eprintln!(
            "Warning: ignoring invalid MINIMAX_API_BASE '{}'. Use https://... (or http://localhost).",
            raw_base
        );
        return;
    };
    ...
    if resolved.api_base == MINIMAX_CHINA_API_BASE {
