// sonar-file-size: {"waive":350,"owner":"jcode-maintainers","ticket":"Jcode-control-plane-policy","baseline_loc":841,"max_loc":841,"expires":"2026-12-01","removal_trigger":"module reaches 350 LOC","reason":"Policy extraction keeps inherited module at or below mission-base LOC."}
use super::*;
#[path = "env_overrides/tool_policy.rs"]
mod tool_policy;
impl Config {
    ...
    pub(crate) fn apply_env_overrides(&mut self) {
        ...
        tool_policy::apply_root_tool_policy_env(self);
        ...
        hook_env_override(&mut self.hooks.permission, "JCODE_HOOK_PERMISSION");
        ...
        // Comma-separated provider list to keep out of automatic failover, e.g.
        // JCODE_DISABLED_PROVIDERS=claude. Set to an empty value to re-enable
        // everything for one run without editing config.
        ...
        // Anthropic OAuth subscription guard overrides (DOD: env vars prefixed
        // anthropic_subscription_guard_). The runtime guard core also reads
        // these directly so a config reload is not required.
        if let Ok(v) = std::env::var("JCODE_ANTHROPIC_SUBSCRIPTION_GUARD_ENABLED") {
            if let Some(parsed) = parse_env_bool(&v) {
                self.provider.anthropic_subscription_guard_enabled = parsed;
            }
        }
        if let Ok(v) = std::env::var("JCODE_ANTHROPIC_SUBSCRIPTION_GUARD_THRESHOLD_PERCENT") {
            if let Ok(parsed) = v.trim().parse::<f32>() {
                if parsed.is_finite() && parsed >= 0.0 {
                    self.provider.anthropic_subscription_guard_threshold_percent = parsed;
                }
            }
        }
        if let Ok(v) = std::env::var("JCODE_ANTHROPIC_SUBSCRIPTION_GUARD_MAX_PREFLIGHT_PERCENT") {
            if let Ok(parsed) = v.trim().parse::<f32>() {
                if parsed.is_finite() && (0.0..=100.0).contains(&parsed) {
                    self.provider
                        .anthropic_subscription_guard_max_preflight_percent = parsed;
                }
            }
        }
        if let Ok(v) = std::env::var("JCODE_ANTHROPIC_SUBSCRIPTION_GUARD_FAIL_CLOSED") {
            if let Some(parsed) = parse_env_bool(&v) {
                self.provider.anthropic_subscription_guard_fail_closed = parsed;
            }
        }
        if let Ok(v) = std::env::var("JCODE_ANTHROPIC_SUBSCRIPTION_GUARD_ALLOWED_MODELS") {
            self.provider.anthropic_subscription_guard_allowed_models = parse_env_list(&v);
        }
