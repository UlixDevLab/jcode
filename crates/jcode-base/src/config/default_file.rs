// sonar-file-size: {"waive":350,"owner":"jcode-maintainers","ticket":"Jcode-control-plane-policy","baseline_loc":823,"max_loc":823,"expires":"2026-12-01","removal_trigger":"module reaches 350 LOC","reason":"Policy extraction keeps inherited module at or below mission-base LOC."}
use super::*;
...
mermaid = true
# Auto-poke: opt in to post-turn continuation when the model stops with
# incomplete todos. It defaults off so the user-facing report remains the
# final visible output; /poke on and /poke off override this per session.
auto_poke = false
...
[consent]
# Default allows only typed local app/file and HTTP(S) page navigation.
# Set "prompt" to require approval for navigation too. Input and desktop mutations always prompt.
navigation = "allow"
...
# Defaults to `off` so OpenAI uses its standard tier. Set `priority` only when
# lower latency is worth the higher usage.
...
# Providers to keep OUT of automatic failover, even when fully logged in.
# Use this to stop jcode drifting onto a subscription you want to rest, e.g.
# work on MiniMax only for a few days without logging out of Anthropic:
#   disabled_providers = ["claude"]
# Explicitly picking the provider with /model still works, so you are never
# locked out of a plan you pay for. Override for one run with
# JCODE_DISABLED_PROVIDERS=claude (empty value re-enables everything).
# disabled_providers = []
...
# ── Anthropic OAuth subscription guard ────────────────────────────────────
# Serializes every Anthropic OAuth call behind a single cross-process
# exclusive lease so two sessions can't burst against the same 5-hour
# subscription window between measurements. State (deltas, kills, in-flight
# markers) lives under `JCODE_HOME`. Credentials never persist in the state.
# Disabled by default for upstream users. Env override prefix:
# JCODE_ANTHROPIC_SUBSCRIPTION_GUARD_
anthropic_subscription_guard_enabled = false
# Move the observed 5-hour utilization by at least this many percentage points
# and future subscription-backed calls pause for operator approval. The call
# that observed the movement keeps its completed response. Default 5.0.
# anthropic_subscription_guard_threshold_percent = 5.0
# Require operator approval before a new OAuth call when preflight 5-hour
# utilization is at or above this percentage (0-100). Default 95.0.
# anthropic_subscription_guard_max_preflight_percent = 95.0
# Compatibility setting retained for older config files. Meter failures now
# produce a warning and telemetry without creating a durable pause.
# anthropic_subscription_guard_fail_closed = true
# Optional explicit allowlist of model ids the monitor will admit. Empty or
# `claude-*` admits supported Claude models, including Haiku and older models.
# Fable is the one exception: it is admitted only when listed by its exact id,
# so neither an empty list nor `claude-*` can select it by accident.
# Comma-separated env override:
# JCODE_ANTHROPIC_SUBSCRIPTION_GUARD_ALLOWED_MODELS=claude-*
# anthropic_subscription_guard_allowed_models = ["claude-*"]
# To make Fable selectable as a session's main model, name it exactly:
# anthropic_subscription_guard_allowed_models = ["claude-*", "claude-fable-5"]
...
#
# Root sessions: "normal" (default) or "control-plane-only"; persisted workers remain normal. Env: JCODE_ROOT_TOOL_POLICY_MODE.
# root_tool_policy_mode = "normal"
...
# Use the special value "pool" or "rotate" to draw the next non-blank model from
# `swarm_model_pool` below (process-global atomic round-robin); an empty pool
# with that sentinel fails clearly instead of silently inheriting.
# Env override: JCODE_SWARM_MODEL
# swarm_model = "inherit"
#
# Optional model pool used by the "pool" / "rotate" sentinel above. Each
# request rotates through the non-blank entries in declaration order; blank
# entries are skipped. Unset / empty list = no rotation.
# swarm_model_pool = ["minimax:MiniMax-M2.7-highspeed", "gpt-5.6-luna"]
#
...
# post_tool = ""
#
# Runs when the agent BLOCKS on you for consent to a protected operation, and
# again when that wait ends. This is the only place jcode is genuinely stuck
# waiting on a human, so it is the signal to use for "needs my attention"
# notifications — pre_tool fires for every tool call, including ones that are
# auto-approved and never prompt. Extra fields: JCODE_HOOK_STATE
# ("waiting"/"resolved"), JCODE_HOOK_TOOL_NAME, JCODE_HOOK_ACTION,
# JCODE_HOOK_TARGET, and on resolve JCODE_HOOK_OUTCOME
# ("approved"/"denied"/"timeout"/"disconnected") + JCODE_HOOK_WAITED_MS.
# Every "waiting" is followed by exactly one "resolved", so an indicator armed
# on the first is always disarmed by the second.
# permission = ""
...
[rlm]
# Experimental context mode used only by the `jcode-rlm` launcher.
# Ordinary `jcode` ignores this section and keeps transcript-based requests.
mode = "off" # off | observe | pilot (pilot currently fails closed to observe)
max_depth = 1
max_children = 0
...
mod tests {
    ...
    #[test]
    fn default_config_template_documents_safe_rlm_defaults() {
        let template = Config::default_config_file_contents();
        assert!(template.contains("[rlm]"));
        assert!(template.contains("mode = \"off\""));
        let parsed: Config = toml::from_str(&template).expect("parse template");
        assert_eq!(parsed.rlm.mode, RlmMode::Off);
        assert_eq!(parsed.rlm.max_depth, 1);
        assert_eq!(parsed.rlm.max_children, 0);
    }
