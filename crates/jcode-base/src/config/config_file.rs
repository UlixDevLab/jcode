...
impl Config {
    ...
    pub fn save(&self) -> anyhow::Result<()> {
        ...
        let content = Self::preserve_unrepresentable_tables(&path, content);
        std::fs::write(&path, content)?;
        Self::invalidate_cache();
        Ok(())
    }

    /// Re-attach top-level tables that exist on disk but that this binary's
    /// `Config` has no field for at all.
    ///
    /// Saving is a deserialize/serialize round-trip, so a binary older than a
    /// config field silently deletes it. That really happened here: several
    /// long-lived processes (notably the shared-server daemon) keep running an
    /// older build, and a config write from one of them dropped the whole
    /// `[accounts]` section, silently disabling per-directory subscription
    /// binding and sending company work back to the default account.
    ///
    /// The test is "does `Config` have this field", not "is it missing from
    /// the new output". Known tables are sometimes omitted on purpose (e.g.
    /// `[sponsors]` when a frozen opt-out is being cleared), and resurrecting
    /// those would undo deliberate migrations.
    ///
    /// Best-effort: any parse failure leaves the new content untouched, so
    /// this can never block a save.
    fn preserve_unrepresentable_tables(path: &std::path::Path, content: String) -> String {
        let Ok(existing) = std::fs::read_to_string(path) else {
            return content;
        };
        let (Ok(existing), Ok(mut new)) = (
        ...
        for (key, value) in existing {
            if !value.is_table() || new.contains_key(&key) {
                continue;
            }
            if Self::key_is_known(&key, &value) {
                // Known to this binary but intentionally not serialized.
                continue;
            }
        ...
        match toml::to_string_pretty(&new) {
            Ok(merged) => {
                crate::logging::info(&format!(
                    "config: preserved table(s) this binary has no field for: {}. Likely written \
                     by a newer build; keeping them so a save does not delete them.",
                    restored.join(", ")
                ));
                merged
            }
    ...
    /// Whether this binary's `Config` understands a top-level table.
    ///
    /// Probes by deserializing `{key = value}` on its own: if `Config` has a
    /// matching field, the parsed config differs from `Config::default()`;
    /// if the field does not exist, `#[serde(default)]` ignores the key and
    /// the result is exactly the default.
    ///
    /// Deserialization rather than serialization is the right test because a
    /// field with `skip_serializing_if` (e.g. `[sponsors]`, deliberately
    /// omitted to clear a frozen opt-out) is absent from serialized output yet
    /// is still very much a known field, and resurrecting it would undo an
    /// intentional migration.
    ///
    /// A table that merely equals the default is reported as unknown, which is
    /// harmless: preserving it rewrites the same values.
    fn key_is_known(key: &str, value: &toml::Value) -> bool {
        let mut probe = toml::Table::new();
        probe.insert(key.to_string(), value.clone());
        match probe.try_into::<Config>() {
            Ok(parsed) => {
                let parsed = toml::Value::try_from(&parsed).ok();
                let default = toml::Value::try_from(Config::default()).ok();
                parsed != default
            }
            // Failing to parse means the field exists but the value is
            // malformed; that is a known key, so leave it to normal handling.
            Err(_) => true,
        }
    }
...
#[cfg(test)]
mod save_preservation_tests {
    use super::*;

    /// Regression: a binary must not delete config tables it has no field
    /// for. This really happened to `[accounts]`, written by a new build and
    /// then erased by the older shared-server daemon on its next save, which
    /// silently disabled per-directory subscription binding.
    ///
    /// Uses a table no `Config` field can claim, to stand in for "written by a
    /// newer build than the one saving".
    #[test]
    fn save_preserves_tables_this_binary_has_no_field_for() {
        let dir = tempfile::tempdir().expect("tempdir");
        let path = dir.path().join("config.toml");
        std::fs::write(
            &path,
            "[feature_from_a_newer_build]\n\
             enabled = true\n\
             mode = \"strict\"\n",
        )
        .unwrap();
        ...
        let merged = Config::preserve_unrepresentable_tables(&path, serialized);

        let parsed: toml::Table = merged.parse().expect("merged config parses");
        assert!(
            parsed.contains_key("feature_from_a_newer_build"),
            "a table this binary cannot represent must survive a save"
        );
        assert_eq!(
            parsed["feature_from_a_newer_build"]["mode"].as_str(),
            Some("strict"),
            "preserved table must keep its contents, not just its name"
        );
        assert!(
            parsed.contains_key("provider"),
            "the saving binary's own tables must still be written"
        );
    }

    /// `[sponsors]` is omitted on purpose when clearing a machine-frozen
    /// opt-out, so a known-but-skipped table must not be resurrected.
    #[test]
    fn save_does_not_resurrect_known_tables_that_were_intentionally_omitted() {
        let dir = tempfile::tempdir().expect("tempdir");
        let path = dir.path().join("config.toml");
        std::fs::write(&path, "[sponsors]\nenabled = false\n").unwrap();

        let serialized = "[provider]\ndefault_model = \"gpt-5\"\n".to_string();
        let merged = Config::preserve_unrepresentable_tables(&path, serialized);

        let parsed: toml::Table = merged.parse().expect("parses");
        assert!(
            !parsed.contains_key("sponsors"),
            "a known field skipped on purpose must stay out of the saved file"
        );
    }

    #[test]
    fn save_does_not_resurrect_tables_the_binary_intentionally_dropped() {
        let dir = tempfile::tempdir().expect("tempdir");
        let path = dir.path().join("config.toml");
        std::fs::write(&path, "[provider]\ndefault_model = \"old\"\n").unwrap();

        // `provider` is known and present, so the new value must win rather
        // than being merged back to the on-disk one.
        let serialized = "[provider]\ndefault_model = \"new\"\n".to_string();
        let merged = Config::preserve_unrepresentable_tables(&path, serialized);

        let parsed: toml::Table = merged.parse().expect("parses");
        assert_eq!(parsed["provider"]["default_model"].as_str(), Some("new"));
    }
}
