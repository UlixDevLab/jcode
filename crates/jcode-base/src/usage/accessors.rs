...
fn active_openai_usage_account_label() -> Option<String> {
    let label = auth::codex::active_account_label()?;
    let email = auth::codex::list_accounts().ok().and_then(|accounts| {
        accounts
            .into_iter()
            .find(|account| account.label == label)
            .and_then(|account| account.email)
    });
    Some(email.map(|email| mask_email(&email)).unwrap_or(label))
}

async fn fetch_openai_usage_data() -> OpenAIUsageData {
    let account_label = active_openai_usage_account_label();
    let mut data = match fetch_openai_usage_report().await {
