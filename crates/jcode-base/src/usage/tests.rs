...
#[test]
fn test_openai_usage_report_includes_explicit_local_estimates() {
    let report = provider_report_from_openai_usage_data(
        "OpenAI (ChatGPT)".to_string(),
        &OpenAIUsageData::default(),
    );
    assert_eq!(report.extra_info.len(), 4);
    assert!(report.extra_info.iter().all(|(key, value)| {
        key.starts_with("Local estimate:") && value.contains("not provider quota")
    }));
}
