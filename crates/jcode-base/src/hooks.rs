...
// sonar-file-size: waive=350 reason=pre-existing legacy hook module; this mission adds only trusted native-consent metadata and full decomposition is out of scope
//!
...
/// A permission lifecycle pair must not let a stuck observer prevent the
/// terminal event from being delivered. This bound is enforced on the
/// background dispatcher, never on the consent request task.
const ORDERED_OBSERVER_COMMAND_TIMEOUT: Duration = Duration::from_secs(1);
...
pub fn hook_commands(event: &str) -> Vec<String> {
    ...
    let raw = match event {
        ...
        "permission" => hooks.permission.as_ref(),
        _ => None,
    };
    let mut commands: Vec<String> = raw
        .into_iter()
    ...
    // A shared daemon outlives the terminal that started it. Lifecycle hook
    // overrides injected by Cate therefore arrive with each client's Subscribe
    // request instead of the daemon process environment. Add that client-local
    // observer without replacing the user's process-wide hook configuration.
    if let Some(key) = client_hook_env_key(event)
        && let Ok(client_env) = CLIENT_TERMINAL_ENV.try_with(|env| env.clone())
        && let Some(command) = client_env
            .iter()
            .find_map(|(name, value)| (name == key).then_some(value.trim()))
            .filter(|command| !command.is_empty())
        && !commands.iter().any(|configured| configured == command)
    {
...
fn client_hook_env_key(event: &str) -> Option<&'static str> {
    match event {
        "turn_start" => Some("JCODE_HOOK_TURN_START"),
        "turn_end" => Some("JCODE_HOOK_TURN_END"),
        "session_start" => Some("JCODE_HOOK_SESSION_START"),
        "session_end" => Some("JCODE_HOOK_SESSION_END"),
        "pre_tool" => Some("JCODE_HOOK_PRE_TOOL"),
        "post_tool" => Some("JCODE_HOOK_POST_TOOL"),
        "permission" => Some("JCODE_HOOK_PERMISSION"),
        _ => None,
    }
}
...
fn build_hook_process(
    ...
    let client_terminal_env = CLIENT_TERMINAL_ENV.try_with(Clone::clone).ok();
    build_hook_process_with_client_terminal_env(command_line, event, client_terminal_env.as_deref())
}

fn build_hook_process_with_client_terminal_env(
    command_line: &str,
    event: &HookEvent,
    client_terminal_env: Option<&[(String, String)]>,
) -> anyhow::Result<std::process::Command> {
    ...
    if let Some(env) = client_terminal_env {
        crate::terminal_launch::apply_client_terminal_env(&mut cmd, env);
    }
    Ok(cmd)
}

struct OrderedObserverCommand {
...
/// A handle for appending to one ordered observer stream.
///
/// The dispatcher owns a dedicated background thread, so callers can append a
/// terminal event without waiting for an observer command. Events submitted to
/// one handle are run in submission order, with every command bounded.
pub struct OrderedObserver {
    sender: std::sync::mpsc::Sender<Vec<OrderedObserverCommand>>,
    command_lines: Vec<String>,
    client_terminal_env: Option<Vec<(String, String)>>,
}

impl OrderedObserver {
    /// Append the next event to this ordered stream. Observer failures remain
    /// best-effort and never reach the caller.
    pub fn dispatch(self, event: HookEvent) {
        let commands = prepare_ordered_observer_commands(
            &self.command_lines,
            &event,
            self.client_terminal_env.as_deref(),
        );
        if commands.is_empty() {
            return;
        }
        if self.sender.send(commands).is_err() {
            crate::logging::warn(&format!(
                "Ordered hook '{}' dispatcher stopped before the event could be delivered",
                event.event
            ));
        }
    }
...
/// Start an observer stream whose subsequent events must remain ordered.
///
/// This is intentionally narrower than `dispatch_observer`: ordinary lifecycle
/// observers remain independently fire-and-forget. Callers use this only for a
/// matched pair where a consumer must never receive the terminal state first.
pub fn begin_ordered_observer(event: HookEvent) -> Option<OrderedObserver> {
    ...
    let client_terminal_env = CLIENT_TERMINAL_ENV.try_with(Clone::clone).ok();
    let first_commands =
        prepare_ordered_observer_commands(&command_lines, &event, client_terminal_env.as_deref());
    ...
    if let Err(error) = std::thread::Builder::new()
        .name("jcode-ordered-hook".to_string())
        .spawn(move || {
            for commands in receiver {
                for command in commands {
                    run_ordered_observer_command(command);
                }
            }
        })
    {
        crate::logging::warn(&format!(
            "Ordered hook '{}' dispatcher failed to start: {error}",
            event.event
        ));
        return None;
    }
    // The receiver is live because the thread was started above. If it exits
    // immediately, the returned handle logs the later terminal delivery error.
    let _ = sender.send(first_commands);
    Some(OrderedObserver {
        sender,
        command_lines,
        client_terminal_env,
    })
...
fn prepare_ordered_observer_commands(
    command_lines: &[String],
    event: &HookEvent,
    client_terminal_env: Option<&[(String, String)]>,
) -> Vec<OrderedObserverCommand> {
    command_lines
        .iter()
        .filter_map(|command_line| {
            match build_hook_process_with_client_terminal_env(
                command_line,
                event,
                client_terminal_env,
            ) {
                ...
                Err(error) => {
                    crate::logging::warn(&format!(
                        "Hook '{}' command '{command_line}' is invalid: {error}",
                        event.event
                    ));
                    None
                }
...
fn run_ordered_observer_command(mut ordered: OrderedObserverCommand) {
    let event_name = ordered.event_name;
    match ordered.command.spawn() {
        Ok(mut child) => {
            let deadline = std::time::Instant::now() + ORDERED_OBSERVER_COMMAND_TIMEOUT;
            loop {
                match child.try_wait() {
                    Ok(Some(_)) => {
                        crate::logging::debug(&format!(
                            "Ordered hook '{event_name}' dispatched to '{}' (session={:?})",
                            ordered.command_line, ordered.session_id
                        ));
                        return;
                    }
                    Ok(None) if std::time::Instant::now() >= deadline => {
                        ...
                        let _ = child.wait();
                        crate::logging::warn(&format!(
                            "Ordered hook '{event_name}' command '{}' timed out after {}ms",
                            ordered.command_line,
                            ORDERED_OBSERVER_COMMAND_TIMEOUT.as_millis()
                        ));
                        return;
                    }
                    ...
                    Err(error) => {
                        crate::logging::warn(&format!(
                            "Ordered hook '{event_name}' command '{}' failed while waiting: {error}",
                            ordered.command_line
                        ));
                        return;
                    }
        ...
        Err(error) => crate::logging::warn(&format!(
            "Ordered hook '{event_name}' command '{}' failed to start: {error}",
            ordered.command_line
        )),
    }
}
...
///
/// Fire-and-forget: failures are logged, never propagated, and the hook process
/// cannot block the agent. Observer hooks are reaped but intentionally not
/// session-detached, so reload cleanup can terminate them before Unix `exec`.
        ...
        match build_hook_process(&command_line, &event) {
            Ok(mut cmd) => {
                cmd.stdin(std::process::Stdio::null())
                    .stdout(std::process::Stdio::null())
                    .stderr(std::process::Stdio::null());
                match crate::platform::spawn_reaped(&mut cmd) {
                    Ok(_) => {
                        crate::logging::debug(&format!(
...
pub async fn run_pre_tool_gate(
    ...
    if native_consent_granted {
        // This field originates only from Registry after a current TUI user
        // approves this in-flight call. It is not parsed from tool JSON.
        event = event.field("NATIVE_CONSENT", "1");
    }
...
mod tests {
    ...
    async fn pre_tool_gate_allows_on_exit_zero_and_blocks_on_exit_two() {
        ...
        );
        {
            ...
            let decision =
                run_pre_tool_gate("ses_g", None, "bash", r#"{"command":"rm -rf /"}"#, false).await;
            assert_eq!(
                decision,
                GateDecision::Block {
        ...
        let allow = write_executable_script(temp.path(), "allow.sh", "#!/bin/sh\nexit 0\n");
        {
            ...
            let decision = run_pre_tool_gate("ses_g", None, "read", "{}", false).await;
    ...
    async fn pre_tool_gate_runs_every_configured_command_and_preserves_first_block() {
        ...
        let decision = run_pre_tool_gate("ses_multi", None, "bash", "{}", false).await;
    ...
    async fn pre_tool_gate_fails_open_on_timeout_and_odd_exits() {
        ...
        let hang = write_executable_script(temp.path(), "hang.sh", "#!/bin/sh\nsleep 30\n");
        {
            ...
            let decision = run_pre_tool_gate("ses_g", None, "bash", "{}", false).await;
            assert_eq!(decision, GateDecision::Allow);
        }
    ...
    async fn pre_tool_gate_receives_input_on_stdin() {
        ...
        let decision = run_pre_tool_gate("ses_g", None, "write", input, false).await;
        assert_eq!(decision, GateDecision::Allow);
    ...
    #[cfg(unix)]
    #[tokio::test]
    async fn pre_tool_gate_exposes_native_consent_only_when_registry_sets_it() {
        let _guard = crate::storage::lock_test_env();
        let temp = tempfile::TempDir::new().expect("temp dir");
        let script = write_executable_script(
            temp.path(),
            "consent.sh",
            "#!/bin/sh\n[ \"$JCODE_HOOK_NATIVE_CONSENT\" = 1 ] || exit 2\n",
        );
        let _env = gate_test_config(&script.to_string_lossy(), 5000);

        let denied = run_pre_tool_gate("session", None, "browser", "{}", false).await;
        let allowed = run_pre_tool_gate("session", None, "browser", "{}", true).await;

        assert!(matches!(denied, GateDecision::Block { .. }));
    ...
    #[cfg(unix)]
    #[tokio::test]
    async fn ordered_observer_bounds_a_stuck_event_before_dispatching_the_terminal_event() {
        let _guard = crate::storage::lock_test_env();
        let temp = tempfile::TempDir::new().expect("temp dir");
        let record = temp.path().join("ordered-events.txt");
        let script = write_executable_script(
            temp.path(),
            "ordered.sh",
            &format!(
                "#!/bin/sh\nif [ \"$JCODE_HOOK_STATE\" = waiting ]; then exec /bin/sleep 30; fi\nprintf '%s|%s|%s|%s|%s' \"$JCODE_HOOK_STATE\" \"$TMUX_PANE\" \"$HERDR_PANE_ID\" \"$JCODE_CLIENT_TMUX_PANE\" \"$JCODE_CLIENT_HERDR_PANE_ID\" >> {}\n",
                crate::terminal_launch::sh_escape(&record.to_string_lossy())
            ),
        );
        let previous = std::env::var_os("JCODE_HOOK_PERMISSION");
        crate::env::set_var(
        ...
        with_client_terminal_env(
            vec![
                ("TMUX_PANE".to_string(), "ordered-pane".to_string()),
                ("HERDR_PANE_ID".to_string(), "ordered-herdr".to_string()),
            ],
            async {
                let observer =
                    begin_ordered_observer(HookEvent::new("permission").field("STATE", "waiting"))
                        .expect("permission observer");
                observer.dispatch(HookEvent::new("permission").field("STATE", "resolved"));
            },
        )
        .await;
        ...
        match previous {
            Some(value) => crate::env::set_var("JCODE_HOOK_PERMISSION", value),
            None => crate::env::remove_var("JCODE_HOOK_PERMISSION"),
        }
        crate::config::invalidate_config_cache();

        assert_eq!(
            recorded,
            "resolved|ordered-pane|ordered-herdr|ordered-pane|ordered-herdr"
        );
        assert!(
            started.elapsed() < std::time::Duration::from_secs(2),
            "a stuck observer must be bounded before the terminal event is dispatched"
        );
    }
    ...
    #[cfg(unix)]
    #[tokio::test]
    async fn client_scoped_hook_command_and_cate_context_survive_shared_server_dispatch() {
        let _guard = crate::storage::lock_test_env();
        let temp = tempfile::TempDir::new().expect("temp dir");
        let script = write_executable_script(temp.path(), "cate-hook.sh", "#!/bin/sh\nexit 0\n");
        let command_line = script.to_string_lossy().to_string();

        with_client_terminal_env(
            vec![
                ("JCODE_HOOK_TURN_END".to_string(), command_line.clone()),
                (
                    "CATE_HOOK_ENDPOINT".to_string(),
                    "http://127.0.0.1:4567".to_string(),
                ),
                ("CATE_HOOK_TOKEN".to_string(), "terminal-token".to_string()),
                ("CATE_TERMINAL_ID".to_string(), "rpty-cate".to_string()),
            ],
            async move {
                assert!(hook_commands("turn_end").contains(&command_line));
                let command = build_hook_process(&command_line, &HookEvent::new("turn_end"))
                    .expect("client-scoped hook command");
                let env = command
                    .get_envs()
                    .map(|(key, value)| {
                        (
                ...
                assert_eq!(
                    env.get("CATE_HOOK_ENDPOINT").and_then(Option::as_deref),
                    Some("http://127.0.0.1:4567")
                );
                assert_eq!(
                    env.get("CATE_HOOK_TOKEN").and_then(Option::as_deref),
                    Some("terminal-token")
                );
                assert_eq!(
                    env.get("CATE_TERMINAL_ID").and_then(Option::as_deref),
                    Some("rpty-cate")
                );
            },
        )
