...
#[test]
fn test_rule_bound_openai_account_is_excluded_from_failover() {
    // End-to-end over the real candidate path: an account bound to a directory
    // by an [accounts.openai] rule must not be offered as a failover target,
    // and a session already on it must not be moved off. Otherwise a rate limit
    // silently crosses the company/personal billing boundary.
    with_clean_provider_test_env(|| {
        let now_ms = chrono::Utc::now().timestamp_millis() + 60_000;
        crate::auth::codex::upsert_account(crate::auth::codex::OpenAiAccount {
            label: "seed-a".to_string(),
            ...
            account_id: Some("acct-a".to_string()),
            expires_at: Some(now_ms),
            email: Some("company@example.com".to_string()),
        })
        .unwrap();
        crate::auth::codex::upsert_account(crate::auth::codex::OpenAiAccount {
            label: "seed-b".to_string(),
        ...
        let home = std::env::var("JCODE_HOME").expect("test home");
        std::fs::write(
            std::path::Path::new(&home).join("config.toml"),
            "[accounts.openai]\n\
             default = \"personal@example.com\"\n\n\
             [[accounts.openai.rules]]\n\
             path = \"/tmp/company-work\"\n\
             account = \"company@example.com\"\n",
        )
        .unwrap();
        crate::config::invalidate_config_cache();

        // On the personal (default) account: the company account is reserved,
        // so there is nothing to fail over to.
        crate::auth::codex::set_active_account("openai-2").unwrap();
        assert_eq!(
            MultiProvider::same_provider_account_candidates(ActiveProvider::OpenAI),
            Vec::<String>::new(),
            "unbound session must not fail over onto a rule-bound company account"
        );

        // On the company account: exhaustion must surface, not spend personal.
        crate::auth::codex::set_active_account("openai-1").unwrap();
        assert_eq!(
            MultiProvider::same_provider_account_candidates(ActiveProvider::OpenAI),
            Vec::<String>::new(),
            "rule-bound session must not fail over away onto the personal account"
        );
    });
}
