...
/// `[provider].disabled_providers` must keep a provider out of *automatic*
/// failover. This is the "work on MiniMax only for a few days" control: the
/// operator stays logged into Anthropic but jcode stops drifting onto it.
#[test]
fn test_disabled_providers_are_skipped_by_automatic_failover() {
    let disabled = vec!["claude".to_string()];
    assert!(matches_disabled(ActiveProvider::Claude, &disabled));
    // Everything not named stays eligible.
    assert!(!matches_disabled(ActiveProvider::OpenAI, &disabled));
    assert!(!matches_disabled(ActiveProvider::OpenRouter, &disabled));
}

/// "anthropic" and "claude" name the same slot, and matching ignores case and
/// surrounding whitespace, so the operator does not have to guess a spelling.
#[test]
fn test_disabled_providers_accepts_anthropic_spelling_and_ignores_case() {
    for entry in [" Anthropic ", "CLAUDE", "anthropic"] {
        assert!(
            matches_disabled(ActiveProvider::Claude, &[entry.to_string()]),
            "{entry:?} should disable the Anthropic slot"
        );
    }
}

/// An empty list must disable nothing, so the default config path keeps every
/// provider eligible for failover.
#[test]
fn test_no_disabled_providers_leaves_every_provider_eligible() {
    for provider in [
        ActiveProvider::Claude,
        ActiveProvider::OpenAI,
        ActiveProvider::Copilot,
        ...
        assert!(!matches_disabled(provider, &[]));
    }
}

/// An unrelated entry must not disable anything, so a typo silently widens
/// nothing.
#[test]
fn test_unknown_disabled_entry_disables_nothing() {
    let disabled = vec!["not-a-provider".to_string()];
    for provider in [
        ActiveProvider::Claude,
        ActiveProvider::OpenAI,
        ActiveProvider::OpenRouter,
    ] {
        assert!(!matches_disabled(provider, &disabled));
    }
}

fn matches_disabled(provider: ActiveProvider, disabled: &[String]) -> bool {
    crate::provider::failover::provider_matches_disabled_entry(provider, disabled)
}
