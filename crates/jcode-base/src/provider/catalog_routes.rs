...
const PROVIDER_ALLOWLIST_ENV: &str = "JCODE_PROVIDER_ALLOWLIST";

/// Restrict picker routes to an explicitly allowed set of provider identities.
///
/// This is intentionally an opt-in environment seam rather than a general
/// config setting. Distributions such as Jcode Lite can ship a curated provider
/// surface without changing normal Jcode behavior or deleting provider code.
/// Tokens match the rendered provider, the route method, or the named-profile
/// suffix in methods such as `openai-compatible:stables`.
fn apply_provider_allowlist(routes: Vec<ModelRoute>) -> Vec<ModelRoute> {
    let Some(raw) = std::env::var_os(PROVIDER_ALLOWLIST_ENV) else {
        return routes;
    };
    let allowed = raw
        .to_string_lossy()
    ...
    routes
        ...
        .filter(|route| {
            ...
            let method = route.api_method.trim().to_ascii_lowercase();
            let profile = method.rsplit_once(':').map(|(_, suffix)| suffix);
            allowed.iter().any(|candidate| {
                candidate == &provider
                    || candidate == &method
                    || profile.is_some_and(|suffix| candidate == suffix)
            })
        })
        .collect()
}
...
pub fn simplified_model_routes_for_picker(
    ...
    let mut routes = apply_provider_allowlist(routes);

    if routes.is_empty()
        && !current_model.is_empty()
        && current_model != "unknown"
        && apply_provider_allowlist(vec![ModelRoute {
...
pub(super) fn multiprovider_model_routes(provider: &MultiProvider) -> Vec<ModelRoute> {
    ...
    let mut routes = dedupe_model_routes(apply_provider_allowlist(routes));
...
pub fn remote_model_routes_fallback(
    ...
    for model in remote_available_entries {
        ...
        // The server may downgrade an oversized catalog update to model names
        // only. Antigravity serves several model families that are also claimed
        // by Gemini, Anthropic, or OpenRouter, so model-name heuristics alone
        // can silently attach the wrong runtime. When the remote session is
        // currently on Antigravity, use its catalog's known-model set to retain
        // the provider identity that was lost during frame compaction.
        if remote_provider_name.is_some_and(|name| name.eq_ignore_ascii_case("Antigravity"))
            && super::antigravity::is_known_model(model)
        {
            routes.push(ModelRoute {
                model: model.clone(),
                provider: "Antigravity".to_string(),
                api_method: "https".to_string(),
                available: auth.antigravity == AuthState::Available,
                detail: String::new(),
                usage: None,
                cheapness: None,
            });
...
mod tests {
    ...
    impl EnvGuard {
        fn new() -> Self {
            ...
            let vars = vec![
                ...
                (
                    "JCODE_PROVIDER_ALLOWLIST",
                    std::env::var_os("JCODE_PROVIDER_ALLOWLIST"),
                ),
    ...
    #[test]
    fn provider_allowlist_matches_named_profile_and_rejects_other_routes() {
        let _guard = EnvGuard::new();
        crate::env::set_var("JCODE_PROVIDER_ALLOWLIST", "stables");
        let routes = vec![
            ModelRoute {
                model: "codex/gpt-5.6-terra".to_string(),
                provider: "stables".to_string(),
                api_method: "openai-compatible:stables".to_string(),
                available: true,
                detail: String::new(),
                usage: None,
                cheapness: None,
        ...
        let filtered = apply_provider_allowlist(routes);

        assert_eq!(filtered.len(), 1);
        assert_eq!(filtered[0].provider, "stables");
        assert_eq!(filtered[0].api_method, "openai-compatible:stables");
    }

    #[test]
    fn simplified_picker_honors_named_profile_allowlist_for_slash_models() {
        let _guard = EnvGuard::new();
        let jcode_home = std::env::var_os("JCODE_HOME").expect("JCODE_HOME set");
        std::fs::write(
            std::path::PathBuf::from(jcode_home).join("config.toml"),
            "[providers.stables]\ntype = \"openai-compatible\"\nbase_url = \"https://router.example/v1\"\ndefault_model = \"codex/gpt-5.6-terra\"\n\n[[providers.stables.models]]\nid = \"codex/gpt-5.6-terra\"\ninput = [\"text\"]\n\n[[providers.stables.models]]\nid = \"codex/gpt-5.6-sol\"\ninput = [\"text\"]\n",
        )
        ...
        assert_eq!(routes.len(), 2, "unexpected picker routes: {routes:?}");
        assert!(routes.iter().all(|route| route.provider == "stables"));
        assert!(
            routes
                .iter()
                .all(|route| route.api_method == "openai-compatible:stables")
        );
        assert!(
            routes
                .iter()
                .all(|route| !route.model.starts_with("claude"))
        );
    }
