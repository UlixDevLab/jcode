...
pub use jcode_provider_core::{
    FallbackPickOptions, error_looks_like_capacity_exhaustion, error_looks_like_credential_failure,
    error_looks_like_rate_limited, model_route_provider_labels_match,
...
impl MultiProvider {
    ...
    async fn complete_with_failover(
        ...
        for candidate in sequence {
            ...
            // Operator opted this provider out of automatic failover. The
            // provider the caller actually selected is never skipped: turning
            // off a paid subscription should stop jcode from *drifting* onto
            // it, not refuse a route the operator explicitly asked for.
            if candidate != active && failover::provider_is_disabled_for_failover(candidate) {
                notes.push(format!(
                    "{}: disabled for automatic failover ([provider].disabled_providers)",
                    label
                ));
                continue;
            }
...
impl Provider for MultiProvider {
    ...
    async fn complete_without_failover(
        &self,
        messages: &[Message],
        tools: &[ToolDefinition],
        system: &str,
        ...
        let active = self.active_provider();
        let stream = self
            .complete_on_provider(active, messages, tools, system, resume_session_id)
            .await?;
