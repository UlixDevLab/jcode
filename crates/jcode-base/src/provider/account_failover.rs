...
pub(super) fn same_provider_account_candidates(provider: ActiveProvider) -> Vec<String> {
    ...
    // Accounts bound to a directory by an `[accounts.*]` rule are excluded in
    // both directions, so an exhausted company subscription surfaces as an
    // error rather than silently spending personal quota (or the reverse).
    let binding_prefix = match provider {
        ActiveProvider::OpenAI => Some("openai"),
        _ => None,
    };
    ...
    let mut push_unique = |label: String| {
        ...
        if let Some(prefix) = binding_prefix
            && !crate::auth::account_binding::failover_allowed(
                prefix,
