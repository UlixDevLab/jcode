// sonar-file-size: {"waive":350,"owner":"jcode-maintainers","ticket":"Jcode-control-plane-policy","baseline_loc":1345,"max_loc":1345,"expires":"2026-12-01","removal_trigger":"module reaches 350 LOC","reason":"Policy extraction keeps inherited module at or below mission-base LOC."}
use super::{
...
#[test]
fn test_openai_service_tier_defaults_to_off() {
    assert_eq!(
...
#[path = "config_tests/tool_policy.rs"]
mod tool_policy;
...
#[test]
fn auto_poke_feature_defaults_off_and_parses_true() {
...
fn test_env_override_lifecycle_hooks() {
    ...
    let prev_permission = std::env::var_os("JCODE_HOOK_PERMISSION");
    let prev_timeout = std::env::var_os("JCODE_HOOK_PRE_TOOL_TIMEOUT_MS");

    crate::env::set_var("JCODE_HOOK_TURN_END", "my-notifier --fast");
    crate::env::set_var("JCODE_HOOK_PERMISSION", "permission-notifier --direct");
    ...
    assert_eq!(
        cfg.hooks.permission.as_ref().and_then(HookCommands::first),
        Some("permission-notifier --direct")
    );
    assert_eq!(cfg.hooks.pre_tool_timeout_ms, 250);
    assert!(crate::config::CONFIG_ENV_KEYS.contains(&"JCODE_HOOK_PERMISSION"));
    ...
    restore_env_var("JCODE_HOOK_PERMISSION", prev_permission);
...
fn test_generated_default_config_has_expected_user_defaults() {
    ...
    assert!(
        ...
        "generated default config should explicitly disable the OpenAI priority tier"
    );
    assert!(
