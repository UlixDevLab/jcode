// sonar-file-size: waive=350 reason=pre-existing legacy test module; this mission adds scoped credential-marker regressions and full decomposition is out of scope
use super::*;

#[test]
fn credential_marker_keys_match_only_credential_shapes() {
    for key in [
        "OPENAI_API_KEY",
        "SERVICE_TOKEN",
        "STRIPE_SECRET_KEY",
        ...
        assert!(has_credential_marker_key(key), "{key} should be protected");
    }
    for key in [
        "MONKEY",
        ...
        assert!(!has_credential_marker_key(key), "{key} should be safe");
    }
}
