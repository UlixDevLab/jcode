...
use jcode_tool_core::{NativeConsentRequirement, Tool, ToolContext};
...
impl Tool for McpTool {
    ...
    async fn native_consent_requirement(&self, _input: &Value) -> Option<NativeConsentRequirement> {
        let manager = self.manager.read().await;
        let server_config = manager.config().servers.get(&self.server_name);

        // Narrow policy first: known-safe Firecrawl retrieval tools
        // auto-run even when the server has credential markers (explicit
        // first-party exception for public read-only scraping), and
        // Playwright read-only/navigation tools auto-run when the launch
        // config is provably `--headless` + `--isolated` and carries no
        // credential markers. Everything else stays protected.
        //
        // The model's display-only `intent` field is deliberately not
        // consulted: classification is driven by the recognized server
        // name, the canonical tool allowlist, and the launch config's
        // structural flags. Server-held enforcement in the Registry
        // remains authoritative.
        if let Some(server_config) = server_config {
            let classification = super::policy::classify(
                &self.server_name,
                &self.tool_def.name,
                Some(server_config),
            );
            if classification == super::policy::Classification::AutoSafe {
                return None;
            }
            if classification == super::policy::Classification::Protected
                && (self.server_name.eq_ignore_ascii_case("playwright")
                    || self.server_name.eq_ignore_ascii_case("firecrawl"))
            {
                return Some(NativeConsentRequirement::new(
                    safe_mcp_identifier(&self.server_name),
                    safe_mcp_identifier(&self.tool_def.name),
        ...
        // Playwright input is model-controlled, including the display-only
        // `intent` field. Until jcode has trusted runtime provenance for a
        // loopback-only UI-test capability, every Playwright call must request
        // one-shot native consent rather than accepting the model's claim that
        // an operation is a UI test.
        //
        // Local full-interaction Playwright therefore remains via the
        // project's own Playwright CLI/tests, where the user (or CI) is the
        // actual principal. We do not fake loopback provenance here.
        if self.server_name.eq_ignore_ascii_case("playwright") {
            return Some(NativeConsentRequirement::new(
                "playwright",
                safe_mcp_identifier(&self.tool_def.name),
                "external browser automation",
...
fn safe_mcp_identifier(value: &str) -> String {
...
#[cfg(test)]
#[path = "tool_playwright_tests.rs"]
mod tool_playwright_tests;
