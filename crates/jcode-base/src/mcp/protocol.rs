...
// sonar-file-size: waive=350 reason=pre-existing legacy protocol module; this mission adds only credential classification metadata and full decomposition is out of scope
...
impl McpServerConfig {
    /// Whether this configuration declares credential-bearing headers or
    /// environment variables. Values are deliberately never inspected.
    pub fn has_credential_markers(&self) -> bool {
        self.env
            .keys()
            .chain(self.headers.keys())
            .any(|key| has_credential_marker_key(key))
    }
...
/// Classify a configuration key without examining its associated value.
pub fn has_credential_marker_key(key: &str) -> bool {
    let key = key
        .chars()
        .map(|ch| {
            if ch == '-' {
