...
impl BackgroundTaskManager {
    ...
    /// Cancel every in-process background task owned by one session.
    ///
    /// Explicitly detached tasks are deliberately absent from the in-process
    /// task map. Their persisted status survives client disconnect and server
    /// reload, so this method preserves that explicit opt-out while ensuring a
    /// normal session-owned task cannot outlive loss of session control.
    pub async fn cancel_live_tasks_for_session(&self, session_id: &str) -> Result<usize> {
        let task_ids = self
            .tasks
            .read()
            .await
            .iter()
            .filter_map(|(task_id, task)| (task.session_id == session_id).then(|| task_id.clone()))
            .collect::<Vec<_>>();
