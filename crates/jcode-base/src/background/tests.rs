...
#[tokio::test]
async fn cancel_live_tasks_for_session_stops_only_non_detached_owned_tasks() -> Result<()> {
    let tmp = tempdir()?;
    let manager = BackgroundTaskManager::with_output_dir(tmp.path().to_path_buf());
    ...
    let owned = manager
        .spawn_with_notify(
            "bash",
            Some("session-owned command".to_string()),
            "session-control-loss",
            false,
            false,
            |_output_path| async move {
                sleep(Duration::from_secs(60)).await;
    ...
    let other_session = manager
        .spawn_with_notify(
            "bash",
            Some("other session command".to_string()),
            "session-unrelated",
            false,
            false,
            |_output_path| async move {
    ...
    manager
        .register_detached_task(
            &detached,
            "bash",
            Some("explicitly detached command".to_string()),
            "session-control-loss",
            std::process::id(),
            &Utc::now().to_rfc3339(),
            false,
    ...
    let cancelled = manager
        .cancel_live_tasks_for_session("session-control-loss")
        .await?;
    assert_eq!(cancelled, 1, "only the owned in-process task should stop");

    let owned_status = manager
        .status(&owned.task_id)
        .await
        .ok_or_else(|| anyhow!("owned task status should exist"))?;
    assert_eq!(owned_status.status, BackgroundTaskStatus::Failed);
    assert_eq!(owned_status.error.as_deref(), Some("Cancelled by user"));

    assert!(
        manager.is_live_task(&other_session.task_id),
        "a task owned by another session must continue"
    );
    let detached_status = manager
        .status(&detached.task_id)
        .await
        .ok_or_else(|| anyhow!("detached task status should exist"))?;
    assert_eq!(detached_status.status, BackgroundTaskStatus::Running);
    assert!(detached_status.detached);

    manager.cancel(&other_session.task_id).await?;
    Ok(())
}
