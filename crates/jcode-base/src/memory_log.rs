...
fn log_dir() -> Option<PathBuf> {
    memory_log_dir(std::env::var_os("JCODE_HOME"), dirs::home_dir())
}
...
fn memory_log_dir(
    ...
    override_home
        .map(PathBuf::from)
        .or_else(|| home.map(|h| h.join(".jcode")))
        .map(|h| h.join("logs"))
}

#[cfg(test)]
#[test]
fn memory_log_respects_distribution_home() {
    assert_eq!(
        memory_log_dir(Some("/isolated/lite".into()), Some("/user".into())),
        Some("/isolated/lite/logs".into())
    );
    assert_eq!(
        memory_log_dir(None, Some("/user".into())),
        Some("/user/.jcode/logs".into())
    );
}
