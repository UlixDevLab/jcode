// sonar-file-size: waive=350 reason=pre-existing cross-platform module; this fix adds only the detached-child integration seam
use std::path::Path;

mod detached_child;
pub use detached_child::DetachedChild;
mod reload_children;
pub use reload_children::{ReloadChildReapOutcome, reap_reload_children};
...
pub fn spawn_detached(cmd: &mut std::process::Command) -> std::io::Result<DetachedChild> {
    #[cfg(unix)]
    {
    ...
    detached_child::ensure_reaper_started()?;
    cmd.spawn().map(DetachedChild::new)
}

/// Spawn a fire-and-forget child that remains in the current process session.
///
/// Dropping the returned handle submits it to the shared reaper during normal
/// runtime. Unlike `spawn_detached`, reload-boundary cleanup may terminate and
/// reap it before `exec`, so short observer processes cannot outlive the reaper
/// thread and become zombies in the replacement server image.
pub fn spawn_reaped(cmd: &mut std::process::Command) -> std::io::Result<DetachedChild> {
    detached_child::ensure_reaper_started()?;
    cmd.spawn().map(DetachedChild::new)
}
