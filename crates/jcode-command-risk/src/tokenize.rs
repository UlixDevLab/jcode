...
/// Extract `$(...)` and backtick substitutions from an unquoted here-doc body.
///
/// These are the only parts a shell will execute, so they are the only parts
/// that must survive into risk assessment.
fn command_substitutions(body: &str) -> Vec<String> {
