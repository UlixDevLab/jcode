...
/// Wrappers that are ordinary informational commands when invoked with no
/// program at all.
///
/// `env` prints the environment, `time`/`nice`/`ionice` report or query
/// scheduling state, and `command`/`builtin` are shell no-ops. Because they
/// wrap nothing in that form, there is no hidden payload to be cautious about,
/// and treating them as unknowable blocked routine inspection like
/// `env | grep -i KEY`.
///
/// Privilege changers (`sudo`, `doas`, `su`, `chroot`) are deliberately absent:
/// even bare, they are worth a confirmation rather than a silent pass.
const BARE_WRAPPER_IS_INFORMATIONAL: &[&str] = &[
    "env", "time", "nice", "ionice", "command", "builtin", "stdbuf",
];
...
fn assess_segment(tokens: &[Token], ctx: &RiskContext, findings: &mut Vec<RiskFinding>) {
    ...
    // Number of words that followed the wrapper before we ran out. A wrapper
    // with nothing after it is not hiding a payload.
    let mut consumed_after_wrapper = 0usize;
    loop {
        let Some(first) = tokens.first() else {
            // Ran off the end while unwrapping, so the payload is invisible and
            // we must not assume safety.
            //
            // The one exception is a wrapper that consumed nothing and is a
            // normal informational command in that form (`env` on its own
            // prints the environment). There is no hidden payload there, and
            // escalating it blocked routine inspection such as
            // `env | grep -i KEY`. A false block is not free: the sanctioned
            // way past one is to re-issue with a justification, which is
            // exactly the habit this gate exists to discourage.
            if let Some(wrapper) = wrapped_by.filter(|name| {
                consumed_after_wrapper > 0
                    || !BARE_WRAPPER_IS_INFORMATIONAL.contains(&name.as_str())
            }) {
                findings.push(RiskFinding {
                    level: RiskLevel::Confirm,
                    reason: format!(
        ...
        // Anything after the wrapper counts as an operand we were meant to
        // resolve. `env` alone consumes nothing, so it stays quiet, while
        // `env -u PATH` (which did have arguments) still escalates.
        consumed_after_wrapper += tokens.len() - 1;
