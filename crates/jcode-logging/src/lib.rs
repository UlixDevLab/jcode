...
mod changed;
pub mod watchdog;
pub use changed::{info_if_changed, warn_if_changed};
use chrono::Local;
...
pub struct Logger {
    file: File,
    /// Local date (`%Y-%m-%d`) the open handle belongs to.
    ///
    /// Sessions routinely outlive a calendar day (the shared server runs for
    /// days), and the handle is opened once at startup. Without re-checking the
    /// date, every long-lived process keeps appending to the file named after
    /// the day it *started*, so today's log is split across several stale
    /// filenames and the 7-day pruner keeps refreshing their mtime instead of
    /// retiring them.
    date: String,
}
...
impl Logger {
    ...
    /// Repoint the handle at today's file when the local date has rolled over.
    ///
    /// Best-effort: if the new file cannot be opened we keep writing to the
    /// previous one, because losing log lines is worse than a stale filename.
    fn rotate_if_needed(&mut self) {
        let Some(log_dir) = log_dir() else { return };
        self.rotate_in(&log_dir, Local::now());
    }

    /// Core of [`Self::rotate_if_needed`], parameterized on the directory and
    /// "now" so it can be unit-tested without touching the real log directory
    /// or mutating process-global env.
    fn rotate_in(&mut self, log_dir: &std::path::Path, now: chrono::DateTime<Local>) {
        let today = now.format("%Y-%m-%d").to_string();
        if today == self.date {
            return;
        }
        let path = log_dir.join(format!("jcode-{}.log", today));
...
mod tests {
    ...
    /// A process that outlives midnight must start writing to the new day's
    /// file. Before this, the handle opened at startup was kept forever, so a
    /// long-lived shared server appended days of output to the filename of the
    /// day it launched.
    #[test]
    fn logger_rotates_to_todays_file_after_date_rollover() {
        use std::time::SystemTime;

        let dir = std::env::temp_dir().join(format!(
            "jcode-log-rotate-test-{}-{}",
            std::process::id(),
            SystemTime::now()
        ...
        // Simulate a logger opened yesterday and still running now.
        let stale_path = dir.join(format!("jcode-{}.log", yesterday));
        let file = OpenOptions::new()
            .create(true)
            .append(true)
            .open(&stale_path)
        ...
        logger.rotate_in(&dir, Local::now());
        logger.write("INFO", "after midnight");

        assert_eq!(
            logger.date, today,
            "logger should adopt today's date after rollover"
        );
        let fresh_path = dir.join(format!("jcode-{}.log", today));
        assert!(
            fresh_path.exists(),
            "today's log file should be created after rollover"
        );
        assert!(
            fs::read_to_string(&fresh_path)
                .unwrap_or_default()
                .contains("after midnight"),
            "post-midnight lines belong in today's file"
        );
        assert!(
            !fs::read_to_string(&stale_path)
                .unwrap_or_default()
                .contains("after midnight"),
            "post-midnight lines must not land in yesterday's file"
        );

        // Same-day writes must not churn the handle.
        logger.rotate_in(&dir, Local::now());
        assert_eq!(logger.date, today, "same-day rotate must be a no-op");
