// sonar-file-size: {"waive":350,"owner":"jcode-maintainers","ticket":"Jcode-control-plane-policy","baseline_loc":2000,"max_loc":2000,"expires":"2026-12-01","removal_trigger":"module reaches 350 LOC","reason":"Policy extraction keeps inherited module at or below mission-base LOC."}
use super::*;
...
#[tokio::test]
async fn current_brief_reaches_root_and_worker_provider_context_and_survives_resume() {
    let _guard = crate::storage::lock_test_env();
    ...
    let provider: Arc<dyn Provider> = Arc::new(NativeAutoCompactionProvider);
    let registry = Registry::new(provider.clone()).await;
    let mut root = Agent::new(provider.clone(), registry.clone());
    let brief = crate::session::CurrentMissionBrief {
        outcome: "Deliver the agreed focused change.".to_string(),
        in_scope: vec!["provider context".to_string()],
        out_of_scope: vec!["deployment".to_string()],
        constraints: vec!["preserve native consent".to_string()],
        acceptance_observations: vec!["the next request sees this brief".to_string()],
        autonomous_decisions: vec!["ordinary local implementation".to_string()],
        unresolved_user_decisions: vec!["material scope expansion".to_string()],
    };
    let metadata = serde_json::json!({"current_mission_brief": brief});
    root.register_current_mission_brief_from_tool_metadata("mission_brief", Some(&metadata))
        .expect("root should register current brief");
    let session_id = root.session_id().to_string();
    assert_eq!(
        crate::session::Session::load(&session_id)
            .expect("persisted root session")
    ...
    let root_prompt = root.build_system_prompt_split(None).dynamic_part;
    assert_eq!(root_prompt.matches("# Current Mission Brief").count(), 1);
    assert!(root_prompt.contains("provider context"));

    let worker_session = crate::session::Session::create_with_id(
        "current-brief-worker".to_string(),
        Some(session_id.clone()),
        None,
    );
    let mut worker =
        Agent::new_with_session(provider.clone(), registry.clone(), worker_session, None);
    let worker_prompt = worker.build_system_prompt_split(None).dynamic_part;
    assert_eq!(worker_prompt.matches("# Current Mission Brief").count(), 1);
    assert!(worker_prompt.contains("A worker cannot replace it."));

    let mut resumed = Agent::new(provider, registry);
    resumed.restore_session(&session_id).expect("resume root");
    let resumed_prompt = resumed.build_system_prompt_split(None).dynamic_part;
    assert_eq!(resumed_prompt.matches("# Current Mission Brief").count(), 1);
    assert!(resumed_prompt.contains("Deliver the agreed focused change."));
...
#[tokio::test]
async fn current_brief_registration_rejects_invalid_metadata_and_worker_mutation() {
    let provider: Arc<dyn Provider> = Arc::new(NativeAutoCompactionProvider);
    let registry = Registry::new(provider.clone()).await;
    let mut root = Agent::new(provider.clone(), registry.clone());
    let invalid_metadata = serde_json::json!({
        "current_mission_brief": {"outcome": "\u{0001}"}
    });
    assert!(
        root.register_current_mission_brief_from_tool_metadata(
            "mission_brief",
            Some(&invalid_metadata)
        )
        .is_err()
    );
    assert!(root.session.current_mission_brief.is_none());

    let worker_session = crate::session::Session::create_with_id(
        "worker-current-brief-test".to_string(),
        Some(root.session_id().to_string()),
        None,
    );
    let mut worker = Agent::new_with_session(provider, registry, worker_session, None);
    let brief = crate::session::CurrentMissionBrief {
        outcome: "A valid brief.".to_string(),
        in_scope: vec![],
        out_of_scope: vec![],
        constraints: vec![],
        acceptance_observations: vec![],
        autonomous_decisions: vec![],
        unresolved_user_decisions: vec![],
    };
    assert!(worker.register_current_mission_brief(brief).is_err());
    assert!(worker.session.current_mission_brief.is_none());
}

#[tokio::test]
async fn readable_compaction_without_current_brief_is_byte_identical() {
    let provider: Arc<dyn Provider> = Arc::new(NativeAutoCompactionProvider);
    let registry = Registry::new(provider.clone()).await;
    let mut agent = Agent::new(provider, registry);
    agent.session.compaction = Some(crate::session::StoredCompactionState {
        summary_text: "already compacted conversation".to_string(),
    ...
    assert_eq!(
        ...
        "## Previous Conversation Summary\n\nalready compacted conversation\n\n---\n\n"
    );
}

#[tokio::test]
async fn encrypted_compaction_keeps_current_brief_out_of_history_messages() {
    let provider: Arc<dyn Provider> = Arc::new(NativeAutoCompactionProvider);
    let registry = Registry::new(provider.clone()).await;
    let mut agent = Agent::new(provider, registry);
    agent.session.compaction = Some(crate::session::StoredCompactionState {
        summary_text: String::new(),
        openai_encrypted_content: Some("enc_opaque".to_string()),
        covers_up_to_turn: 0,
        original_turn_count: 0,
        compacted_count: 0,
    });
    ...
    assert!(matches!(
        ...
        ContentBlock::OpenAICompaction { ref encrypted_content } if encrypted_content == "enc_opaque"
    ));
}
...
#[tokio::test]
async fn skill_allowed_tools_constrain_model_visible_builtin_schemas() {
    let _guard = crate::storage::lock_test_env();
    let previous_home = std::env::var_os("JCODE_HOME");
    let temp_home = tempfile::TempDir::new().expect("temp home");
    let skill_dir = temp_home.path().join("skills/narrow-tool-surface");
    std::fs::create_dir_all(&skill_dir).expect("create skill directory");
    std::fs::write(
        skill_dir.join("SKILL.md"),
        "---\nname: narrow-tool-surface\ndescription: scoped test skill\nallowed-tools: read, bash\n---\n\nUse only the declared tools.",
    )
    .expect("write skill");
    ...
    let registry = Registry::new(provider.clone()).await;
    let mut agent = Agent::new(provider, registry);
    agent.set_active_skill("narrow-tool-surface".to_string());
    ...
    assert_eq!(
        names,
        HashSet::from(["bash".to_string(), "read".to_string()]),
        "a skill's existing allowed-tools declaration must remove unrelated built-in schemas"
    );
    agent
        .validate_tool_allowed("read")
        .expect("declared tool remains executable");
    assert!(
        agent.validate_tool_allowed("gmail").is_err(),
        "hidden schemas must not remain directly executable through this session"
    );
...
#[path = "agent_tests/tool_policy.rs"]
mod tool_policy;

#[tokio::test]
async fn canary_skill_surface_retains_selfdev_control_tool() {
    let temp_project = tempfile::TempDir::new().expect("temp project");
    let skill_dir = temp_project.path().join(".jcode/skills/scoped-canary");
    std::fs::create_dir_all(&skill_dir).expect("create skill directory");
    std::fs::write(
        skill_dir.join("SKILL.md"),
        "---\nname: scoped-canary\ndescription: scoped canary test skill\nallowed-tools: read\n---\n\nUse the declared tool.",
    )
    .expect("write skill");
    ...
    session.working_dir = temp_project.path().to_str().map(str::to_string);
    let mut agent = Agent::new_with_session(provider, registry, session, None);
    agent.set_active_skill("scoped-canary".to_string());
    ...
    assert!(
        names.contains("selfdev"),
        "canary sessions must retain selfdev when an active skill narrows built-ins"
    );
    assert!(!names.contains("gmail"));
}

#[tokio::test]
async fn fable_specialist_tool_is_visible_only_inside_the_explicit_fable_skill() {
    let _guard = crate::storage::lock_test_env();
    let previous_home = std::env::var_os("JCODE_HOME");
    let temp_home = tempfile::TempDir::new().expect("temp home");
    let skill_dir = temp_home.path().join("skills/fable");
    std::fs::create_dir_all(&skill_dir).expect("create skill directory");
    std::fs::write(
        skill_dir.join("SKILL.md"),
        "---\nname: fable\ndescription: explicit isolated specialist\nallowed-tools: fable, read\n---\n\nCall Fable only for this explicit invocation.",
    )
    .expect("write skill");
    crate::env::set_var("JCODE_HOME", temp_home.path());
    crate::config::Config::invalidate_cache();
    ...
    let ordinary_names: HashSet<String> = agent
        .tool_definitions()
        .await
        .into_iter()
        .map(|tool| tool.name)
        .collect();
    assert!(
        !ordinary_names.contains("fable"),
        "the expensive specialist must not be visible during ordinary turns"
    );
    assert!(
        agent.validate_tool_allowed("fable").is_err(),
        "ordinary turns must not be able to execute a guessed Fable tool call"
    );

    agent.set_active_skill("fable".to_string());
    let fable_names: HashSet<String> = agent
        .tool_definitions()
        .await
        .into_iter()
        .map(|tool| tool.name)
    ...
    assert_eq!(
        fable_names,
        HashSet::from(["fable".to_string(), "read".to_string()]),
        "an explicit /fable invocation should expose only its bounded context tool and isolated specialist"
    );
    agent
        .validate_tool_allowed("fable")
        .expect("the explicit Fable skill authorizes its isolated tool");
