// sonar-file-size: waive=350 reason=pre-existing legacy notification module; this mission closes one fire-and-forget child leak and full decomposition is out of scope
...
mod tests {
    ...
    #[cfg(unix)]
    #[test]
    fn notification_child_is_reaped_after_exit() {
        let mut command = std::process::Command::new("/bin/sh");
        command.arg("-c").arg("exit 0");
        let child = command.spawn().expect("spawn notification fixture");
        ...
        loop {
            ...
            assert!(
                ...
                "notification child {pid} was not reaped before the deadline"
            );
