...
fn prune_old_build_versions(context: &str) {
    match build::prune_unreferenced_versions() {
        Ok(report) => {
            if !report.removed.is_empty() {
                crate::logging::info(&format!(
                    "{context}: pruned unused build versions: {}",
                    report.removed.join(", ")
                ));
            }
            if !report.failures.is_empty() {
                crate::logging::warn(&format!(
                    "{context}: could not prune some unused build versions: {:?}",
                    report.failures
                ));
            }
        }
        Err(error) => crate::logging::warn(&format!(
            "{context}: build version pruning was skipped: {error}"
        )),
    }
}
...
fn install_main_source_update_blocking(latest_sha: &str) -> Result<PathBuf> {
    ...
    metadata.save()?;
    prune_old_build_versions("main source update");
...
pub fn download_and_install_blocking_with_progress(
    ...
    metadata.last_check = SystemTime::now();
    metadata.save()?;
    prune_old_build_versions("release update");
