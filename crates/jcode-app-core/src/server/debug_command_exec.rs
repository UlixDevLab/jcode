...
pub(super) async fn execute_debug_command(
    ...
    if trimmed == "reload" {
        ...
        manifest.save()?;
        match crate::build::prune_unreferenced_versions() {
            Ok(report) if !report.failures.is_empty() => crate::logging::warn(&format!(
                "debug reload could not prune some unused versions: {:?}",
                report.failures
            )),
            Err(error) => crate::logging::warn(&format!(
                "debug reload version pruning was skipped: {error}"
            )),
            _ => {}
        }
