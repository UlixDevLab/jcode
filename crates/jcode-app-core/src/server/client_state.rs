// sonar-file-size: {"waive":350,"owner":"@jcode-maintainers","ticket":"JCODE-QUALITY-RATCHET","baseline_loc":928,"max_loc":928,"expires":"2026-11-30","removal_trigger":"Continue extracting History response responsibilities","reason":"Inherited oversized file changed for the bounded session activity protocol"}
use super::ClientConnectionInfo;
use super::client_writer::write_encoded;
...
pub(super) use super::client_activity::{
    session_activity_context_snapshot, session_activity_snapshot,
};
...
pub(super) async fn handle_get_model_catalog(
    ...
    let delivery_ms = write_started.elapsed().as_millis();
    crate::logging::info(&format!(
        "[TIMING] handle_get_model_catalog: session={}, source={}, bytes={}, build={}ms, encode={}ms, delivery={}ms, total={}ms",
        session_id,
        source,
        json.len(),
        build_ms,
...
async fn send_history_with_guard(
    ...
    let (
        ...
        (
            ...
            session_activity_context_snapshot(
                ...
                agent_guard.session_started_at(),
                activity.as_ref(),
            ),
            history_snapshot_ms,
            image_render_ms,
    ...
    let result = write_encoded(writer, json.as_bytes()).await;
    // Release the serialized payload before any further work (logging below
    // only needs the captured length).
    drop(json);
    let delivery_ms = delivery_start.elapsed().as_millis();

    crate::logging::info(&format!(
        "[TIMING] send_history write: session={}, bytes={}, encode={}ms, delivery={}ms, total={}ms",
        session_id,
        json_len,
        encode_ms,
        delivery_ms,
...
async fn write_event(writer: &Arc<Mutex<WriteHalf>>, event: &ServerEvent) -> Result<()> {
    ...
    buf.push(b'\n');
    write_encoded(writer, &buf).await?;
    drop(buf);
    Ok(())
}
...
#[cfg(test)]
#[path = "client_activity_context_tests.rs"]
mod client_activity_context_tests;
