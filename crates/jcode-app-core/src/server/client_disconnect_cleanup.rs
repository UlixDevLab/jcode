...
pub(super) async fn cleanup_client_connection(
    ...
    // Stop and join the connection-owned turn before sweeping background tasks.
    // Otherwise a just-disconnecting turn can promote a tool after the sweep,
    // leaving a normal session-owned command running without an owner.
    if let Some(handle) = processing_task.take() {
        handle.abort();
        let _ = handle.await;
    }
    ...
    match crate::background::global()
        .cancel_live_tasks_for_session(client_session_id)
        .await
    {
        Ok(cancelled) if cancelled > 0 => crate::logging::info(&format!(
            "Cancelled {} non-detached background task(s) for disconnected session {}",
            cancelled, client_session_id
        )),
        Ok(_) => {}
        Err(error) => crate::logging::error(&format!(
            "Failed to cancel background tasks for disconnected session {}: {}",
...

    {
        ...
        crate::tool::read_ledger::forget_session(client_session_id);
...
mod tests {
    ...
    #[tokio::test]
    async fn final_disconnect_cancels_live_background_tasks_owned_by_the_session() {
        let session_id = "session_disconnect_background_task";
        let task = crate::background::global()
            .spawn_with_notify(
                "bash",
                Some("must stop when client exits".to_string()),
                session_id,
                false,
                false,
        ...
        let event_handle = tokio::spawn(std::future::pending::<()>());
