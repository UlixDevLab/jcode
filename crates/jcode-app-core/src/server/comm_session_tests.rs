// sonar-file-size: {"waive":350,"owner":"jcode-maintainers","ticket":"Jcode-control-plane-policy","baseline_loc":1268,"max_loc":1268,"expires":"2026-12-01","removal_trigger":"module reaches 350 LOC","reason":"Policy extraction keeps inherited module at or below mission-base LOC."}
...
use super::{
    ...
    resolve_stop_target_session, resolve_swarm_spawn_selection, select_pool_spawn_model,
    spawn_admission_lock, swarm_stop_allowed_by_owner, try_resolve_swarm_spawn_selection,
};
...
async fn resolve_spawn_working_dir_prefers_explicit_then_spawner_agent_dir() {
    // These fixtures are real directories on purpose. They used to be
    // synthetic strings ("/tmp/explicit") that never existed, which passed only
    // because nothing checked whether a worker could run in them. That is
    // exactly the gap this resolver now closes, so keeping fictional paths here
    // would mean the test asserts behavior the product no longer has.
    let explicit_dir = tempfile::TempDir::new().expect("tempdir");
    let agent_dir = tempfile::TempDir::new().expect("tempdir");
    let explicit = explicit_dir.path().display().to_string();
    let agent = agent_dir.path().display().to_string();

    let sessions = Arc::new(RwLock::new(HashMap::new()));
    sessions.write().await.insert(
        "req".to_string(),
        test_agent_with_working_dir("req", &agent).await,
    );
    let swarm_members = Arc::new(RwLock::new(HashMap::new()));

    assert_eq!(
        resolve_spawn_working_dir(Some(explicit.clone()), "req", &sessions, &swarm_members)
            .await
            .as_deref(),
...
fn prepare_visible_spawn_session_persists_startup_before_launch() {
    ...
    assert_eq!(
        session.tool_policy_mode,
        crate::config::SessionToolPolicyMode::Normal,
        "visible swarm workers must remain normal even when root control-plane mode is configured"
    );
    let path = crate::storage::jcode_dir()
        .expect("jcode dir")
        .join(format!("client-input-{}", session_id));
...
#[test]
fn model_pool_alternates_models_and_skips_blank_entries() {
    let pool = vec![
    ...
    assert_eq!(
        select_pool_spawn_model(&pool, 0).unwrap(),
        "minimax:MiniMax-M3"
    );
    assert_eq!(select_pool_spawn_model(&pool, 1).unwrap(), "gpt-5.6-luna");
    assert_eq!(
        select_pool_spawn_model(&pool, 2).unwrap(),
        "minimax:MiniMax-M3"
    );
}

#[test]
fn model_pool_sentinel_errors_when_pool_has_no_model() {
    let error = try_resolve_swarm_spawn_selection(
        Some("rotate".to_string()),
        None,
        &coordinator_identity(Some("gpt-5.6-sol"), Some("openai"), None),
        &[" ".to_string()],
    )
    .expect_err("pool sentinel must reject an empty configured pool");

    assert!(error.to_string().contains("swarm_model_pool is empty"));
}

#[test]
fn concrete_spawn_model_does_not_consume_or_depend_on_model_pool() {
    ...
    .expect("concrete requested model must not inspect the configured pool");

    assert_eq!(selection.model.as_deref(), Some("gpt-5.6-luna"));
}
...
#[tokio::test]
async fn resolve_spawn_working_dir_rejects_a_path_that_does_not_exist() {
    // A nonexistent working directory currently survives resolution and fails
    // much later, when Bash applies `current_dir` and the process spawn itself
    // errors. By then the worker exists, has been messaged, and the failure
    // reads as a broken tool rather than a bad dispatch.
    //
    // This is the common operator case: jcode is started in directory A and
    // asked to work on repository B, so a hand-written or stale path is easy
    // to pass. Rejecting it here names the real problem at the only moment the
    // caller can still fix it.
    let sessions = Arc::new(RwLock::new(HashMap::new()));
    sessions.write().await.insert(
        "req".to_string(),
        test_agent_with_working_dir("req", "/tmp").await,
    );
    let swarm_members = Arc::new(RwLock::new(HashMap::new()));

    let resolved = resolve_spawn_working_dir(
        Some("/tmp/jcode-definitely-not-a-real-directory-92f1a".to_string()),
        "req",
        &sessions,
        &swarm_members,
    )
    ...
    // Falls through to the coordinator's directory rather than resolving to
    // nothing: somewhere recoverable beats nowhere.
    assert_eq!(
        resolved.as_deref(),
        Some("/tmp"),
        "a nonexistent explicit working_dir must fall through, not resolve"
    );
}

#[tokio::test]
async fn resolve_spawn_working_dir_rejects_a_path_that_is_a_file() {
    // A file is not a working directory. Left unchecked it reaches the same
    // late `current_dir` failure as a missing path.
    ...
    // No coordinator dir is registered here, so rejection leaves nothing to
    // fall back to and the result is None.
    assert_eq!(resolved, None, "a file must not resolve as a working dir");
}

#[tokio::test]
async fn resolve_spawn_working_dir_still_accepts_a_real_directory() {
    // The guard must not reject the ordinary case. Without this, a validation
    // bug that rejects everything would look like a passing test suite.
    let temp = tempfile::TempDir::new().expect("tempdir");
    let sessions = Arc::new(RwLock::new(HashMap::new()));
    let swarm_members = Arc::new(RwLock::new(HashMap::new()));
    ...
    assert_eq!(
        resolved.as_deref(),
        Some(temp.path().display().to_string()).as_deref()
    );
}
