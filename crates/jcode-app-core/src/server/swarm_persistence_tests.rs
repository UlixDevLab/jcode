...
#[test]
fn member_routing_facts_survive_persistence() {
    // Which model a worker actually ran is the fact that makes a swarm
    // auditable. It was observed live and shown in the gallery, but dropped on
    // persist and reset to default on load, so every finished member read as
    // "inherited". That silently hides the expensive routing failure: an
    // open-ended task sent to a model reserved for mechanical work.
    let dir = tempfile::TempDir::new().expect("tempdir");
    let _env = test_env(&dir);

    let (event_tx, _event_rx) = tokio::sync::mpsc::unbounded_channel();
    let members = vec![SwarmMember {
        session_id: "session-routed".to_string(),
        event_tx,
        event_txs: HashMap::new(),
        working_dir: Some(PathBuf::from("/tmp/swarm-routing")),
        swarm_id: Some("swarm-routing".to_string()),
        swarm_enabled: true,
        status: "running".to_string(),
        detail: None,
        friendly_name: Some("otter".to_string()),
        ...
        task_label: Some("forge".to_string()),
    }];

    persist_swarm_state("swarm-routing", None, None, &members);
    let loaded = load_runtime_state();

    let member = loaded
        .members
        .get("session-routed")
        .expect("member survives reload");

    assert_eq!(member.runtime.model.as_deref(), Some("gpt-5.6-terra"));
    assert_eq!(member.runtime.provider.as_deref(), Some("openai"));
    assert_eq!(member.runtime.effort.as_deref(), Some("high"));
    assert_eq!(member.task_label.as_deref(), Some("forge"));

    // Timing is deliberately not restored. `elapsed_secs` measures this
    // process's view of a live worker, so replaying it after a reload would
    // report a stale duration as if it were current. Which model ran is a
    // historical fact; how long ago it started is not.
    assert_eq!(member.runtime.elapsed_secs, None);
}

#[test]
fn unobserved_routing_is_absent_rather_than_guessed() {
    // A member whose worker never reported a turn must round-trip as "not
    // observed", not as some inherited default. Recording a guess here would
    // make the audit trail assert a routing claim that never happened.
    let dir = tempfile::TempDir::new().expect("tempdir");
    let _env = test_env(&dir);

    let (event_tx, _event_rx) = tokio::sync::mpsc::unbounded_channel();
    let members = vec![SwarmMember {
        session_id: "session-unobserved".to_string(),
        event_tx,
        event_txs: HashMap::new(),
        working_dir: None,
        swarm_id: Some("swarm-unobserved".to_string()),
        swarm_enabled: true,
        status: "ready".to_string(),
        detail: None,
        friendly_name: Some("moth".to_string()),
        report_back_to_session_id: None,
        latest_completion_report: None,
        role: "agent".to_string(),
        joined_at: Instant::now(),
    ...
    persist_swarm_state("swarm-unobserved", None, None, &members);
    let loaded = load_runtime_state();

    let member = loaded
        .members
        .get("session-unobserved")
        .expect("member survives reload");

    assert_eq!(member.runtime.model, None);
    assert_eq!(member.runtime.provider, None);
    assert_eq!(member.runtime.effort, None);
}

#[test]
fn plan_items_keep_their_parallelism_fields_across_a_disk_round_trip() {
    // file_scope and blocked_by are the only machine-checkable answer to
    // "can these two items run at the same time": disjoint scopes mean they
    // will not fight over a file, and an empty blocked_by means ready now.
    //
    // Every existing persistence test builds them empty, so the populated case
    // has never crossed the disk boundary. That is exactly the case that can
    // break silently here, because both fields carry
    // `skip_serializing_if = "Vec::is_empty"`: the empty value is omitted from
    // the file entirely and restored by `default`, so an empty round trip
    // passes without ever exercising the serialize-and-read-back path the
    // populated value takes.
    //
    // A plan that loses its scopes on reload is worse than one that never had
    // them: the items still look coordinated while the data that kept two
    // writers off one file is gone.
    let dir = tempfile::TempDir::new().expect("tempdir");
    let _env = test_env(&dir);

    let mut plans = HashMap::new();
    plans.insert(
        "swarm-parallel".to_string(),
        VersionedPlan {
            items: vec![
                crate::plan::PlanItem {
                    content: "extract the parser".to_string(),
                    status: "pending".to_string(),
                    priority: "high".to_string(),
                    id: "task-a".to_string(),
                    subsystem: Some("parsing".to_string()),
                    file_scope: vec!["src/parse.rs".to_string(), "src/lex.rs".to_string()],
                    blocked_by: Vec::new(),
                    assigned_to: None,
                },
                crate::plan::PlanItem {
                    ...
                    priority: "medium".to_string(),
                    id: "task-b".to_string(),
                    subsystem: Some("render".to_string()),
                    file_scope: vec!["src/render.rs".to_string()],
                    blocked_by: vec!["task-a".to_string()],
                    assigned_to: None,
                },
            ],
            version: 1,
    ...
    persist_swarm_state("swarm-parallel", plans.get("swarm-parallel"), None, &[]);
    let loaded = load_runtime_state();

    let plan = loaded.plans.get("swarm-parallel").expect("loaded plan");
    let a = plan
        .items
        .iter()
        .find(|item| item.id == "task-a")
        .expect("task-a survives reload");
    let b = plan
        .items
        .iter()
        .find(|item| item.id == "task-b")
        .expect("task-b survives reload");

    assert_eq!(a.file_scope, vec!["src/parse.rs", "src/lex.rs"]);
    assert_eq!(a.subsystem.as_deref(), Some("parsing"));
    assert!(
        a.blocked_by.is_empty(),
        "task-a was ready and must stay ready"
    );

    assert_eq!(b.file_scope, vec!["src/render.rs"]);
    assert_eq!(b.blocked_by, vec!["task-a"]);

    // The property the fields exist to express: these two items claim
    // different files, so a reader can still tell they may run in parallel.
    let overlap = a.file_scope.iter().any(|path| b.file_scope.contains(path));
    assert!(
        !overlap,
        "disjoint file scopes must survive reload, or parallelism cannot be judged"
    );
}
