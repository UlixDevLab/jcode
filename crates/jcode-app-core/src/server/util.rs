...
/// Resolve only the already-running executable for stalled-path recovery.
/// Unlike `reload_exec_target`, this never inspects a channel candidate and
/// therefore cannot activate a newly-published or otherwise unaccepted build.
/// A deleted, missing, or unresolvable executable declines recovery rather than
/// falling back to a channel target.
pub(crate) fn reload_current_executable_target() -> Option<(PathBuf, &'static str)> {
    let current = std::env::current_exe().ok().map(strip_deleted_suffix)?;
    let payload = build::resolve_binary_payload(&current);
    if !current.exists() || !payload.exists() {
        crate::logging::warn(&format!(
            "stalled recovery declined: current executable {:?} (payload {:?}) is not an existing file",
            current, payload
        ));
        return None;
    }
    Some((current, "current executable"))
}
