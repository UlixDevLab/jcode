// sonar-file-size: waive=350 reason=pre-existing reload coordinator; this fix adds only the reload-boundary child reaping seam and decomposition is out of scope
...
pub(super) async fn await_reload_signal(
    ...
    loop {
        ...
        // Unix `exec` preserves the process tree but discards every Rust Child
        // handle and the detached-child reaper thread. Drain normal direct
        // children at this special lifecycle boundary so completed hooks/MCP
        // processes cannot accumulate as zombies in the replacement image.
        // `spawn_detached` jobs are setsid session leaders and remain running.
        let reload_children =
            crate::platform::reap_reload_children(std::time::Duration::from_secs(2));
        super::reload_trace::record_value(
            &signal.request_id,
            "reload_children_reaped",
            serde_json::json!({
                "discovered": reload_children.discovered,
                "skipped_detached": reload_children.skipped_detached,
                "signalled": reload_children.signalled,
                "reaped": reload_children.reaped,
                "remaining": reload_children.remaining,
            }),
        );
        if reload_children.remaining > 0 {
            crate::logging::warn(&format!(
                "Server: {} direct child process(es) remained at reload deadline",
                reload_children.remaining
            ));
        }

        let reload_target = if signal.current_executable_only {
            super::reload_current_executable_target()
        } else {
            super::reload_exec_target(signal.prefer_selfdev_binary)
        };

        if let Some((binary, label)) = reload_target {
            if binary.exists() {
