...
use crate::server::comm_graph::{
    GraphSeedRuntime, handle_comm_complete_node, handle_comm_expand_node, handle_comm_seed_graph,
};
...
impl GraphFixture {
    async fn seed(&mut self, mode: &str, nodes: Vec<TaskGraphNodeSpec>) {
        handle_comm_seed_graph(
            ...
            &self.event_history,
            &self.event_counter,
            GraphSeedRuntime::with_sessions(&self.swarm_event_tx, &self.sessions),
        )
        .await;
    }
...
async fn e2e_identical_seed_replay_succeeds_without_version_or_node_churn() {
    ...
    assert!(events.iter().all(|event| !matches!(event, ServerEvent::Error { .. })),
...
async fn e2e_solo_seeder_is_elected_coordinator_and_can_assign() {
    ...
    handle_comm_seed_graph(
        ...
        &event_history,
        &event_counter,
        GraphSeedRuntime::with_sessions(&swarm_event_tx, &sessions),
    )
    .await;
...
async fn e2e_seed_does_not_displace_live_coordinator() {
    ...
    handle_comm_seed_graph(
        ...
        &event_history,
        &event_counter,
        GraphSeedRuntime::without_scout(&swarm_event_tx),
    )
    .await;
