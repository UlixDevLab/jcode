// sonar-file-size: {"waive":350,"owner":"jcode-maintainers","ticket":"Jcode-control-plane-policy","baseline_loc":1581,"max_loc":1581,"expires":"2026-12-01","removal_trigger":"module reaches 350 LOC","reason":"Policy extraction keeps inherited module at or below mission-base LOC."}
...
use crate::protocol::{
    MissionDispatchBinding, NotificationType, ServerEvent, UntrustedDispatchIntent,
};
...
#[path = "context_scout.rs"]
mod context_scout;
#[path = "spawn_admission_context.rs"]
mod spawn_admission_context;
use spawn_admission_context::resolve_spawn_working_dir;
type SessionAgents = Arc<RwLock<HashMap<String, Arc<Mutex<Agent>>>>>;
type ChannelSubscriptions = Arc<RwLock<HashMap<String, HashMap<String, HashSet<String>>>>>;
type ClientConnections = Arc<RwLock<HashMap<String, ClientConnectionInfo>>>;

pub(super) struct SpawnMetadata {
    pub(super) mission_binding: Option<MissionDispatchBinding>,
    pub(super) dispatch_intent: Option<UntrustedDispatchIntent>,
}

impl SpawnMetadata {
    pub(super) fn new(
        mission_binding: Option<MissionDispatchBinding>,
        dispatch_intent: Option<UntrustedDispatchIntent>,
    ) -> Self {
        Self {
            mission_binding,
            dispatch_intent,
...
/// True when a model string is the explicit "draw from the configured pool"
/// sentinel. Either `pool` or `rotate` triggers round-robin selection against
/// `agents.swarm_model_pool`.
fn is_pool_sentinel(model: &str) -> bool {
    let trimmed = model.trim();
    trimmed.eq_ignore_ascii_case("pool") || trimmed.eq_ignore_ascii_case("rotate")
}
/// Process-global atomic counter for the model pool round-robin. Each call to
/// `next_pool_spawn_model` advances this by one and resolves the pool against
/// the returned ordinal. Tests can drive the helper directly with a known
/// ordinal for determinism; the counter is the production dispatcher.
static SWARM_MODEL_POOL_COUNTER: std::sync::atomic::AtomicUsize =
    std::sync::atomic::AtomicUsize::new(0);
/// Pure helper: pick the next non-blank model from `pool` using `ordinal` as
/// the index modulo the count of non-blank entries. Blank entries are skipped
/// so config typos don't waste a slot. Empty pool with a sentinel returns an
/// error so the operator sees a clear failure rather than silent inheritance.
fn select_pool_spawn_model(pool: &[String], ordinal: usize) -> anyhow::Result<String> {
    let models: Vec<_> = pool
        .iter()
        .map(|model| model.trim())
        .filter(|model| !model.is_empty())
        .collect();
    let Some(model) = models.get(ordinal % models.len().max(1)) else {
        anyhow::bail!(
            "agents.swarm_model_pool is empty; the `pool`/`rotate` sentinel requires at least one non-blank entry"
        );
    };
    Ok((*model).to_string())
}
/// Production dispatcher: advance the process-global counter and resolve the
/// next model from `pool`. Errors bubble up to the spawn caller.
fn next_pool_spawn_model(pool: &[String]) -> anyhow::Result<String> {
    let ordinal = SWARM_MODEL_POOL_COUNTER.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
    select_pool_spawn_model(pool, ordinal)
}
...
fn try_resolve_swarm_spawn_selection(
    ...
    // the config pins a different model. The `pool`/`rotate` sentinel pulls
    // the next non-blank entry from the configured pool; an empty pool with
    // the sentinel fails clearly via the caller's bail!.
    ...
    if let Some(requested) = requested_model {
        ...
        if is_pool_sentinel(&requested) {
            return Ok(selection_for_concrete_model(
                next_pool_spawn_model(pool)?,
                coordinator,
            ));
        }
        return Ok(selection_for_concrete_model(requested, coordinator));
    }
    let configured_swarm_model = configured_swarm_model
        .map(|model| model.trim().to_string())
        .filter(|model| !model.is_empty());
    match configured_swarm_model.as_deref() {
        Some(model) if is_inherit_sentinel(model) => Ok(inherit_coordinator_selection(coordinator)),
        Some(model) if is_pool_sentinel(model) => Ok(selection_for_concrete_model(
            next_pool_spawn_model(pool)?,
            coordinator,
        )),
        Some(_) => Ok(selection_for_concrete_model(
            configured_swarm_model.expect("matched concrete configured model"),
            coordinator,
        )),
...
fn resolve_swarm_spawn_selection(
    ...
    try_resolve_swarm_spawn_selection(requested_model, configured_swarm_model, coordinator, &[])
        .expect("legacy selection tests do not use a model-pool sentinel")
}
...
pub(super) async fn spawn_swarm_agent(
    ...
    structural_review_runtime: Option<&Arc<super::structural_review::StructuralReviewRuntime>>,
    ...
    let selection = try_resolve_swarm_spawn_selection(
        requested_model.clone(),
        configured_swarm_model.clone(),
        &coordinator,
        &agents_config.swarm_model_pool,
    )?;
    ...
    // Tell the worker the spend budget it will actually be held to. The cap
    // depends on the resolved model (MiniMax routes are uncapped), and a
    // worker that cannot see the limit cannot wrap up before hitting it.
    let worker_token_cap = super::swarm_worker_budget::token_cap_for(spawn_model.as_deref());
    let startup_message = initial_message.as_deref().map(|message| {
        let message = append_swarm_completion_report_instructions(message);
        jcode_swarm_core::append_worker_budget_notice(&message, Some(worker_token_cap))
    });
        ...
        && is_headless_fallback
    {
        ...
        if let Some(agent_arc) = agent_arc {
            ...
            tokio::spawn(async move {
                ...
                let (start_message_index, worker_model) = {
                    let agent = agent_arc.lock().await;
                    (agent.message_count(), agent.provider_model())
                };
                let result = super::swarm_worker_budget::run_with_model(
                    &sid_clone,
                ...
                .await;
                // A headless worker cannot press the interactive fallback key,
                // so a credential/capacity failure would otherwise fail the
                // node while a working route sat idle. Switch routes and take
                // one more shot before reporting failure.
                let result = match result {
                    Err(error) => {
                        let error_text = crate::util::format_error_chain(&error);
                        match super::unattended_reroute::try_reroute_unattended_session(
                            &sid_clone,
                            &agent_arc,
                            &error_text,
                        )
                        ...
                        {
                            Some(reroute) => {
                                crate::logging::info(&reroute.notice());
                                let retry_event_tx = super::session_event_fanout_sender(
                                    sid_clone.clone(),
                                    Arc::clone(&swarm_members2),
                                );
                                let retry_model = {
                                ...
                                super::swarm_worker_budget::run_with_model(
                                    &sid_clone,
                                    process_message_streaming_mpsc(
                                        Arc::clone(&agent_arc),
                                        &initial_msg,
...
pub(super) async fn handle_comm_spawn(
    ...
    label: Option<String>,
    _request_metadata: SpawnMetadata,
