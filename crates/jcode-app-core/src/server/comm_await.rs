...
const DEFAULT_AWAIT_TIMEOUT_SECS: u64 = 4 * 60 * 60;
...
pub(super) fn timeout_summary(member_statuses: &[AwaitedMemberStatus]) -> String {
    let pending: Vec<String> = member_statuses
        ...
        .collect();
    format!(
        "Wait window ended; workers were not stopped and may still finish. Still waiting on: {}",
        pending.join(", ")
    )
}
...
pub(super) async fn handle_comm_await_members(
    ...
    if let Some(swarm_id) = swarm_id {
        ...
        let requested_deadline = SystemTime::now()
            ...
            .unwrap_or_default()
            .as_millis() as u64
            + Duration::from_secs(timeout_secs.unwrap_or(DEFAULT_AWAIT_TIMEOUT_SECS)).as_millis()
                as u64;
