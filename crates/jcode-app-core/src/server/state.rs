...
impl SwarmMember {
    pub fn durable_record(&self) -> SwarmMemberRecord {
        SwarmMemberRecord {
            ...
            // Routing facts are observed live and were previously dropped
            // here, which is why a finished swarm could not be audited.
    ...
    pub fn from_record(
        ...
        Self {
            ...
            todo_progress: None,
            todo_items: Vec::new(),
            // Restore the routing facts. Timing is deliberately not restored:
            // `elapsed_secs` measures the current process's view and would be
            // misleading across a reload, whereas which model ran is a
            // historical fact that stays true.
