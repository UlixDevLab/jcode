...
use super::client_session::{
    StdinRequestRoute, handle_clear_session, handle_reload, handle_resume_session, handle_subscribe,
};
use super::client_state::{
    handle_get_compacted_history, handle_get_history, handle_get_model_catalog, handle_get_state,
};
use super::client_writer::{PeerWriteError, write_direct_event, write_encoded};
...
use super::comm_session::{SpawnMetadata, handle_comm_spawn, handle_comm_stop};
use super::comm_sync::{
...
async fn handle_client_stdin_response(
    id: u64,
    request_id: String,
    input: String,
    ...
    let still_controls_session = {
        let connections = client_connections.read().await;
        connections
            .get(client_connection_id)
            .is_some_and(|info| info.session_id == client_session_id)
    };
    if !still_controls_session {
        // Dropping the sender resolves the protected operation as disconnected.
        // A superseded client can remain readable for one biased select
        // iteration, but it must never win an approval race in that window.
        stdin_responses.lock().await.remove(&request_id);
        crate::logging::event_warn(
            "CONSENT_GUARDRAIL",
            vec![
                ("decision", "reject_stale_response".to_string()),
        ...
        let _ = client_event_tx.send(ServerEvent::Error {
            id,
            message: "Stdin response rejected because this client no longer controls the session."
                .to_string(),
            retry_after_secs: None,
        });
        return;
    }
    handle_stdin_response(id, request_id, input, stdin_responses, client_event_tx).await;
}
    ...
    fn spawn(ctx: RequestHandlerWatchdogContext) -> Self {
        ...
        tokio::spawn(async move {
            ...
            for threshold_ms in REQUEST_HANDLER_STALL_THRESHOLDS_MS {
                ...
                // Diagnostics at 2s and 10s are intentionally non-invasive.
                // At the final 60s threshold, reserve a same-build recovery
                // only when the process-wide admission gate can prove there is
                // no active generation. Unknown/busy/cooldown states preserve
                // the running process and are observable below.
                if threshold_ms == *REQUEST_HANDLER_STALL_THRESHOLDS_MS.last().unwrap_or(&0) {
                    match crate::server::reserve_idle_stalled_recovery().await {
                        crate::server::IdleRecoveryReservation::Reserved => {
                            ...
                            crate::logging::event_warn(
                                "SERVER_REQUEST_HANDLER_IDLE_RECOVERY_RESERVED",
                                vec![
                                    ("request_id", ctx.request_id.to_string()),
                        ...
                        outcome => crate::logging::event_warn(
                            "SERVER_REQUEST_HANDLER_IDLE_RECOVERY_SKIPPED",
                            vec![
                                ("request_id", ctx.request_id.to_string()),
...
pub(super) async fn handle_client(
    ...
    structural_review_runtime: Arc<super::structural_review::StructuralReviewRuntime>,
) -> Result<()> {
                ...
                if request.is_lightweight_control_request() {
                    ...
                    handle_lightweight_control_request(
                        ...
                        LightweightControlContext {
                            ...
                            structural_review_runtime: &structural_review_runtime,
                        },
                    )
                    .await?;
    ...
    let provider = {
        // Bind this session to the subscription configured for its working
        // directory. The guard must outlive provider construction, which loads
        // credentials for the pinned account.
        let _account_pin =
            jcode_base::auth::account_binding::pin_for_new_session("openai", &initial_working_dir);
        provider_template.fork_for_new_session()
    };
    ...
    let mut new_agent =
        crate::hooks::with_client_terminal_env(active_terminal_env.clone(), async {
            Agent::new_initial_client_with_initial_working_dir(
                Arc::clone(&provider),
                registry.clone(),
    ...
    new_agent.set_structural_review_authority(
        structural_review_runtime.capability(client_session_id.clone()),
    );
    ...
    let disconnect_tx_for_events = disconnect_tx.clone();
    let event_handle = tokio::spawn(async move {
        while let Some(event) = client_event_rx.recv().await {
            ...
            if let Err(error) = write_encoded(&writer_clone, json.as_bytes()).await {
                ...
                if matches!(&error, PeerWriteError::TimedOut { .. }) {
                    crate::logging::event_warn(
                        "SERVER_CLIENT_EVENT_WRITE_TIMEOUT",
                        vec![
                            (
                                "client_connection_id",
                                client_connection_id_for_events.clone(),
                            ),
                            ("event", event_desc.clone()),
                            ("bytes", json.len().to_string()),
                        ],
                    );
                    // `write_all` may have emitted a partial frame before its
                    // deadline. Tell the owning lifecycle to close both halves
                    // instead of letting this peer reuse a corrupt stream.
                    let _ = disconnect_tx_for_events.send(());
                }
                crate::logging::warn(&format!(
    ...
    let stdin_request_route = StdinRequestRoute::new(stdin_req_tx.clone());
    {
        let mut agent_guard = agent.lock().await;
        agent_guard.set_stdin_request_tx(stdin_request_route.sender());
    }
    let stdin_forwarder = {
        ...
        let mut request = if let Some(request) = pending_request.take() {
                ...
                match bus_event {
                    ...
                    Ok(BusEvent::ModelsUpdated) => {
                        ...
                        if encoded_len > MAX_LIVE_AVAILABLE_MODELS_UPDATE_BYTES {
                            // Preserve compact route identities before falling
                            // back to names-only. This keeps dynamic Antigravity
                            // ids and also prevents the same compaction from
                            // silently dropping other providers' routes.
                            let compact_event = compact_available_models_event(&event);
                            let compact_encoded =
                                compact_event.as_ref().map(crate::protocol::encode_event);
                            if let (Some(compact_event), Some(compact_encoded)) =
                                (compact_event, compact_encoded)
                                && compact_encoded.len()
                                    <= MAX_LIVE_AVAILABLE_MODELS_UPDATE_BYTES
                            {
                                crate::logging::info(&format!(
                                    "Downgrading oversized bus AvailableModelsUpdated frame to compact route identities for connection {} ({} -> {} bytes)",
                                    client_connection_id,
                                    encoded_len,
                                    compact_encoded.len()
                                ));
                                let _ = client_event_tx.send(compact_event);
            ...
            match decode_request(&line) {
                ...
                Err(e) => {
                    ...
                    let json = encode_event(&event);
                    if write_encoded(&writer, json.as_bytes()).await.is_err() {
                        break;
                    }
                    continue;
                }
        ...
        match request {
                        ...
                        || sessions.read().await.contains_key(&target_session_id)
                    {
                        ...
                        agent = crate::hooks::with_client_terminal_env(
                            ...
                            handle_resume_session(
                                ...
                                &event_counter,
                                &swarm_event_tx,
                                Some(stdin_request_route.clone()),
                                supports_pdf_panels,
                            ),
                        )
                        .await?;
            ...
            Request::CommSeedGraph {
                ...
                super::comm_graph::handle_comm_seed_graph(
                    ...
                    &event_history,
                    &event_counter,
                    super::comm_graph::GraphSeedRuntime::with_sessions(&swarm_event_tx, &sessions),
                )
                .await;
            }
            ...
            Request::CommSpawn {
                ...
                handle_comm_spawn(
                    ...
                    effort,
                    label,
                    SpawnMetadata::new(mission_binding, dispatch_intent),
                    &client_event_tx,
                    &sessions,
                    &global_session_id,
                    &provider_template,
                    ...
                    &swarms_by_id,
                    &swarm_coordinators,
                    Some(&structural_review_runtime),
                    &swarm_plans,
                    &channel_subscriptions,
                    &channel_subscriptions_by_session,
                    &event_history,
...
async fn start_processing_message(
    ...
    // Only an accepted message from the controlling client starts a fresh
    // consent turn. Internal retries, swarm assignments, relays, reload
    // continuations, and other callers of `process_message_streaming_mpsc`
    // must not clear a denial or abandoned approval prompt.
    crate::tool::begin_consent_user_turn(client_session_id);
    if !agent
        .lock()
        .await
...
async fn cancel_processing_message(
    ...
    match crate::background::global()
        .cancel_live_tasks_for_session(&session_label)
        .await
    {
        Ok(cancelled) if cancelled > 0 => crate::logging::info(&format!(
            "SERVER_INTERRUPT_CANCEL_BACKGROUND_TASKS_CANCELLED request_id={:?} session={} count={}",
            request_id, session_label, cancelled
        )),
        Ok(_) => {}
        Err(error) => crate::logging::error(&format!(
            "SERVER_INTERRUPT_CANCEL_BACKGROUND_TASKS_FAILED request_id={:?} session={} error={}",
            request_id, session_label, error
        )),
    }
...
/// Keep route identities when the full multi-provider catalog is too large for
/// a bus frame. The route presentation fields are deliberately dropped, but
/// `model`/`provider`/`api_method` remain intact so dynamic ids such as
/// Antigravity's `chat_*` and `tab_*` models survive compaction.
fn compact_available_models_event(event: &ServerEvent) -> Option<ServerEvent> {
    let ServerEvent::AvailableModelsUpdated {
        provider_name,
        provider_model,
        available_models,
...
/// Server generation boundary for both ordinary and display-role streaming
/// turns. The mutually exclusive Agent streaming entry point owns admission
/// before any agent/provider work begins.
pub(super) async fn process_message_streaming_mpsc_with_display_role(
    agent: Arc<Mutex<Agent>>,
    content: &str,
    images: Vec<(String, String)>,
    system_reminder: Option<String>,
    ...
    process_locked_message_streaming_mpsc_with_display_role(
        &mut agent,
        content,
        images,
        system_reminder,
...
async fn process_locked_message_streaming_mpsc_with_display_role(
    agent: &mut Agent,
    content: &str,
    images: Vec<(String, String)>,
    system_reminder: Option<String>,
