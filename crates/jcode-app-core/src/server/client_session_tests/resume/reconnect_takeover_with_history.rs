...
async fn handle_resume_session_allows_reconnect_takeover_with_local_history() -> Result<()> {
    ...
    existing_agent
        .lock()
        .await
        .set_stdin_request_tx(old_stdin_tx);
    ...
    existing_agent
        ...
        .expect("resumed agent should retain an stdin route")
        .send(crate::tool::StdinInputRequest {
            request_id: "consent_after_takeover".to_string(),
            prompt: "test".to_string(),
            is_password: false,
            response_tx,
        })
        .expect("new owner stdin route should remain open");
    assert_eq!(
        new_stdin_rx
            .recv()
            .await
            .expect("new owner should receive future stdin requests")
            .request_id,
        "consent_after_takeover"
    );
    assert!(
        old_stdin_rx.try_recv().is_err(),
        "old owner must not receive prompts after takeover"
    );
