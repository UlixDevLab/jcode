...
async fn handle_resume_session_allows_multiple_live_tui_attach() -> Result<()> {
    ...
    let (owner_stdin_tx, mut owner_stdin_rx) = mpsc::unbounded_channel();
    existing_agent
        .lock()
        .await
        .set_stdin_request_tx(owner_stdin_tx);
    let (observer_stdin_tx, mut observer_stdin_rx) = mpsc::unbounded_channel();
    ...
    handle_resume_session(
        ...
        &event_counter,
        &swarm_event_tx,
        Some(StdinRequestRoute::new(observer_stdin_tx)),
        false,
    )
    .await?;
    ...
    existing_agent
        ...
        .expect("live owner route should remain configured")
        .send(crate::tool::StdinInputRequest {
            request_id: "after_observer_attach".to_string(),
            prompt: "test".to_string(),
            is_password: false,
            response_tx,
        })
        .expect("owner route should remain open");
    assert_eq!(
        owner_stdin_rx
            .recv()
            .await
            .expect("existing owner receives future prompts")
            .request_id,
        "after_observer_attach"
    );
    assert!(
        observer_stdin_rx.try_recv().is_err(),
        "observer attach must not steal stdin prompt ownership"
    );
