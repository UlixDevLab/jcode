...
async fn startup_recovery_resumes_interrupted_headless_sessions_after_reload() -> Result<()> {
    ...
    let recovered_peer = server
        .sessions
        .read()
        .await
        .get(&peer.id)
        .cloned()
        .expect("recovery retains the stored headless session id");
    let recovered_peer = recovered_peer.lock().await;
    assert_eq!(recovered_peer.session_id(), peer.id);
    let error = recovered_peer
        .execute_tool(
            "structural_review",
            serde_json::json!({
                "action": "submit",
        ...
        .await
        .expect_err("recovered Agent reaches the authority-bound tool path");
    assert!(
        !error
            .to_string()
            .contains("structural review authority is unavailable"),
        "recovered Agent must receive the server-issued capability: {error:#}"
    );
    assert!(
        error.to_string().contains("structural review request"),
        "the recovered Agent must reach receipt validation: {error:#}"
    );
