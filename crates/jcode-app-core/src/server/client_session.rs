// sonar-file-size: {"waive":350,"owner":"jcode-maintainers","ticket":"Jcode-control-plane-policy","baseline_loc":1881,"max_loc":1881,"expires":"2026-12-01","removal_trigger":"module reaches 350 LOC","reason":"Policy extraction keeps inherited module at or below mission-base LOC."}
...
use super::{
    ...
    StructuralReviewRuntime, SwarmEvent, SwarmMember, SwarmState, VersionedPlan,
...
#[derive(Clone)]
pub(super) struct StdinRequestRoute {
    sender: mpsc::UnboundedSender<crate::tool::StdinInputRequest>,
    generation: Arc<AtomicU64>,
}

impl StdinRequestRoute {
    pub(super) fn new(sender: mpsc::UnboundedSender<crate::tool::StdinInputRequest>) -> Self {
        Self {
            sender,
    ...
    pub(super) fn sender(&self) -> mpsc::UnboundedSender<crate::tool::StdinInputRequest> {
        self.sender.clone()
    }
...
pub(super) async fn handle_clear_session(
    ...
    let (preserve_debug, working_dir, tool_policy_mode) = {
        let agent_guard = agent.lock().await;
        (
            agent_guard.is_debug(),
            agent_guard.working_dir().map(str::to_string),
            agent_guard.session_tool_policy_mode(),
        )
    };
    ...
    let mut new_agent = Agent::new_with_initial_working_dir_and_tool_policy_mode(
        Arc::clone(provider),
        registry.clone(),
...
fn apply_or_defer_subscribe_rlm_identity(
    agent: &Arc<Mutex<Agent>>,
    session_id: &str,
    identity: crate::rlm::ClientIdentity,
) {
    if let Ok(mut agent_guard) = agent.try_lock() {
        agent_guard.set_rlm_client_identity(identity);
        return;
    }
    ...
    let session_id = session_id.to_string();
    tokio::spawn(async move {
        agent.lock().await.set_rlm_client_identity(identity);
        crate::logging::info(&format!(
            "Applied deferred RLM subscribe identity for session {}",
            session_id
        ));
    });
}
...
pub(super) async fn handle_subscribe(
    ...
    apply_or_defer_subscribe_rlm_identity(
        agent,
        client_session_id,
        crate::rlm::ClientIdentity::from(rlm_client),
    );
    ...
    let mcp_register_ms = if register_mcp_tools {
        ...
        registry
            .rebind_working_dir(mcp_working_dir.as_deref())
            .await;
        registry
...
pub(super) async fn handle_resume_session(
    ...
    event_counter: &Arc<std::sync::atomic::AtomicU64>,
    swarm_event_tx: &broadcast::Sender<SwarmEvent>,
    stdin_request_route: Option<StdinRequestRoute>,
    ...
    if let Some(live_target_agent) = live_target_agent.as_ref() {
        ...
        let mut transfer_stdin_owner = conflicting_live_client.is_none();
        let mut defer_stdin_rebind = live_target_busy;
        if let Some(conflict) = conflicting_live_client {
            ...
            if can_take_over_live_session {
                ...
                transfer_stdin_owner = true;
                defer_stdin_rebind = transferred_processing || live_target_busy;
        ...
        if transfer_stdin_owner {
            let _rebind_task = bind_resumed_stdin_owner(
                live_target_agent,
                stdin_request_route.clone(),
                defer_stdin_rebind,
                client_connections,
                client_connection_id,
                &session_id,
        ...
        Ok(_prev_status) => {
            ...
            *client_session_id = session_id.clone();
            ...
            agent.lock().await.set_structural_review_authority(
                structural_review_runtime.capability(session_id.clone()),
            );
            update_member_status(
                &session_id,
                "ready",
...
async fn bind_resumed_stdin_owner(
    agent: &Arc<Mutex<Agent>>,
    stdin_request_route: Option<StdinRequestRoute>,
    defer_until_agent_is_idle: bool,
    client_connections: &Arc<RwLock<HashMap<String, ClientConnectionInfo>>>,
    client_connection_id: &str,
    session_id: &str,
) -> Option<tokio::task::JoinHandle<()>> {
    let Some(stdin_request_route) = stdin_request_route else {
        return None;
    };
    if !client_still_controls_session(client_connections, client_connection_id, session_id).await {
        return None;
    }
    let generation = stdin_request_route.claim_generation();
    if defer_until_agent_is_idle {
        let agent = Arc::clone(agent);
        let client_connections = Arc::clone(client_connections);
        let client_connection_id = client_connection_id.to_string();
        let session_id = session_id.to_string();
        Some(tokio::spawn(async move {
            let mut agent = agent.lock().await;
            let still_controls_session = client_connections
                .try_read()
                .ok()
                ...
                .unwrap_or(false);
            if still_controls_session && stdin_request_route.is_current_generation(generation) {
                agent.set_stdin_request_tx(stdin_request_route.sender());
            }
        }))
    } else {
        let mut agent = agent.lock().await;
...
async fn client_still_controls_session(
    client_connections: &Arc<RwLock<HashMap<String, ClientConnectionInfo>>>,
    client_connection_id: &str,
    session_id: &str,
) -> bool {
