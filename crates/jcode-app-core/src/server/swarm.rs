// sonar-file-size: {"waive":350,"owner":"jcode-maintainers","ticket":"Jcode-control-plane-policy","baseline_loc":3424,"max_loc":3424,"expires":"2026-12-01","removal_trigger":"module reaches 350 LOC","reason":"Policy extraction keeps inherited module at or below mission-base LOC."}
...
/// Upper bound (in characters) for completion-report text surfaced into a
/// parent-facing notification. The full report stays stored on the
/// `SwarmMember` and stays reachable through swarm status/report inspection
/// or `comm_await_members`, while this cap keeps a child worker's TLDR-less
/// dump from flooding the coordinator's context.
pub(super) const PARENT_COMPLETION_PREVIEW_MAX_CHARS: usize = 800;

/// Suffix appended to a clipped completion preview so the parent knows the
/// text was truncated and how to fetch the full stored report. Length is
/// included in [`PARENT_COMPLETION_PREVIEW_MAX_CHARS`].
const PARENT_COMPLETION_PREVIEW_TRUNCATED_SUFFIX: &str = "\n\n[Preview truncated by jcode. Retrieve the full report with swarm status or swarm await_members.]";

/// Render the bounded completion text a parent should see.
///
/// Prefers an explicit TLDR (the worker's one-line promise). When absent,
/// takes the first [`PARENT_COMPLETION_PREVIEW_MAX_CHARS`] of the trimmed
/// report and, if truncation happened, appends a retrieval hint pointing at
/// the still-stored full report. Short reports are returned verbatim.
///
/// The full report remains in `SwarmMember.latest_completion_report` and is
/// still retrievable through explicit status/report inspection — this only
/// governs the text emitted into automatic parent wake-ups.
pub(super) fn parent_completion_preview(
    report: Option<&str>,
    tldr: Option<&str>,
) -> Option<String> {
    if let Some(tldr) = tldr {
        let normalized: String = tldr.split_whitespace().collect::<Vec<_>>().join(" ");
    ...
    const MAX: usize = PARENT_COMPLETION_PREVIEW_MAX_CHARS;
    if report.chars().count() <= MAX {
        return Some(report.to_string());
    }

    let keep = MAX.saturating_sub(PARENT_COMPLETION_PREVIEW_TRUNCATED_SUFFIX.chars().count());
    let mut preview: String = report.chars().take(keep).collect();
    preview.push_str(PARENT_COMPLETION_PREVIEW_TRUNCATED_SUFFIX);
    Some(preview)
}
...
pub(super) async fn touch_swarm_task_progress(
    ...
    let revived = {
        ...
        // This is a server-generated bookkeeping heartbeat, not proof that the
        // provider or a tool made progress. Do not reset the productive-activity
        // clock used by the worker inactivity watchdog.
    ...
    if let Some(ref id) = swarm_id {
        ...
        if should_notify_coordinator {
            ...
            if let Some(recipient_session_id) = recipient_session_id {
                ...
                // Keep automatic coordinator wake-ups compact. The complete report remains
                // stored on the member and is available through await/status reads, while the
                // explicitly supplied TLDR avoids pinning a multi-kilobyte report into every
                // subsequent coordinator request.
                let notification_report =
                    parent_completion_preview(completion_report.as_deref(), report_tldr.as_deref());
                let msg =
                    completion_notification_message(name, status, notification_report.as_deref());
                let _ = fanout_session_event(
                    swarm_members,
...
pub(super) async fn run_swarm_task(
    ...
    session.tool_policy_mode = crate::config::SessionToolPolicyMode::Normal;
...
mod tests {
    use super::{
        PARENT_COMPLETION_PREVIEW_MAX_CHARS, broadcast_swarm_plan,
    ...
    #[test]
    fn parent_completion_preview_prefers_tldr_when_present() {
        use super::parent_completion_preview;
        let preview = parent_completion_preview(
            Some(&"x".repeat(PARENT_COMPLETION_PREVIEW_MAX_CHARS + 200)),
            Some("verified parser; focused tests pass"),
        )
        .expect("tldr present");
        assert_eq!(preview, "verified parser; focused tests pass");
        // The full report must not leak when a TLDR is supplied.
        assert!(!preview.contains('x'));
    }

    #[test]
    fn parent_completion_preview_returns_full_short_report_unchanged() {
        use super::parent_completion_preview;
        let report = "All ten tests passed; coverage 91%.";
        let preview = parent_completion_preview(Some(report), None).expect("short report kept");
        assert_eq!(preview, report);
    }

    #[test]
    fn parent_completion_preview_caps_long_report_at_max_chars() {
        use super::parent_completion_preview;
        let long: String = "a".repeat(PARENT_COMPLETION_PREVIEW_MAX_CHARS * 4);
        let preview = parent_completion_preview(Some(&long), None).expect("long report");
        assert!(
            preview.chars().count() <= PARENT_COMPLETION_PREVIEW_MAX_CHARS,
            "preview must respect bound, got chars={}",
            preview.chars().count()
        );
        assert!(preview.contains("Preview truncated by jcode"));
        assert!(preview.contains("Retrieve the full report"));
    }

    #[test]
    fn parent_completion_preview_collapses_whitespace_in_tldr() {
        use super::parent_completion_preview;
        let preview = parent_completion_preview(
            Some("any report"),
    ...
    #[test]
    fn parent_completion_preview_returns_none_when_both_inputs_blank() {
        use super::parent_completion_preview;
        assert!(parent_completion_preview(Some("   \n\t  "), None).is_none());
        assert_eq!(
            parent_completion_preview(Some("anything"), Some("   ")).as_deref(),
            Some("anything")
        );
        assert!(parent_completion_preview(None, None).is_none());
    }
    ...
    #[tokio::test]
    async fn update_member_status_prefers_tldr_in_owner_notification() {
        let swarm_members = Arc::new(RwLock::new(HashMap::new()));
        let swarms_by_id = Arc::new(RwLock::new(HashMap::from([(
            "swarm-1".to_string(),
            HashSet::from(["coord".to_string(), "worker".to_string()]),
        ...
        update_member_status_with_report_tldr(
            ...
            "ready",
            None,
            Some("FULL REPORT SHOULD STAY STORED".to_string()),
            Some("Parser verified; focused tests pass.".to_string()),
            &swarm_members,
            &swarms_by_id,
            None,
            None,
        ...
        assert_eq!(
            swarm_members
                ...
                .await
                .get("worker")
                .and_then(|member| member.latest_completion_report.as_deref()),
            Some("FULL REPORT SHOULD STAY STORED")
        );
        let events: Vec<_> = std::iter::from_fn(|| coord_rx.try_recv().ok()).collect();
        assert!(events.iter().any(|event| {
            matches!(
                event,
                ServerEvent::Notification {
                    notification_type: NotificationType::Message { tldr, .. },
                    message,
                    ..
                } if tldr.as_deref() == Some("Parser verified; focused tests pass.")
                    && message.contains("Report:\nParser verified; focused tests pass.")
                    && !message.contains("FULL REPORT SHOULD STAY STORED")
            )
        }));
    }

    #[tokio::test]
    async fn update_member_status_caps_owner_notification_for_huge_report() {
        // Surface the regression that motivated this change: a TLDR-less worker
        // submitting a multi-kilobyte completion report must not push the full
        // body into the coordinator's wake-up message. The full report stays
        // stored on the SwarmMember, retrievable through status/report or
        // comm_await_members (latest_completion_report).
        let swarm_members = Arc::new(RwLock::new(HashMap::new()));
        let swarms_by_id = Arc::new(RwLock::new(HashMap::from([(
            "swarm-1".to_string(),
            HashSet::from(["coord".to_string(), "worker".to_string()]),
        ...
        // A worker dumped ~3.8KB with no TLDR. normalize_completion_report
        // keeps the full body on the member; the bounded preview is only for
        // the parent-facing notification.
        let huge_report: String = "y".repeat(3800);
        let stored_chars = huge_report.chars().count();

        update_member_status_with_report_tldr(
            "worker",
            "ready",
            None,
            Some(huge_report.clone()),
            None,
            &swarm_members,
            &swarms_by_id,
            None,
        ...
        // The full report stays intact for explicit retrieval.
        let stored = swarm_members
            .read()
            .await
            .get("worker")
            .and_then(|member| member.latest_completion_report.clone())
            .expect("full report stored");
        assert_eq!(
            stored.chars().count(),
            stored_chars,
            "full report must remain unchanged in member storage"
        );

        let events: Vec<_> = std::iter::from_fn(|| coord_rx.try_recv().ok()).collect();
        let bounded_message = events
            .iter()
            .find_map(|event| match event {
                ServerEvent::Notification { message, .. } => Some(message.as_str()),
                _ => None,
            })
            .expect("owner notification emitted");

        // The bounded parent-visible ceiling: intro + Report header +
        // PARENT_COMPLETION_PREVIEW_MAX_CHARS preview + retrieval hint +
        // followup guidance. Includes slack for the prefix/suffix text.
        const PARENT_VISIBLE_CEILING: usize = 1400;
        assert!(
            bounded_message.chars().count() <= PARENT_VISIBLE_CEILING,
            "owner-notification message leaked full report: chars={}",
            bounded_message.chars().count()
        );

        // A bounded prefix is expected, but a run longer than the preview cap
        // proves the full body leaked into the notification.
        assert!(
            !bounded_message.contains(&"y".repeat(PARENT_COMPLETION_PREVIEW_MAX_CHARS + 1)),
            "full report body was forwarded verbatim to coordinator wake-up"
        );

        // Retrieval guidance must be present so the parent knows how to fetch
        // the stored full report.
        assert!(
            bounded_message.contains("Retrieve the full report"),
            "bounded preview must include retrieval guidance, got: {bounded_message}"
        );
    }
