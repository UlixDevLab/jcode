...
/// Owner-only control socket used solely to prove a live daemon supports the
/// stalled-request SIGUSR1 recovery fallback. It is tied to one primary socket,
/// not the registry, so stale registry PID records cannot authorize a signal.
#[cfg(unix)]
pub fn reload_control_socket_path(path: &std::path::Path) -> Option<PathBuf> {
    let filename = path.file_name()?.to_str()?;
    let base = filename.strip_suffix(".sock")?;
    Some(path.with_file_name(format!("{base}-reload-control.sock")))
}
...
pub fn cleanup_socket_pair(path: &std::path::Path) {
    ...
    #[cfg(unix)]
    if let Some(control) = reload_control_socket_path(path) {
...
pub async fn has_live_listener(path: &std::path::Path) -> bool {
    // A Unix listener can accept a connection yet never service it. Keep this
    // weaker liveness probe independently bounded so callers that combine it
    // with a readiness check cannot wait forever behind that peer.
    matches!(
        tokio::time::timeout(Duration::from_millis(250), socket_has_live_listener(path),).await,
        Ok(true)
    )
}
