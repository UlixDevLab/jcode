// sonar-file-size: waive=350 reason=pre-existing swarm control module; this mission passes the assigned worker model to the shared deterministic budget and decomposition is out of scope
...
fn spawn_assigned_task_run(
    ...
    tokio::spawn(async move {
        ...
        match result {
            ...
            Err(error) => {
                let retryable_budget_failure =
                    super::swarm_worker_budget::retryable_budget_error(&error);
                let mut task_requeued = false;
                {
                    let now_ms = now_unix_ms();
                    let mut plans = swarm_plans.write().await;
                    if let Some(plan) = plans.get_mut(&swarm_id)
                    ...
                    {
                        let progress = plan.task_progress.entry(task_id.clone()).or_default();
                        let prior_reclaims = progress.dead_assignee_reclaims.unwrap_or(0);
                        task_requeued = retryable_budget_failure
                            && prior_reclaims < crate::plan::MAX_DEAD_ASSIGNEE_RECLAIMS;
                        if task_requeued {
                            item.status = "queued".to_string();
                            item.assigned_to = None;
                            progress.assigned_session_id = None;
                            progress.dead_assignee_reclaims = Some(prior_reclaims + 1);
                            progress.completed_at_unix_ms = None;
                        } else {
                            item.status = "failed".to_string();
                            progress.completed_at_unix_ms = Some(now_ms);
                        ...
                        progress.checkpoint_summary = Some(truncate_detail(
                            &if task_requeued {
                                format!("task requeued after transient worker watchdog: {}", error)
                            } else {
