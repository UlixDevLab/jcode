...
/// A stalled request may only initiate a same-build recovery after this durable
/// interval. It prevents a still-sick daemon from repeatedly execing itself.
const STALLED_RECOVERY_COOLDOWN: Duration = Duration::from_secs(5 * 60);

/// Admission is shared by every fresh generation start and the idle-only
/// recovery reservation. A writer reservation closes the old process to new
/// generations before it observes liveness, avoiding an observe-then-exec
/// race. The new process gets a fresh, open in-memory gate after exec.
static RELOAD_ADMISSION: OnceLock<Arc<RwLock<()>>> = OnceLock::new();
static ACTIVE_GENERATIONS: AtomicUsize = AtomicUsize::new(0);
static STALLED_RECOVERY_RESERVED: AtomicBool = AtomicBool::new(false);

fn reload_admission() -> &'static Arc<RwLock<()>> {
    RELOAD_ADMISSION.get_or_init(|| Arc::new(RwLock::new(())))
}
...
/// A lease represents one live provider generation. Dropping it, including on
/// task cancellation, decrements the process-wide liveness count.
pub struct GenerationLease;

impl Drop for GenerationLease {
    fn drop(&mut self) {
        ACTIVE_GENERATIONS.fetch_sub(1, Ordering::AcqRel);
    }
}

pub async fn begin_generation() -> Result<GenerationLease, GenerationAdmission> {
    let admission = Arc::clone(reload_admission()).read_owned().await;
    if STALLED_RECOVERY_RESERVED.load(Ordering::Acquire) {
        return Err(GenerationAdmission::ReloadReserved);
    }
    ACTIVE_GENERATIONS.fetch_add(1, Ordering::AcqRel);
    drop(admission);
    Ok(GenerationLease)
}

#[derive(Debug, Clone, PartialEq, Eq)]
pub enum IdleRecoveryReservation {
    Reserved,
    ReloadAlreadyReserved,
    ActiveGenerations { count: usize },
    ReloadInProgress,
    CoolingDown,
...
struct StalledRecoveryCooldown {
    pid: u32,
    timestamp: String,
}

pub fn stalled_recovery_cooldown_path() -> PathBuf {
    crate::storage::runtime_dir().join("jcode.stalled-recovery.json")
}

fn cooldown_is_active() -> Result<bool, String> {
    let path = stalled_recovery_cooldown_path();
    if !path.exists() {
        return Ok(false);
    }
    let cooldown: StalledRecoveryCooldown = crate::storage::read_json(&path).map_err(|error| {
        format!(
            "cannot read stalled recovery cooldown {}: {error}",
            path.display()
        )
    })?;
    let timestamp = chrono::DateTime::parse_from_rfc3339(&cooldown.timestamp).map_err(|error| {
        format!(
            "cannot parse stalled recovery cooldown timestamp {}: {error}",
            cooldown.timestamp
        )
    })?;
    let elapsed = chrono::Utc::now().signed_duration_since(timestamp.with_timezone(&chrono::Utc));
    let Some(elapsed) = elapsed.to_std().ok() else {
        return Err("stalled recovery cooldown timestamp is in the future".to_string());
    };
    Ok(elapsed < STALLED_RECOVERY_COOLDOWN)
}

fn persist_stalled_recovery_cooldown() -> Result<(), String> {
    let path = stalled_recovery_cooldown_path();
    let cooldown = StalledRecoveryCooldown {
        pid: std::process::id(),
        timestamp: chrono::Utc::now().to_rfc3339(),
    };
    crate::storage::write_json(&path, &cooldown).map_err(|error| {
        format!(
            "cannot persist stalled recovery cooldown {}: {error}",
            path.display()
        )
    })
}

/// Reserve the running daemon for a same-build idle recovery. It never waits
/// for an agent/provider lock: any nonzero active count, an existing reload,
/// unreadable durability state, or a cooldown declines recovery safely.
pub async fn reserve_idle_stalled_recovery() -> IdleRecoveryReservation {
    let _admission = Arc::clone(reload_admission()).write_owned().await;
    if STALLED_RECOVERY_RESERVED.load(Ordering::Acquire) {
        return IdleRecoveryReservation::ReloadAlreadyReserved;
    }
    let active = ACTIVE_GENERATIONS.load(Ordering::Acquire);
    if active != 0 {
        return IdleRecoveryReservation::ActiveGenerations { count: active };
    }
    if reload_marker_active(Duration::from_secs(30)) {
        return IdleRecoveryReservation::ReloadInProgress;
    }
    // Validate the exact same-build exec target before recording a reservation.
    // If this cannot be established, leave the current generation untouched.
    if super::reload_current_executable_target().is_none() {
        return IdleRecoveryReservation::Indeterminate(
            "current executable is not safely reloadable".to_string(),
        );
    }
    match cooldown_is_active() {
        Ok(true) => return IdleRecoveryReservation::CoolingDown,
        Ok(false) => {}
        Err(error) => return IdleRecoveryReservation::Indeterminate(error),
    }
    if let Err(error) = persist_stalled_recovery_cooldown() {
        return IdleRecoveryReservation::Indeterminate(error);
    }
    STALLED_RECOVERY_RESERVED.store(true, Ordering::Release);
    IdleRecoveryReservation::Reserved
}
...
pub async fn await_reload_handoff(
    ...
    loop {
        let remaining = max_age.saturating_sub(started.elapsed());
        if remaining.is_zero() {
            return ReloadWaitStatus::Failed(Some(format!(
                "reload handoff exceeded {}ms deadline",
                max_age.as_millis()
            )));
        }
        let status = match tokio::time::timeout(
            remaining,
            inspect_reload_wait_status(socket_path, max_age, last_known_pid),
        )
        .await
        {
            Ok(status) => status,
            Err(_) => {
                return ReloadWaitStatus::Failed(Some(format!(
                    "reload handoff inspection exceeded {}ms deadline",
                    max_age.as_millis()
                )));
            }
        };
        match status {
            ReloadWaitStatus::Waiting { pid } => {
                ...
                {
                    return ReloadWaitStatus::Failed(Some(format!(
                        "reload handoff event wait exceeded {}ms deadline",
                        max_age.as_millis()
                    )));
                }
            }
...
pub struct ReloadSignal {
    ...
    /// Stalled-path recovery is never allowed to activate a channel candidate.
    /// It may only re-exec the executable that is already running.
    pub current_executable_only: bool,
...
pub fn send_reload_signal(
    ...
    let _ = tx.send(Some(ReloadSignal {
        ...
        triggering_session,
        prefer_selfdev_binary,
        current_executable_only: false,
        request_id: request_id.clone(),
    }));
    request_id
}

/// Send the recovery-only reload variant. The consumer must refuse recovery if
/// the current executable cannot be resolved to an existing on-disk file.
pub fn send_current_binary_reload_signal() -> String {
    let request_id = crate::id::new_id("stalled-recovery");
    let hash = jcode_build_meta::git_hash().to_string();
    crate::logging::info(&format!(
        "send_current_binary_reload_signal: request={} hash={} current_pid={}",
        request_id,
        hash,
        std::process::id()
    ));
...
mod tests {
    ...
    async fn idle_recovery_preserves_process_for_cooldown_unknown_and_persistence_failure() {
        let _lock = crate::storage::lock_test_env();
        let temp = tempfile::tempdir().expect("tempdir");
        let _guard = EnvGuard::set_runtime_dir(temp.path());
        let cooldown_path = stalled_recovery_cooldown_path();

        crate::storage::write_json(
            &cooldown_path,
            &StalledRecoveryCooldown {
                pid: std::process::id(),
                timestamp: chrono::Utc::now().to_rfc3339(),
            },
        )
        .expect("write active cooldown");
        assert_eq!(
            reserve_idle_stalled_recovery().await,
            IdleRecoveryReservation::CoolingDown,
            "an active cooldown must decline recovery before it can reserve or exec"
        );

        std::fs::write(&cooldown_path, b"not json").expect("write corrupt cooldown");
        assert!(
            matches!(
                reserve_idle_stalled_recovery().await,
                IdleRecoveryReservation::Indeterminate(_)
            ),
            "an unreadable cooldown must preserve the running process"
        );

        let non_directory = temp.path().join("not-a-runtime-directory");
        std::fs::write(&non_directory, b"file").expect("create non-directory");
        let _bad_runtime = EnvGuard::set_runtime_dir(&non_directory);
        assert!(
            matches!(
                reserve_idle_stalled_recovery().await,
                IdleRecoveryReservation::Indeterminate(_)
            ),
            "a cooldown persistence failure must preserve the running process"
        );
    }

    #[tokio::test]
    async fn new_generation_waits_behind_idle_recovery_admission_writer() {
        // `reserve_idle_stalled_recovery` owns this same writer before it
        // snapshots active leases and records its permanent reservation. A
        // generation cannot pass the reader side between those steps.
        let reservation = Arc::clone(reload_admission()).write_owned().await;
        let mut generation = tokio::spawn(begin_generation());
        assert!(
            tokio::time::timeout(Duration::from_millis(50), &mut generation)
                .await
                .is_err(),
            "a new generation must wait until the idle-recovery writer releases admission"
        );
        drop(reservation);
        let lease = tokio::time::timeout(Duration::from_secs(1), generation)
            .await
            .expect("generation should proceed after writer release")
            .expect("generation task should join")
            .expect("generation should be admitted after writer release");
        drop(lease);
    }
    ...
    #[tokio::test]
    #[allow(clippy::await_holding_lock)]
    async fn reload_handoff_deadline_reports_failed_without_a_responsive_peer() {
        let _lock = crate::storage::lock_test_env();
        let temp = tempfile::tempdir().expect("tempdir");
        let _guard = EnvGuard::set_runtime_dir(temp.path());
        ReloadState {
        ...
        let started = Instant::now();
        let status = await_reload_handoff(
            &temp.path().join("unresponsive.sock"),
        ...
        assert!(matches!(
            status,
            ReloadWaitStatus::Failed(Some(detail)) if detail.contains("deadline")
        ));
    }
