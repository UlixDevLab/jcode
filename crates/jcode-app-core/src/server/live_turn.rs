...
pub(super) async fn spawn_tracked_live_turn(
    ...
    tokio::spawn(async move {
        ...
        // A route that just died for credential or capacity reasons will die
        // the same way on the retry. An unattended session has nobody to press
        // the interactive fallback key, so switch routes here and take one more
        // shot before reporting failure.
        let result = match result {
            Err(error) => {
                let error_text = crate::util::format_error_chain(&error);
                match super::unattended_reroute::try_reroute_locked_agent(
                    &session_id,
                    &mut agent,
                    &error_text,
                ) {
