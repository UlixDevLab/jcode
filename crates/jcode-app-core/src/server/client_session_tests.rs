// sonar-file-size: waive=350 reason=pre-existing session integration suite; P3a adds reconnect identity regression coverage, and test-suite decomposition is out of scope
use super::{
    StdinRequestRoute, apply_or_defer_subscribe_rlm_identity, apply_or_defer_subscribe_working_dir,
    bind_resumed_stdin_owner, claim_live_target_agent, effective_subscribe_working_dir,
...
#[tokio::test]
async fn deferred_stdin_rebind_does_not_restore_a_superseded_controller() -> Result<()> {
    let provider: Arc<dyn Provider> = Arc::new(MockProvider);
    let registry = Registry::new(Arc::clone(&provider)).await;
    let agent = Arc::new(Mutex::new(Agent::new(provider, registry)));
    let (old_stdin_tx, mut old_stdin_rx) = mpsc::unbounded_channel();
    agent.lock().await.set_stdin_request_tx(old_stdin_tx);
    let (new_stdin_tx, mut new_stdin_rx) = mpsc::unbounded_channel();
    let now = Instant::now();
    let client_connections = Arc::new(RwLock::new(HashMap::from([(
        "conn_pending".to_string(),
        ClientConnectionInfo {
            client_id: "conn_pending".to_string(),
    ...
    let rebind_task = bind_resumed_stdin_owner(
        &agent,
        Some(StdinRequestRoute::new(new_stdin_tx)),
        true,
        &client_connections,
        "conn_pending",
        "session_owned",
    ...
    .await
    .expect("busy controller rebind should be deferred");
    client_connections.write().await.remove("conn_pending");
    drop(busy_guard);
    rebind_task.await?;
    ...
    agent
        .lock()
        .await
        .stdin_request_tx_for_test()
        .expect("original stdin route should remain")
        .send(crate::tool::StdinInputRequest {
            request_id: "after_supersede".to_string(),
            prompt: "test".to_string(),
            is_password: false,
            response_tx,
        })
        .expect("original route should remain open");
    assert_eq!(
        old_stdin_rx
            .recv()
            .await
            .expect("original route receives the test request")
            .request_id,
        "after_supersede"
    );
    assert!(
        new_stdin_rx.try_recv().is_err(),
        "superseded deferred route must never become active"
    );
    Ok(())
}

#[tokio::test]
async fn later_stdin_route_generation_wins_when_agent_was_busy() -> Result<()> {
    let provider: Arc<dyn Provider> = Arc::new(MockProvider);
    let registry = Registry::new(Arc::clone(&provider)).await;
    let agent = Arc::new(Mutex::new(Agent::new(provider, registry)));
    let (first_tx, mut first_rx) = mpsc::unbounded_channel();
    let first_route = StdinRequestRoute::new(first_tx);
    let (second_tx, mut second_rx) = mpsc::unbounded_channel();
    let second_route = StdinRequestRoute {
        sender: second_tx,
        generation: Arc::clone(&first_route.generation),
    };
    let now = Instant::now();
    let client_connections = Arc::new(RwLock::new(HashMap::from([(
    ...
    let busy_guard = agent.lock().await;
    let first_task = bind_resumed_stdin_owner(
        &agent,
        Some(first_route),
        true,
        &client_connections,
    ...
    .await
    .expect("first busy rebind should be deferred");
    let second_task = bind_resumed_stdin_owner(
        &agent,
        Some(second_route),
        true,
        &client_connections,
    ...
    .await
    .expect("replacement busy rebind should be deferred");
    drop(busy_guard);
    first_task.await?;
    second_task.await?;
    ...
    agent
        ...
        .await
        .stdin_request_tx_for_test()
        .expect("latest stdin route should be installed")
        .send(crate::tool::StdinInputRequest {
            request_id: "latest_generation".to_string(),
            prompt: "test".to_string(),
            is_password: false,
            response_tx,
        })
        .expect("latest route should remain open");
    assert_eq!(
        second_rx
            .recv()
            .await
            .expect("latest route receives the request")
            .request_id,
        "latest_generation"
    );
    assert!(
        first_rx.try_recv().is_err(),
        "earlier deferred generation must not overwrite the latest route"
    );
    Ok(())
}
...
#[tokio::test]
async fn subscribe_rlm_identity_is_agent_scoped_and_reconnect_is_authoritative() {
    let provider: Arc<dyn Provider> = Arc::new(MockProvider);
    let registry = Registry::new(Arc::clone(&provider)).await;
    let ordinary = Arc::new(Mutex::new(Agent::new(provider.clone(), registry.clone())));
    let rlm = Arc::new(Mutex::new(Agent::new(provider, registry)));

    apply_or_defer_subscribe_rlm_identity(
        &ordinary,
        "ordinary",
        crate::rlm::ClientIdentity::Ordinary,
    );
    apply_or_defer_subscribe_rlm_identity(&rlm, "rlm", crate::rlm::ClientIdentity::Rlm);
    assert_eq!(
        ordinary.lock().await.rlm_client_identity(),
        crate::rlm::ClientIdentity::Ordinary
    );
    assert_eq!(
        rlm.lock().await.rlm_client_identity(),
        crate::rlm::ClientIdentity::Rlm
    );

    // A resumed ordinary client must clear an earlier RLM client capability.
    apply_or_defer_subscribe_rlm_identity(&rlm, "rlm", crate::rlm::ClientIdentity::Ordinary);
    assert_eq!(
        rlm.lock().await.rlm_client_identity(),
        crate::rlm::ClientIdentity::Ordinary
    );
