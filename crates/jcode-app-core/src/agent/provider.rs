...
impl Agent {
    /// Register the root-owned current brief on this live Agent and persist the
    /// same session instance. Workers may receive the root brief read-only but
    /// cannot replace it through tool metadata.
    pub(crate) fn register_current_mission_brief(
        &mut self,
        brief: crate::session::CurrentMissionBrief,
    ) -> Result<()> {
        if self.session.parent_id.is_some() {
            anyhow::bail!("current mission brief requires the root coordinator");
        }
        brief.validate().map_err(|error| anyhow::anyhow!(error))?;
        self.session.current_mission_brief = Some(brief);
        self.session.save()
    }

    pub(crate) fn register_current_mission_brief_from_tool_metadata(
        &mut self,
        tool_name: &str,
        metadata: Option<&serde_json::Value>,
    ) -> Result<()> {
        if tool_name != "mission_brief" {
            return Ok(());
        }
        let Some(brief) = metadata.and_then(|metadata| metadata.get("current_mission_brief"))
        else {
            return Ok(());
        };
        let brief = serde_json::from_value(brief.clone())
            .context("mission brief returned invalid current mission brief")?;
        self.register_current_mission_brief(brief)
    }

    pub(crate) fn current_mission_brief_for_provider(
        &self,
    ) -> Option<crate::session::CurrentMissionBrief> {
        self.session.current_mission_brief.clone().or_else(|| {
            self.session.parent_id.as_deref().and_then(|parent_id| {
                crate::session::Session::load(parent_id)
                    .ok()
                    .and_then(|parent| parent.current_mission_brief)
            })
        })
    }
