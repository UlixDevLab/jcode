...
impl Agent {
    ...
    pub async fn debug_context(&mut self) -> serde_json::Value {
    ...
    fn append_current_mission_brief(&self, split: &mut crate::prompt::SplitSystemPrompt) {
        let Some(brief) = self.current_mission_brief_for_provider() else {
            return;
        };
        ...
        split.dynamic_part.push_str(&brief.render_for_provider());
        if self.session.parent_id.is_some() {
            split
                .dynamic_part
                .push_str("\n\nThis root-owned brief is context for the assigned work. A worker cannot replace it.");
        }
    }
    ...
    pub(super) fn build_system_prompt_split(
        ...
        if let Some(ref override_prompt) = self.system_prompt_override {
            ...
            self.append_current_mission_brief(&mut split);
            return split;
        }
