// sonar-file-size: waive=350 reason=pre-existing legacy blocking turn loop; this mission adds one observe-only RLM call-manifest hook and extracting the loop is out of scope
use super::*;
...
impl Agent {
    ...
    pub(crate) async fn prewarm_provider_idle(&mut self) {
    ...
    pub(super) async fn run_turn(&mut self, print_output: bool) -> Result<String> {
        ...
        let mut repeated_tool_breaker = ToolLoopBreaker::default();
        let mut rlm_call_index = 0u32;
        let rlm_root_message = self.rlm_turn_root_message();
        let mut rlm_previous_selected = None;
        ...
        loop {
            ...
            rlm_call_index = rlm_call_index.saturating_add(1);
            let provider_view = self.prepare_rlm_provider_view(
                &messages,
                rlm_root_message.as_ref(),
                rlm_call_index,
            );
            if rlm_previous_selected.is_some_and(|selected| selected != provider_view.selected) {
                self.cache_tracker.reset();
            }
            rlm_previous_selected = Some(provider_view.selected);
            if provider_view.selected {
                let selected_tokens = provider_view
                    .selection
                    .as_ref()
                    .map(|selection| selection.token_estimate)
                    .unwrap_or_default();
                logging::info(&format!(
                    "RLM pilot selected {} of {} baseline messages ({} estimated tokens)",
                    provider_view.messages.len(),
                    messages.len(),
                    selected_tokens
                ));
            } else if let Some(reason) = provider_view.fallback_reason.as_deref() {
                logging::info(&format!("RLM pilot using baseline: {reason}"));
            }
            let rlm_selection = provider_view.selection.clone();
            let messages: std::sync::Arc<[Message]> = provider_view.messages.into();
            ...
            if let Err(error) = crate::rlm::observe_provider_request(
                &self.session,
                send_messages,
                "non-streaming",
                rlm_call_index,
                self.rlm_requested_mode(),
                self.rlm_effective_mode(),
                rlm_selection.as_ref(),
            ) {
                logging::warn(&format!("RLM observe manifest failed: {error}"));
            }
            let prompt_has_recent_tool_result = Self::messages_end_with_tool_result(send_messages);
            self.last_status_detail = None;
            let mut stream = match jcode_base::usage_journal::scope(
                jcode_base::usage_journal::UsageContext {
                ...
                match event {
                    ...
                    StreamEvent::NativeToolCall {
                        ...
                        let tool_result = self
                            ...
                            .await
                            .and_then(|output| {
                                self.register_current_mission_brief_from_tool_metadata(
                                    &tool_name,
                ...
                || usage_cache_creation.is_some()
            {
                ...
                let _ = tokio::task::spawn_blocking(move || {
                    crate::provider_activity::record_token_usage(input, output);
                })
                .await;
            }
            ...
            for tc in tool_calls {
                ...
                let loop_guard = repeated_tool_breaker.before_call(&tc.name, &tc.input);
                if loop_guard.action == LoopGuardAction::Stop {
                    let message = loop_guard.message.expect("stop has a message");
                ...
                if let Some(error_msg) = tc.validation_error() {
                    ...
                    let error_msg = repeated_tool_breaker
                        .record_result(&tc.name, &tc.input, &error_msg)
                        .annotate(error_msg);
                    self.add_message(
                        Role::User,
                        vec![ContentBlock::ToolResult {
                ...
                if let Some((sdk_content, sdk_is_error)) = sdk_tool_results.remove(&tc.id) {
                    ...
                    if is_native_tool && sdk_is_error {
                        ...
                        let sdk_content = repeated_tool_breaker
                            .record_result(&tc.name, &tc.input, &sdk_content)
                            .annotate(sdk_content);
                        self.add_message(
                            Role::User,
                            vec![ContentBlock::ToolResult {
                                tool_use_id: tc.id,
                ...
                match result {
                    Ok(output) => {
                        ...
                        let raw_output = output.output.clone();
                        let loop_decision =
                            repeated_tool_breaker.record_result(&tc.name, &tc.input, &raw_output);
                        Bus::global().publish(BusEvent::ToolUpdated(ToolEvent {
                            session_id: self.session.id.clone(),
                            message_id: message_id.clone(),
                            tool_call_id: tc.id.clone(),
                        ...
                        output.output = loop_decision.annotate(output.output);
                    ...
                    Err(e) => {
                        ...
                        let loop_decision =
                            repeated_tool_breaker.record_result(&tc.name, &tc.input, &error_msg);
                        crate::telemetry::record_tool_failure();
                        Bus::global().publish(BusEvent::ToolUpdated(ToolEvent {
                            session_id: self.session.id.clone(),
                            message_id: message_id.clone(),
                        ...
                        let error_msg = loop_decision.annotate(error_msg);
                        if trace {
                            eprintln!(
