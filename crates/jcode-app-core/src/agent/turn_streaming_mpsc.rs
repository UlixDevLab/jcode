// sonar-file-size: waive=350 reason=pre-existing legacy streaming turn loop; this mission adds one observe-only RLM call-manifest hook and extracting the loop is out of scope
use super::*;
...
impl Agent {
    pub(super) async fn run_turn_streaming_mpsc(
        ...
        loop {
            ...
            let mut stream = {
                let mut complete_future = std::pin::pin!(jcode_base::usage_journal::scope(
                ...
                || usage_cache_creation.is_some()
            {
                ...
                let fresh_input = total
                    .saturating_sub(output)
                    .saturating_sub(usage_cache_read.unwrap_or(0));
                crate::session_metrics::record_token_usage_detailed(
                    &self.session.id,
                    total,
                    output,
                    fresh_input.saturating_add(output),
                );
            }
                ...
                if let Some(result) = tool_result {
                    ...
                    match result {
                        Ok(output) => {
                            ...
                            let loop_decision = repeated_tool_breaker.record_result(
                                &tc.name,
                                &tc.input,
                                &raw_output,
                            );
