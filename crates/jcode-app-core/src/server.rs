// sonar-file-size: {"waive":350,"owner":"@jcode-maintainers","ticket":"JCODE-QUALITY-RATCHET","baseline_loc":2382,"max_loc":2382,"expires":"2026-11-30","removal_trigger":"Extract server composition declarations into focused modules","reason":"Inherited oversized file changed for the bounded session activity protocol"}
...
mod runtime;
mod socket;
mod structural_review;
mod swarm;
mod swarm_channels;
mod swarm_mutation_state;
mod swarm_persistence;
mod swarm_worker_budget;
mod unattended_reroute;
mod util;
#[cfg(feature = "w13-test-driver")]
pub mod w13_test_support;
...
pub use self::reload_state::{
    GenerationAdmission, IdleRecoveryReservation, ReloadAck, ReloadPhase, ReloadSignal,
    ReloadState, ReloadWaitStatus, acknowledge_reload_signal, await_reload_handoff,
    begin_generation, clear_reload_marker, inspect_reload_wait_status, publish_reload_socket_ready,
    recent_reload_state, reload_marker_active, reload_marker_exists, reload_marker_path,
    reload_process_alive, reload_state_summary, reserve_idle_stalled_recovery,
    send_current_binary_reload_signal, send_reload_signal, wait_for_reload_ack,
    wait_for_reload_handoff_event, write_reload_marker, write_reload_state,
};

#[cfg(unix)]
pub use self::reload_control::signal_stalled_reload;
...
use self::structural_review::StructuralReviewRuntime;
...
use self::util::{
    ...
    reload_current_executable_target, reload_exec_target, startup_headless_recovery_test_delay,
    swarm_id_for_dir, swarm_id_for_session,
};

mod file_activity;
use self::file_activity::file_activity_scope_label;
mod file_touch_service;
mod gate_recovery_startup;
mod gate_resume;
mod hermes_gate_egress;
mod hermes_gate_ingress;
mod jcode_control;
...
pub struct Server {
    ...
    swarm_state: SwarmState,
    /// Server-only authority records and live reviewer designations.
    structural_review_runtime: Arc<StructuralReviewRuntime>,
    ...
    /// Cancels pull-only Hermes ingress tasks when this server stops.
    hermes_gate_ingress_cancellation: CancellationToken,
    /// Cancels the single Hermes gate-request egress task when this server stops.
    hermes_gate_egress_cancellation: CancellationToken,
}
...
impl Server {
    ...
    pub fn new_with_name(provider: Arc<dyn Provider>, server_name: Option<String>) -> Self {
        ...
        let structural_review_runtime = StructuralReviewRuntime::new(
            Arc::clone(&swarm_state.members),
            Arc::clone(&swarm_state.coordinators),
        );
        ...
        Self {
            ...
            hermes_gate_ingress_cancellation: CancellationToken::new(),
            hermes_gate_egress_cancellation: CancellationToken::new(),
        }
    }
    ...
    async fn recover_headless_sessions_on_startup(&self) {
        ...
        for session_id in sessions_to_restore {
            ...
            registry
                .rebind_working_dir(session.working_dir.as_ref().map(std::path::Path::new))
                .await;
            registry
            ...
            recovered_agent.set_structural_review_authority(
                self.structural_review_runtime
    ...
    async fn finish_startup_after_bind(
        ...
        // Publish readiness only after the accept loops are live.
        ...
        // Run recovery only after the replacement daemon accepts reconnects.
        self.recover_headless_sessions_on_startup().await;
        gate_recovery_startup::recover_unconsumed_gate_directives_on_startup(
            &self.sessions,
            &self.swarm_state.members,
        )
        .await;
        hermes_gate_ingress::spawn_if_configured(self);
        hermes_gate_egress::spawn_if_configured(self);
        jcode_control::spawn_if_configured(self);
    ...
    pub async fn run(&self) -> Result<()> {
        ...
        #[cfg(unix)]
        let reload_control_path = socket::reload_control_socket_path(&self.socket_path)
            .ok_or_else(|| anyhow::anyhow!("cannot derive reload control socket path"))?;
        #[cfg(unix)]
        crate::transport::remove_socket(&reload_control_path);

        let main_listener = Listener::bind(&self.socket_path)?;
        let debug_listener = Listener::bind(&self.debug_socket_path)?;
        #[cfg(unix)]
        let reload_control_listener = Listener::bind(&reload_control_path)?;
        ...
        #[cfg(unix)]
        {
            ...
            mark_close_on_exec(&reload_control_listener);
        }
        ...
        #[cfg(unix)]
        let _ = crate::platform::set_permissions_owner_only(&reload_control_path);
        ...
        #[cfg(unix)]
        tokio::spawn(reload_control::run(
            reload_control_listener,
            self.socket_path.clone(),
        ));
        ...
        tokio::select! {
            result = &mut main_handle => {
                ...
                self.hermes_gate_ingress_cancellation.cancel();
                self.hermes_gate_egress_cancellation.cancel();
