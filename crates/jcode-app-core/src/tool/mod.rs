// sonar-file-size: {"waive":350,"owner":"jcode-maintainers","ticket":"Jcode-control-plane-policy","baseline_loc":1351,"max_loc":1351,"expires":"2026-12-01","removal_trigger":"module reaches 350 LOC","reason":"Consent policy extraction keeps inherited registry at mission-base LOC."}
mod agentgrep;
pub mod ambient;
mod apply_patch;
mod bash;
...
mod read;
pub mod read_ledger;
mod request_user_gate;
#[cfg(test)]
mod request_user_gate_tests;
pub mod selfdev;
...
mod side_panel;
mod skill;
pub(crate) mod structural_review;
pub(crate) mod todo;
mod webfetch;
mod websearch;
mod write;
use crate::{compaction::CompactionManager, provider::Provider, skill::SkillRegistry};
use agentgrep::AgentGrepTool;
use anyhow::Result;
...
impl SessionToolPolicyRegistration {
    pub(crate) fn update(
        &self,
        allowed_tools: Option<HashSet<String>>,
        disabled_tools: HashSet<String>,
        ...
        if let Some(policy) = policies.get_mut(&self.session_id)
            && policy.owner == Some(self.owner)
        {
            policy.allowed_tools = allowed_tools;
            policy.disabled_tools = disabled_tools;
        }
    }
}
...
pub fn set_session_tool_policy(
    session_id: &str,
    allowed_tools: Option<HashSet<String>>,
    disabled_tools: HashSet<String>,
) {
    let mut policies = SESSION_TOOL_POLICIES
        .write()
        .unwrap_or_else(|poisoned| poisoned.into_inner());
    let owner = policies.get(session_id).and_then(|policy| policy.owner);
    policies.insert(
        session_id.to_string(),
        SessionToolPolicy {
            allowed_tools,
...
pub(crate) fn clear_session_tool_policy(session_id: &str) {
    ...
    consent::clear_session(session_id);
}

pub(crate) fn begin_consent_user_turn(session_id: &str) {
    consent::begin_user_turn(session_id);
}
...
impl Registry {
    ...
    fn base_tools(skills: &Arc<RwLock<SkillRegistry>>) -> HashMap<String, Arc<dyn Tool>> {
        ...
        let base = BASE.get_or_init(|| {
            ...
            Self::insert_tool_timed(&mut m, &mut timings, "agentgrep", AgentGrepTool::new);
            ...
            Self::insert_tool_timed(
                ...
                &mut timings,
                "structural_review",
                structural_review::StructuralReviewTool::new,
            );
            consent_grants::register_tool(&mut m, &mut timings);
            ...
            Self::insert_tool_timed(
                ...
                &mut timings,
                "mission_brief",
                mission_brief::MissionBriefTool::new,
            );
            let request_user_gate = request_user_gate::RequestUserGateTool::new;
            Self::insert_tool_timed(&mut m, &mut timings, "request_user_gate", request_user_gate);
    ...
    pub async fn new(provider: Arc<dyn Provider>) -> Self {
        ...
        Self::insert_tool(&mut tools_map, "fable", fable::FableTool::new(provider));
        ...
        if let Some(object) = input.as_object() {
            ...
            for key in path_fields {
                if let Some(path) = object.get(key).and_then(Value::as_str) {
                    let path = if consent::is_browser_url(path) {
                        consent::safe_url_summary(path)
                    } else {
                        ctx.resolve_path(std::path::Path::new(path))
                            .display()
                            .to_string()
                    };
                    touched_paths.push(format!("{key}:{}", path));
                }
            }
    ...
    pub async fn execute(&self, name: &str, input: Value, ctx: ToolContext) -> Result<ToolOutput> {
        ...
        drop(tools);
        // Native consent is the first external boundary. It is deliberately
        // before user-configured hooks, and never consults model input, so an
        // untrusted tool call cannot self-authorize or leak desktop/browser
        // access through a hook.
        let operation = match consent::protected_operation_with_context(resolved_name, &input, &ctx)
        {
            Some(operation) => Some(operation),
            None => tool
                .native_consent_requirement(&input)
                .await
                .map(consent::ClassifiedOperation::from_native),
        };
        let native_consent_granted = if let Some(operation) = operation {
            if operation.requires_native_consent(&crate::config::config().consent) {
                consent::request_classified_operation(&operation, &ctx).await?;
                true
            } else {
                crate::logging::event_info(
                    "CONSENT_GUARDRAIL",
                    vec![
                        ("decision".to_string(), "allow-policy".to_string()),
                        ("session_id".to_string(), ctx.session_id.clone()),
                        ("family".to_string(), operation.family().to_string()),
                    ],
                );
                false
            }
    ...
    /// Rebind working-directory-sensitive tool descriptions to this session's
    /// project directory.
    ///
    /// [`Self::base_tools`] is a process-wide `OnceLock` cache, so any tool that
    /// reads project files while building its description would otherwise be
    /// frozen with whatever the *first* session in the process happened to see
    /// (in practice, the server process cwd). `swarm` embeds
    /// `.jcode/swarm-prompt.md` in its description, so on a shared server one
    /// project's routing policy leaked into every other project's sessions.
    ///
    /// Callers that know their session working directory should call this right
    /// after constructing the registry, alongside
    /// [`Self::register_mcp_tools_for_dir`]. Passing `None` is a no-op, so the
    /// global/default path keeps its existing behavior and cost.
    pub async fn rebind_working_dir(&self, working_dir: Option<&std::path::Path>) {
        ...
        tools.insert(
            "swarm".to_string(),
            Arc::new(communicate::CommunicateTool::for_working_dir(Some(
                working_dir,
            ))) as Arc<dyn Tool>,
        );
    }
