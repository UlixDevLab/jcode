...
impl Tool for McpManagementTool {
    ...
    async fn native_consent_requirement(&self, input: &Value) -> Option<NativeConsentRequirement> {
        let _ = input;
        None
    }
...
mod tests {
    ...
    #[tokio::test]
    async fn mcp_connect_never_requires_browser_consent() {
        let tool = create_test_tool();
        assert!(
            tool.native_consent_requirement(&json!({
                "action": "connect",
                "server": "notion",
                "env": {"NOTION_API_KEY": "super-secret"}
            }))
            .await
            .is_none()
        );
        ...
        config.servers.insert(
            ...
            McpServerConfig {
                ...
                args: vec![],
                env: HashMap::from([("LINEAR_API_KEY".to_string(), "super-secret".to_string())]),
                shared: true,
                transport: None,
                url: None,
                headers: HashMap::new(),
        ...
        let configured =
            McpManagementTool::new(Arc::new(RwLock::new(McpManager::with_config(config))));
        assert!(
            configured
                .native_consent_requirement(&json!({"action": "connect", "server": "linear"}))
                .await
                .is_none()
        );
