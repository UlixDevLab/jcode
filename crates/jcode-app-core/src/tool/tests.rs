...
// sonar-file-size: waive=350 reason=pre-existing legacy test module; this mission adds scoped consent regressions and full decomposition is out of scope
...
struct UntrustedConsentTool;

#[async_trait]
impl Tool for UntrustedConsentTool {
    fn name(&self) -> &str {
        "untrusted_consent"
    }

    fn description(&self) -> &str {
        "Test-only untrusted consent descriptor source."
    }
    ...
    async fn execute(&self, _input: Value, _ctx: ToolContext) -> Result<ToolOutput> {
        Ok(ToolOutput::new("should not execute after denial"))
    }

    async fn native_consent_requirement(
        &self,
        _input: &Value,
    ) -> Option<jcode_tool_core::NativeConsentRequirement> {
        Some(jcode_tool_core::NativeConsentRequirement::new(
            "tool\r\naction=forged",
            "call\nextra=forged",
            "https://alice:super-secret@example.test/path?token=super-secret\r\nforged=value",
        ))
    }
}

#[tokio::test]
async fn registry_sanitizes_untrusted_native_consent_metadata() {
    use tokio::sync::mpsc;

    let registry = Registry::empty();
    registry
        .register(
            "untrusted_consent".to_string(),
            Arc::new(UntrustedConsentTool),
        )
        .await;
    let (tx, mut rx) = mpsc::unbounded_channel();
    let context = ToolContext {
        session_id: "sanitization".to_string(),
        message_id: "message".to_string(),
    ...
    let task = tokio::spawn(async move {
        registry
            .execute("untrusted_consent", serde_json::json!({}), context)
            .await
    });

    let request = rx.recv().await.expect("native consent request");
    assert!(request.prompt.contains("tool=<redacted>"));
    assert!(request.prompt.contains("action=<redacted>"));
    assert!(request.prompt.contains("target=https://example.test/path"));
    for forbidden in ["super-secret", "token=", "forged="] {
        assert!(
    ...
    assert!(
        task.await
            .unwrap()
            .expect_err("denial should fail closed")
            .to_string()
            .contains("<redacted> <redacted> blocked")
    );
}

#[test]
fn lifecycle_fields_redact_browser_url_credentials() {
    let context = ToolContext {
    ...
    let fields = Registry::tool_lifecycle_fields(
        "start",
        "open",
        "open",
        &serde_json::json!({
            "target": "https://alice:super-secret@example.test/path?token=super-secret#fragment"
        }),
        &context,
    );
    let touched = fields
        .iter()
        .find_map(|(key, value)| (key == "touched_paths").then_some(value))
        .expect("target should be logged as a touched path");
    assert_eq!(touched, "target:https://example.test/path");
    assert!(!touched.contains("super-secret"));
    assert!(!touched.contains("alice"));
}
...
#[tokio::test]
async fn registry_fails_closed_for_credentialed_service_without_tui_client() {
    let registry = Registry::empty();
    registry
        .register("bash".to_string(), Arc::new(bash::BashTool::new()))
        .await;
    let ctx = ToolContext {
        session_id: "service-consent".to_string(),
        message_id: "message".to_string(),
        tool_call_id: "call".to_string(),
        working_dir: None,
        stdin_request_tx: None,
    ...
    let error = registry
        .execute(
            "bash",
            serde_json::json!({"command": "stripe customers list"}),
            ctx,
        )
        .await
        .expect_err("credentialed service call must require a current TUI client");
