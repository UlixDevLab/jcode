...
pub(super) fn bash_parameters_schema() -> serde_json::Value {
    ...
    serde_json::json!({
        ...
        "properties": {
            ...
            "timeout": {
                "type": "integer",
                "description": "Timeout in MILLISECONDS (not seconds), e.g. 3600000 = 1h. Background commands are killed with exit 124 at this limit; foreground commands are promoted to background instead of killed. Omit for the default foreground handoff or no background timeout."
            },
            "run_in_background": {
                "type": "boolean",
