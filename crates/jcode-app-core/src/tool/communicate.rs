...
#[path = "communicate_input.rs"]
mod communicate_input;
use super::{Tool, ToolContext, ToolOutput};
use crate::background::TaskResult;
use crate::protocol::{
    AgentInfo, AgentStatusSnapshot, AwaitedMemberStatus, ContextEntry, HistoryMessage,
    PlanGraphStatus, Request, ServerEvent, SwarmChannelInfo, TaskGraphNodeSpec, ToolCallSummary,
...
/// Long-running implementation and verification workers frequently exceed the
/// old one-hour wait window. Public waits are asynchronous and run_plan waits
/// in a background task by default, so a four-hour observation window does not
/// block the coordinator's turn while avoiding false plan-driver failures.
const DEFAULT_SWARM_AWAIT_TIMEOUT_MINUTES: u64 = 4 * 60;
...
use communicate_input::{CommunicateInput, dispatch_intent_schema};
...
fn seed_node_id_collision(response: &ServerEvent) -> Option<&str> {
    crate::decomposition_materializer::seed_node_id_collision(response)
}
...
/// The shared root cause behind a wave of worker failures.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
enum WorkerFailureCause {
    /// Broken credential: expired OAuth session, revoked refresh token, or an
    /// invalid API key.
    Credential,
    /// Exhausted account capacity: a subscription usage limit, spent credits,
    /// or a plan quota.
    Capacity,
}
...
///
/// The same shape holds when the account is out of subscription capacity: every
/// worker on that login gets a 429 until the limit resets, so dispatching more
...
struct CredentialFailureWave {
    ...
    /// What the wave's failures have in common.
...
fn detect_credential_failure_wave(
    ...
    let mut cause: Option<WorkerFailureCause> = None;
    for member in members {
        ...
        // Credential first: an auth failure that also mentions a quota is
        // still fixed by re-authenticating, and its message is the actionable
        // one. Any rate-limit shape counts here, not just a spent allowance: a
        // single transient 429 is noise, but a wave of them with nothing
        // completing means dispatching more nodes just fails more of them.
        let member_cause = if crate::provider::error_looks_like_credential_failure(detail) {
            WorkerFailureCause::Credential
        } else if crate::provider::error_looks_like_rate_limited(detail) {
            WorkerFailureCause::Capacity
        } else {
            continue;
        };
        ...
        // Mixed waves report as credential: it is the more actionable fix and
        // never leaves the user waiting on a reset that is not the problem.
        cause = Some(match (cause, member_cause) {
            (Some(WorkerFailureCause::Credential), _) | (_, WorkerFailureCause::Credential) => {
                WorkerFailureCause::Credential
            }
            _ => WorkerFailureCause::Capacity,
        });
    ...
    Some(CredentialFailureWave {
        ...
        provider,
        cause: cause.unwrap_or(WorkerFailureCause::Credential),
    })
}
...
fn format_credential_failure_wave_error(wave: &CredentialFailureWave, window_secs: u64) -> String {
    let count = wave.session_ids.len();
    let first = wave
        .session_ids
        .first()
        .map(String::as_str)
        .unwrap_or("worker");
    let sample = &wave.sample_detail;
    match wave.cause {
        WorkerFailureCause::Capacity => format!(
            "run_plan paused dispatching: {count} worker(s) failed within {window_secs}s with \
             capacity/quota exhaustion and no plan node has completed (e.g. {first}: \"{sample}\"). \
             Subscription capacity is metered per account, so every worker on that login fails \
             until the limit resets and assigning more nodes would only fail more of the plan. \
             Switch the swarm to a route with capacity (a different account, a different provider, \
             or a metered API key), then requeue the failed nodes (`swarm retry`) and run \
             `swarm run_plan` again. The failure detail above names the reset window when the \
             provider reported one."
        ),
        WorkerFailureCause::Credential => format!(
            "run_plan paused dispatching: {count} worker(s) failed within {window_secs}s with \
...
async fn broadcast_plan_alert(ctx: &ToolContext, message: &str) -> Result<()> {
    let request = Request::CommMessage {
        ...
        delivery: None,
        tldr: Some(
            "run_plan paused: worker failure wave; fix the root cause then retry".to_string(),
        ),
    };
    match send_request(request).await {
        Ok(response) => ensure_success(&response),
...
async fn run_swarm_plan_loop(
    ...
    let timeout_minutes = params
        .timeout_minutes
        .unwrap_or(DEFAULT_SWARM_AWAIT_TIMEOUT_MINUTES)
        .max(1);
...
async fn spawn_assignment_session(ctx: &ToolContext, params: &CommunicateInput) -> Result<String> {
    let spawn_request = params.build_spawn_request(
        &ctx.session_id,
        None,
        Some(fresh_spawn_request_nonce(ctx)),
        None,
    )?;
...
impl CommunicateTool {
    pub fn new() -> Self {
        Self::for_working_dir(None)
    }

    /// Build the tool with its swarm prompt resolved against `working_dir`
    /// instead of the process cwd.
    ///
    /// `load_swarm_prompt(None)` falls back to `Path::new(".")`, which is the
    /// *server process* cwd, not the session's project. On a shared server that
    /// meant the first session's `.jcode/swarm-prompt.md` was baked into the
    /// cached tool description and handed to every later session, including
    /// sessions for unrelated projects. Same class of bug as project MCP config
    /// discovery (#420), fixed the same way: a `_for_dir` constructor that the
    /// session-aware registry path calls with its real working directory.
    pub fn for_working_dir(working_dir: Option<&std::path::Path>) -> Self {
...
impl Tool for CommunicateTool {
    ...
    fn parameters_schema(&self) -> Value {
            ...
            "properties": {
                ...
                "action": {
                    ...
                    "enum": ["share", "share_append", "read", "message", "broadcast", "dm", "channel", "list", "list_channels", "channel_members",
                             ...
                             "task_graph", "materialize_graph", "expand_node", "complete_node", "inject_gap",
                ...
                "working_dir": {
                    "type": "string",
                    "description": "Absolute directory the worker runs in. Set it when the target repo differs from this session's."
                },
                "prompt": {
                    "type": "string",
                ...
                "timeout_minutes": {
                    "type": "integer",
                    "minimum": 1,
                    "description": "Observation window for await_members/run_plan. Defaults to 240 minutes. Expiry never stops workers."
                },
                "background": {
                    "type": "boolean",
                ...
                "plan_items": {
                    ...
                    "items": {
                        ...
                        "properties": {
                            "content": { "type": "string", "description": "What this item delivers." },
                            "status": { "type": "string", "description": "pending, running, completed, or blocked." },
                            "priority": { "type": "string", "description": "high, medium, or low." },
                            "id": { "type": "string", "description": "Stable id other items reference in blocked_by." },
                            "subsystem": { "type": "string", "description": "Area this item belongs to, for grouping." },
                            "file_scope": {
                                "type": "array",
                                "items": { "type": "string" },
                                "description": "Files this item may write. Items with disjoint scopes can run in parallel; overlapping scopes must not."
                            },
                            "blocked_by": {
                                "type": "array",
                                "items": { "type": "string" },
                                "description": "Ids that must finish first. Empty means ready to start now."
                            },
                            "assigned_to": { "type": "string", "description": "Session id currently owning this item." }
                        }
                    }
                }
            }
            ...
            .and_then(|value| value.as_object_mut())
        {
            ...
            props.insert("dispatch_intent".to_string(), dispatch_intent_schema());
        }
    ...
    async fn execute(&self, input: Value, ctx: ToolContext) -> Result<ToolOutput> {
        ...
        match params.action.as_str() {
            ...
            "materialize_graph" => {
                let prepared = crate::decomposition_materializer::prepare_root_materialization(
                    &ctx,
                ...
                if !prepared.requires_seed() {
                    return Ok(ToolOutput::new(prepared.outcome_message()));
                }
                let response = send_request(prepared.seed_request())
                    .await
                    .map_err(|error| {
                        anyhow::anyhow!("failed to materialize task graph: {error}")
                    })?;
                ensure_success(&response)?;
                Ok(ToolOutput::new(prepared.outcome_message()))
            }
            ...
            "task_graph" | "seed_graph" => {
                ...
                for _ in 0..=nodes.len() {
                    ...
                    if occupied.is_none() {
                        let summary = fetch_plan_status(&ctx.session_id).await?;
                        occupied = Some(crate::decomposition_materializer::plan_graph_node_ids(
                            &summary,
                        ));
                    }
                    ...
                    let (remapped, mut remaps) = remap_conflicting_seed_nodes(
                        ...
                        occupied,
                        conflicting_id,
                        &crate::decomposition_materializer::seed_retry_scope(&ctx),
                    );
                    if remaps.is_empty() {
                        ensure_success(&response)?;
            ...
            "spawn" => {
                ...
                let request = params.build_spawn_request(
                    &ctx.session_id,
...
#[cfg(test)]
#[path = "communicate_dispatch_intent_tests.rs"]
