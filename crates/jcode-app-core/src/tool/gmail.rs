// sonar-file-size: waive=350 reason=pre-existing legacy module; this mission adds only the native-consent seam and full decomposition is out of scope
...
impl Tool for GmailTool {
    ...
    async fn native_consent_requirement(
        ...
        (input.get("action").and_then(Value::as_str) == Some("connect")).then(|| {
            jcode_tool_core::NativeConsentRequirement::new(
                "gmail",
                "connect",
    ...
    async fn execute(&self, input: Value, _ctx: ToolContext) -> Result<ToolOutput> {
        ...
        match params.action.as_str() {
            ...
            "draft" => {
                ...
                Ok(ToolOutput::new(format!(
                    "Draft created successfully.\nDraft ID: {}\nTo: {}\nSubject: {}\n{}\nTo send this draft, use action 'send_draft' with draft_id '{}'. JCode will ask the current user for native one-shot consent before sending.",
...
mod tests {
    ...
    #[tokio::test]
    async fn gmail_api_actions_auto_run_but_browser_oauth_stays_protected() {
        let tool = GmailTool::new();
        for input in [
            json!({"action": "search", "query": "from:private@example.test"}),
            json!({
                "action": "send",
            ...
            assert!(tool.native_consent_requirement(&input).await.is_none());
        }

        let connect = tool
            .native_consent_requirement(&json!({"action": "connect"}))
            .await
            .expect("browser OAuth still needs consent");
        assert_eq!(connect.tool, "gmail");
        assert_eq!(connect.action, "connect");
        assert_eq!(connect.target_summary, "browser OAuth for Gmail account");

        let schema = tool.parameters_schema();
        let properties = schema["properties"].as_object().expect("properties");
        assert!(!properties.contains_key("confirmed"));
        assert!(!properties.contains_key("user_approval"));
    }
}
