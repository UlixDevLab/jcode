...
#[test]
fn description_labels_the_built_in_tool_as_firefox_only() {
    let tool = BrowserTool::new();
    let description = tool.description();
    assert!(description.contains("Firefox Agent Bridge"));
    assert!(description.contains("not Chromium"));
    assert!(description.contains("Playwright MCP"));
    assert!(description.contains("action='status'"));
    assert!(description.contains("setup only for Firefox"));
}
