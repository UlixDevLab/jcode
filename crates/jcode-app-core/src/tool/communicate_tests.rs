// sonar-file-size: waive=350 reason=pre-existing swarm communication suite; P3a only updates the subscribe fixture for protocol compatibility, and decomposition is out of scope
use super::{
...
/// The swarm prompt is embedded in the tool *description*, and `base_tools` is a
/// process-wide `OnceLock`. Resolving it from the process cwd therefore froze
/// the first session's project policy into every later session, including
/// sessions for unrelated projects on a shared daemon. The description must
/// follow the session working directory instead.
#[test]
fn swarm_prompt_resolves_from_the_session_working_dir_not_the_process_cwd() {
    ...
    for (dir, marker) in [
        (project_a.path(), "ROUTING_POLICY_FOR_PROJECT_A"),
        (project_b.path(), "ROUTING_POLICY_FOR_PROJECT_B"),
    ] {
        std::fs::create_dir_all(dir.join(".jcode")).expect("create .jcode");
        std::fs::write(dir.join(".jcode/swarm-prompt.md"), marker).expect("write swarm prompt");
    }

    let a = CommunicateTool::for_working_dir(Some(project_a.path()));
    let b = CommunicateTool::for_working_dir(Some(project_b.path()));

    assert!(a.description().contains("ROUTING_POLICY_FOR_PROJECT_A"));
    assert!(
        !a.description().contains("ROUTING_POLICY_FOR_PROJECT_B"),
        "project A must not see project B's routing policy"
    );
    assert!(b.description().contains("ROUTING_POLICY_FOR_PROJECT_B"));
    assert!(
        !b.description().contains("ROUTING_POLICY_FOR_PROJECT_A"),
        "project B must not see project A's routing policy"
    );

    match prev_home {
        Some(prev) => jcode_base::env::set_var("JCODE_HOME", prev),
...
fn existing_tool_keeps_prompt_while_new_tool_loads_edit() {
    ...
    let existing = CommunicateTool::for_working_dir(Some(project.path()));
    std::fs::write(&prompt_path, "second routing version").unwrap();
    let newly_created = CommunicateTool::for_working_dir(Some(project.path()));
...
#[test]
fn capacity_exhaustion_wave_is_detected_with_a_capacity_specific_fix() {
    // The Codex-exhaustion incident shape: workers die with a usage-limit 429
    // whose reset is far out, and no node has completed. Dispatching more
    // nodes only converts more of the plan into failures.
    let detail =
        "Rate limited: The usage limit has been reached. Plan: team. Resets in 4h 12m.".to_string();
    let members = vec![
        AgentInfo {
            provider_name: Some("openai".to_string()),
            ..credential_failed_worker("w1", &detail, 2)
        },
        AgentInfo {
            provider_name: Some("openai".to_string()),
            ..credential_failed_worker("w2", &detail, 4)
        },
    ];
    let wave = super::detect_credential_failure_wave(&members, "coord", 0, 60)
        .expect("two recent capacity failures with zero completions is a wave");
    assert_eq!(wave.session_ids, vec!["w1", "w2"]);

    let message = super::format_credential_failure_wave_error(&wave, 60);
    assert!(message.contains("paused dispatching"));
    assert!(message.contains("capacity/quota exhaustion"));
    assert!(message.contains("Resets in 4h 12m"));
    // Capacity is not fixed by re-authenticating, so do not send the user to
    // the login flow.
    assert!(!message.contains("jcode login"));
}

#[test]
fn mixed_wave_reports_the_credential_fix() {
    // An auth failure alongside a quota failure is still fixed by fixing auth.
    let members = vec![
        credential_failed_worker("w1", "Anthropic API error (401 Unauthorized)", 2),
        credential_failed_worker("w2", "Rate limited: usage limit reached.", 3),
    ];
    let wave = super::detect_credential_failure_wave(&members, "coord", 0, 60).expect("wave");
    let message = super::format_credential_failure_wave_error(&wave, 60);
    assert!(message.contains("credential/auth failures"));
    assert!(message.contains("jcode login"));
}
...
/// `jcode run` uses the non-streaming turn path. It used to fire `post_tool`
/// but never `turn_start`/`turn_end`, so an observer was told a tool ran inside
/// a turn that, as far as it could see, never began or ended. Every path that
/// runs a turn must fire the same lifecycle.
#[test]
fn every_turn_path_fires_the_turn_lifecycle_hooks() {
    let source = include_str!("../agent/turn_execution.rs");

    // The three entry points that run a model turn.
    for entry in [
        ...
        let start = source
            .find(entry)
            .unwrap_or_else(|| panic!("turn entry point missing: {entry}"));
        // Bound the search to this function: the next `\n    pub` at method
        // indentation, or end of file.
        let rest = &source[start + entry.len()..];
        let end = rest.find("\n    pub").unwrap_or(rest.len());
        let body = &rest[..end];

        assert!(
            body.contains("fire_turn_start_hook"),
            "{entry} must fire turn_start"
        );
        assert!(
            body.contains("fire_turn_end_hook"),
            "{entry} must fire turn_end"
        );
    }
}

#[test]
fn schema_advertises_the_plan_item_fields_that_make_work_parallelisable() {
    // PlanItem has carried `subsystem`, `file_scope`, and `blocked_by` since it
    // was written, and the TUI renders `blocked_by` in three places. The schema
    // declared plan_items as a bare object with `additionalProperties: true` and
    // named nothing, so the model was never told those fields exist.
    //
    // The measured result: zero plan items in live swarm state and zero uses of
    // propose_plan, against 22 swarm tool spans. A field a caller cannot see is
    // a field that is never set, and these two are the only machine-checkable
    // signal for whether work can actually run in parallel: `blocked_by` empty
    // means ready now, and disjoint `file_scope` means two items will not fight
    // over the same file.
    let schema = CommunicateTool::new().parameters_schema();
    let item = &schema["properties"]["plan_items"]["items"];
    let props = item["properties"]
        .as_object()
        .expect("plan_items.items should declare properties");

    for field in ["content", "status", "priority", "id"] {
        assert!(props.contains_key(field), "plan item must declare {field}");
    }
    for field in ["subsystem", "file_scope", "blocked_by", "assigned_to"] {
        assert!(
            props.contains_key(field),
            "plan item must declare {field}, or the model cannot populate it"
        );
    }

    assert_eq!(props["file_scope"]["type"], json!("array"));
    assert_eq!(props["blocked_by"]["type"], json!("array"));

    // The descriptions have to say what the fields are *for*. A named field with
    // an empty description is only marginally more discoverable than no field.
    let file_scope = props["file_scope"]["description"]
        .as_str()
        .expect("file_scope description");
    assert!(
        file_scope.contains("parallel") || file_scope.contains("disjoint"),
        "file_scope description should explain that disjoint scopes can run in parallel"
    );
    let blocked_by = props["blocked_by"]["description"]
        .as_str()
        .expect("blocked_by description");
    assert!(
        blocked_by.contains("ready") || blocked_by.contains("depend"),
        "blocked_by description should explain readiness"
    );
}

#[test]
fn a_propose_plan_call_carries_the_parallelism_fields_through_deserialization() {
    // The layer nothing covered. The schema test asserts only what the schema
    // *declares*; jcode-plan deserializes a PlanItem on its own; the swarm
    // persistence test covers PlanItem to disk. None of them exercise the path
    // an actual tool call takes: model JSON -> CommunicateInput -> Vec<PlanItem>.
    //
    // That path is where a mismatch would surface as silent data loss rather
    // than an error, because `plan_items` is `#[serde(default)]` on a struct
    // that tolerates unknown fields. A field the model sends but the struct
    // cannot place is dropped without complaint, so the call succeeds, the plan
    // is created, and the scopes that keep two writers off one file are simply
    // absent.
    let params: CommunicateInput = serde_json::from_value(json!({
        "action": "propose_plan",
        "plan_items": [
            {
                ...
                "priority": "high",
                "subsystem": "parsing",
                "file_scope": ["src/parse.rs", "src/lex.rs"],
                "blocked_by": []
            },
            {
                "id": "task-b",
                ...
                "status": "pending",
                "priority": "medium",
                "file_scope": ["src/render.rs"],
                "blocked_by": ["task-a"],
                "assigned_to": "session-abc"
            }
        ]
    }))
    .expect("a propose_plan call using the documented shape must deserialize");

    let items = params.plan_items.expect("plan_items must survive");
    assert_eq!(items.len(), 2);

    let a = &items[0];
    assert_eq!(a.subsystem.as_deref(), Some("parsing"));
    assert_eq!(a.file_scope, vec!["src/parse.rs", "src/lex.rs"]);
    assert!(a.blocked_by.is_empty(), "task-a is ready to start");

    let b = &items[1];
    assert_eq!(b.file_scope, vec!["src/render.rs"]);
    assert_eq!(b.blocked_by, vec!["task-a"]);
    assert_eq!(b.assigned_to.as_deref(), Some("session-abc"));
    // task-b omitted `subsystem` entirely, which must stay optional.
    assert!(b.subsystem.is_none());

    // The property these fields exist to express, checked on the values that
    // actually arrived rather than on the schema that described them.
    let overlap = a.file_scope.iter().any(|path| b.file_scope.contains(path));
    assert!(
        !overlap,
        "disjoint scopes must survive the tool boundary, or parallelism cannot be judged"
    );
}
