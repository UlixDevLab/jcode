// sonar-file-size: {"waive":350,"owner":"jcode-maintainers","ticket":"Jcode-control-plane-policy","baseline_loc":1011,"max_loc":1011,"expires":"2026-12-01","removal_trigger":"module reaches 350 LOC","reason":"Policy extraction keeps inherited module at or below mission-base LOC."}
...
use loop_breaker::{LoopGuardAction, ToolLoopBreaker};
...
pub struct Agent {
    ...
    /// Launcher capability from the most recent Subscribe for this session.
    rlm_client_identity: crate::rlm::ClientIdentity,
    /// First pilot failure permanently restores baseline context for this Agent.
    rlm_pilot_fallback_reason: Option<String>,
    ...
    /// Server-installed actor authority for structural review. This is
    /// transient and never persisted or surfaced to a model schema.
    structural_review_authority: Option<Arc<dyn jcode_tool_core::StructuralReviewAuthority>>,
...
impl Agent {
    ...
    fn build_base(
        ...
        let agent = Self {
            ...
            rlm_client_identity: crate::rlm::ClientIdentity::Ordinary,
            rlm_pilot_fallback_reason: None,
    ...
    pub fn set_rlm_client_identity(&mut self, identity: crate::rlm::ClientIdentity) {
        self.rlm_client_identity = identity;
    }

    pub fn rlm_client_identity(&self) -> crate::rlm::ClientIdentity {
        self.rlm_client_identity
    }

    pub fn rlm_requested_mode(&self) -> crate::config::RlmMode {
        crate::rlm::requested_mode_for(self.rlm_client_identity, crate::config::config().rlm.mode)
    }

    pub fn rlm_effective_mode(&self) -> crate::config::RlmMode {
        crate::rlm::effective_mode_for(self.rlm_client_identity, crate::config::config().rlm.mode)
    }

    pub fn rlm_context_mode_tag(&self) -> &'static str {
        crate::rlm::context_mode_tag_for(self.rlm_client_identity, crate::config::config().rlm.mode)
    }
    ...
    pub(crate) fn fire_session_lifecycle_hook(&self, event_name: &'static str, source: &str) {
        ...
        let event = crate::hooks::HookEvent::new(event_name)
            .session_id(self.session.id.clone())
            .field("SOURCE", source)
            .field("MODEL", self.provider_model());
        crate::hooks::dispatch_observer(self.with_lifecycle_hook_context(event, false));
    }
