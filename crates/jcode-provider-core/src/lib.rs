...
pub use fallback_pick::{
    ...
    error_looks_like_rate_limited, pick_next_fallback_route, pick_next_fallback_route_with_options,
};
...
pub trait Provider: Send + Sync {
    ...
    /// Send one request through the provider selected on this instance without
    /// allowing an orchestrator to switch providers or accounts after failure.
    ///
    /// Single-runtime providers already have no cross-provider failover, so the
    /// default delegates to [`Provider::complete`]. Multiplexing providers must
    /// override this and dispatch only to their currently selected runtime.
