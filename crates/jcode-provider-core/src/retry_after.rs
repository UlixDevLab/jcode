...
/// Build a hint from a delay a provider reported somewhere other than the
/// `Retry-After` header (for example a structured JSON reset window), capped
/// exactly like a header hint so an untrusted upstream cannot stall a turn.
pub fn retry_after_from_duration(delay: Duration) -> RetryAfter {
    RetryAfter::new(delay.min(MAX_RETRY_AFTER))
}
...
mod tests {
    ...
    #[test]
    fn duration_hint_is_capped_like_a_header_hint() {
        let hint = retry_after_from_duration(Duration::from_secs(30 * 86_400));
        assert!(hint.remaining() <= MAX_RETRY_AFTER);
    }

    #[test]
    fn short_duration_hint_is_preserved() {
        let hint = retry_after_from_duration(Duration::from_secs(12));
        assert!(hint.remaining() > Duration::from_secs(11));
        assert!(hint.remaining() <= Duration::from_secs(12));
    }
}
