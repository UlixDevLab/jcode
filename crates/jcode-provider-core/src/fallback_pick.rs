...
pub struct FallbackPickOptions {
    ...
    /// The failure was subscription/quota capacity exhaustion (a Codex or
    /// Claude usage limit, a spent credit balance, a plan quota). Capacity is
    /// metered per account, not per model, so every sibling model behind the
    /// same login is equally exhausted and is excluded exactly like a
    /// credential failure.
    pub capacity_exhausted: bool,
}

impl FallbackPickOptions {
    /// Whether this failure invalidates every route sharing the failed
    /// credential rather than only the failed model.
    fn excludes_whole_credential(self) -> bool {
        self.credential_failure || self.capacity_exhausted
    }
}

/// Whether `error` says the account's subscription or quota capacity is spent,
/// as opposed to a transient burst rate limit that clears on its own.
///
/// Deliberately narrow. Capacity is metered per account, so a match excludes
/// *every* route behind that login from the fallback offer. A plain "429 / rate
/// limit" does not qualify: those routinely clear within seconds, and treating
/// one as exhaustion would push the user off a provider that still works. Only
/// statements about a spent allowance count.
pub fn error_looks_like_capacity_exhaustion(error: &str) -> bool {
    ...
    markers.iter().any(|marker| lower.contains(marker))
        || crate::failover::contains_independent_status_code(&lower, "402")
}

/// Whether `error` is any rate-limit rejection, including a transient burst
/// limit that [`error_looks_like_capacity_exhaustion`] intentionally excludes.
///
/// A single one of these is not evidence of anything: it should be retried.
/// A *wave* of them across workers with nothing completing is, which is the
/// only place this belongs.
pub fn error_looks_like_rate_limited(error: &str) -> bool {
    let lower = error.to_ascii_lowercase();
    let markers = [
        "rate limit",
        "rate_limit",
    ...
    markers.iter().any(|marker| lower.contains(marker))
        || crate::failover::contains_independent_status_code(&lower, "429")
        || error_looks_like_capacity_exhaustion(error)
}
...
pub fn pick_next_fallback_route_with_options(
    ...
    routes
        ...
        .filter_map(|(index, route)| {
            ...
            // A credential failure or an exhausted account quota breaks every
            // route that authenticates with the same credential, not just the
            // failed model. Skip them all so the offer is a route that can
            // plausibly work.
            if options.excludes_whole_credential()
                && same_provider
...
mod tests {
    ...
    #[test]
    fn capacity_exhaustion_is_classified_and_transient_errors_are_not() {
        assert!(error_looks_like_capacity_exhaustion(
            "Rate limited: The usage limit has been reached. Plan: team. Resets in 30d 4h 29m."
        ));
        assert!(error_looks_like_capacity_exhaustion(
        ...
        assert!(!error_looks_like_capacity_exhaustion(
            "model version 4290 failed"
        ));
        // A bare burst limit is not exhaustion: it clears on its own, and
        // treating it as exhaustion would push the user off a provider that
        // still works.
        assert!(!error_looks_like_capacity_exhaustion(
    ...
    #[test]
    fn rate_limited_covers_bursts_and_exhaustion_but_not_unrelated_errors() {
        assert!(error_looks_like_rate_limited("429 Too Many Requests"));
        assert!(error_looks_like_rate_limited(
            "rate limit exceeded, please slow down"
        ));
        assert!(error_looks_like_rate_limited(
            "Rate limited: The usage limit has been reached."
        ));
        assert!(!error_looks_like_rate_limited("connection reset by peer"));
        assert!(!error_looks_like_rate_limited("model version 4290 failed"));
    }

    #[test]
    fn capacity_exhaustion_excludes_every_route_on_the_same_login() {
        let routes = vec![
            route("gpt-5.5", "OpenAI", "openai-oauth", true),
            // A sibling model on the same exhausted subscription is equally
            // out of capacity and must not be offered.
            route("gpt-5.5-codex", "OpenAI", "openai-oauth", true),
        ...
        .expect("fallback");
        assert_eq!(routes[pick].model, "claude-sonnet-5");
    }

    #[test]
    fn transient_rate_limit_still_allows_a_same_provider_sibling() {
        // Classified end to end: a burst 429 is not exhaustion, so the offer
        // keeps the user on the provider they chose.
        ...
        let options = FallbackPickOptions {
            capacity_exhausted: error_looks_like_capacity_exhaustion(error),
            credential_failure: error_looks_like_credential_failure(error),
        };
        let pick = pick_next_fallback_route_with_options(
            &routes,
            "gpt-5.5",
        ...
        .expect("pick");
        assert_eq!(routes[pick].model, "gpt-5.5-codex");
    }
