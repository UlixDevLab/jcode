...
// --- Anthropic OAuth subscription guard integration tests ---
//
// These tests exercise the four execution paths of the guard integration in
// `lib.rs` without making any network calls. The guard exposes test hooks
// (`set_test_guard_config_override`, `set_test_fetcher`) so admission
// decisions can be driven by deterministic config + a no-op fetcher. Clear
// the test override at the end of every test so other tests in the same
// process see a clean baseline.

struct GuardConfigOverride;

impl Drop for GuardConfigOverride {
    fn drop(&mut self) {
        let _ = jcode_base::usage::set_test_guard_config_override(None);
    }
}

fn install_guard_config(cfg: jcode_base::usage::GuardConfig) -> GuardConfigOverride {
    let _ = jcode_base::usage::set_test_guard_config_override(Some(std::sync::Arc::new(cfg)));
    GuardConfigOverride
}

#[tokio::test]
async fn guard_acquire_disabled_returns_none_without_network() {
    // With the guard disabled, every OAuth call must bypass the guard outright
    // (no OAuth usage endpoint hit, no fetch). Use a never-called fetcher to
    // prove the bypass holds: if the helper ever reached the fetch path the
    // test would still pass because the fetcher is a no-op, but the absence
    // of any guard denial is the load-bearing assertion.
    let _guard = jcode_base::storage::lock_test_env();
    let fetcher_calls = std::sync::Arc::new(std::sync::atomic::AtomicUsize::new(0));
    let counter = fetcher_calls.clone();
    let prev_fetcher =
        jcode_base::usage::set_test_fetcher(Some(std::sync::Arc::new(move |_token: &str| {
            counter.fetch_add(1, std::sync::atomic::Ordering::SeqCst);
            let snapshot = jcode_base::usage::UsageMeterSnapshot {
                five_hour_pct: 0.0,
                five_hour_resets_at: None,
            };
            std::pin::Pin::from(Box::pin(async move { Ok(snapshot) }))
        })));

    let _override = install_guard_config(jcode_base::usage::GuardConfig {
        enabled: false,
        threshold_pct: 5.0,
        max_preflight_pct: 95.0,
        fail_closed: true,
        allowed_models: vec!["claude-sonnet-5".to_string()],
    });

    let result = acquire_anthropic_lease(true, "any-oauth-token", "claude-sonnet-5", "call-1")
        .await
        .expect("disabled guard must not surface an error");
    assert!(
        result.is_none(),
        "disabled guard must bypass (returned Some(...) instead), got {result:?}"
    );
    assert_eq!(
        fetcher_calls.load(std::sync::atomic::Ordering::SeqCst),
        0,
        "disabled guard must never invoke the usage fetcher"
    );
    let _ = jcode_base::usage::set_test_fetcher(prev_fetcher);
}

#[tokio::test]
async fn guard_acquire_api_key_bypasses_without_any_guard_call() {
    // Even when the guard is enabled, an API-key call must not invoke the
    // OAuth guard. This is the regression bird flagged: the guard hits the
    ...
    let result = acquire_anthropic_lease(
        false,
        "sk-test-anthropic-apikey",
        "claude-sonnet-5",
        "call-2",
    )
    .await
    .expect("API-key path must not error");
    assert!(
        result.is_none(),
        "API-key calls must bypass the OAuth guard outright"
    );
    assert_eq!(
        fetcher_calls.load(std::sync::atomic::Ordering::SeqCst),
        0,
        "API-key path must never invoke the OAuth usage fetcher"
    );
    let _ = jcode_base::usage::set_test_fetcher(prev_fetcher);
}

#[tokio::test]
async fn guard_acquire_disallowed_oauth_model_returns_denied_error() {
    // OAuth guard enabled, but the requested model is not in the allowlist.
    // The provider must surface a clear error so orchestration can fall back
    // to a non-Anthropic provider. The error must contain the deny marker and
    ...
    let _override = install_guard_config(jcode_base::usage::GuardConfig {
        ...
        max_preflight_pct: 95.0,
        fail_closed: true,
        // Allowlist only Sonnet — Fable must be rejected.
        allowed_models: vec!["claude-sonnet-5".to_string()],
    });

    let bearer = "do-not-leak-test-token";
    let err = acquire_anthropic_lease(true, bearer, "claude-fable-5", "call-3")
        .await
        .expect_err("disallowed model must surface a denial error");
    let msg = format!("{err:#}");
    assert!(
        msg.contains(ANTHROPIC_GUARD_DENIED_MARKER),
        "denial must include the monitor denied marker, got {msg}"
    );
    assert!(
        msg.contains("claude-fable-5"),
        "denial must name the disallowed model, got {msg}"
    );
    assert!(
        msg.contains("non-Anthropic"),
        "denial must tell orchestration to fall back, got {msg}"
    );
    assert!(
        !msg.contains(bearer),
        "denial must NOT leak the bearer token, got {msg}"
    );
}

#[tokio::test]
async fn guard_disabled_allows_any_oauth_fallback_model() {
    // When the guard is disabled, every fallback model is allowed (no
    // restriction), so an admitted call can still route to any substitution
    // candidate if the server forces one.
    let _guard = jcode_base::storage::lock_test_env();
    let _override = install_guard_config(jcode_base::usage::GuardConfig {
        enabled: false,
        threshold_pct: 5.0,
    ...
    assert!(
        oauth_fallback_model_allowed("claude-fable-5"),
        "disabled guard must accept any fallback, including Fable"
    );
    assert!(
        oauth_fallback_model_allowed("claude-haiku-4-5"),
        "disabled guard must accept any fallback, including Haiku"
    );
}

#[tokio::test]
async fn guard_enabled_only_allows_fallback_models_on_allowlist() {
    // When the guard is enabled, only allowlisted models can be the target of
    // an internal fallback. Fable/Haiku must be rejected so an admitted
    // Sonnet/Opus call can never silently switch tier.
    let _guard = jcode_base::storage::lock_test_env();
    let _override = install_guard_config(jcode_base::usage::GuardConfig {
        enabled: true,
        threshold_pct: 5.0,
        max_preflight_pct: 95.0,
        fail_closed: true,
        allowed_models: vec!["claude-sonnet-5".to_string(), "claude-opus-5".to_string()],
    });

    assert!(
        oauth_fallback_model_allowed("claude-sonnet-5"),
        "allowlisted model must be allowed"
    );
    assert!(
        oauth_fallback_model_allowed("claude-opus-5"),
        "allowlisted model must be allowed"
    );
    assert!(
        !oauth_fallback_model_allowed("claude-fable-5"),
        "non-allowlisted Fable must be rejected"
    );
    assert!(
        !oauth_fallback_model_allowed("claude-haiku-4-5"),
        "non-allowlisted Haiku must be rejected"
    );
}
#[test]
