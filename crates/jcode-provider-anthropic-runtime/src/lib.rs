...
use jcode_base::usage::{
    AdmissionError, FinishOutcome, Lease, acquire_anthropic_subscription_guard,
...
impl Provider for AnthropicProvider {
    async fn complete(
        ...
        // DOD: OAuth acquire after final selected model resolution and before
        // any Messages transport. API-key callers and a disabled guard bypass
        // (the helper returns `Ok(None)` for both). Any denial surfaces as an
        // `Err` so orchestration can fall back to a non-Anthropic provider.
        let call_id = anthropic_guard_call_id(&self.oauth_session_id);
        let lease = acquire_anthropic_lease(is_oauth, &token, &api_model, &call_id).await?;
        ...
        jcode_base::logging::info(&format!(
            "Anthropic transport: HTTPS SSE stream (oauth={}, guard_lease_active={})",
            is_oauth,
            lease.is_some(),
        ));
        ...
        //
        // The guard lease (when held) is moved into the task so the final
        // post-call usage measurement always runs across every retry-loop
        // exit, including the early-return cases. The bearer token is never
        // logged; the final token returned by `run_stream_with_retries` is the
        // freshest one (initial or refreshed).
        let pre_acquire_token = token.clone();
        tokio::spawn(async move {
            if tx
                ...
                .is_err()
            {
                // The receiver dropped before we could even announce the
                // transport. Still finalize the lease so the OS lock is
                // released; the post-call meter can run against the initial
                // token since the retry loop never executed.
                finalize_anthropic_lease(lease, &pre_acquire_token, &tx).await;
                return;
            }
            let final_token = run_stream_with_retries(
                client,
                token,
                is_oauth,
                request,
    ...
    async fn complete_split(
        ...
        // Same lease policy as `complete`: OAuth acquire after final model
        // resolution and before transport; API-key and disabled guard bypass.
        let call_id = anthropic_guard_call_id(&self.oauth_session_id);
        let lease = acquire_anthropic_lease(is_oauth, &token, &api_model, &call_id).await?;
        ...
        jcode_base::logging::info(&format!(
            "Anthropic transport: HTTPS SSE split stream (oauth={}, guard_lease_active={})",
            is_oauth,
            lease.is_some(),
        ));
        ...
        // Spawn task to handle streaming with retry logic.
        //
        // The guard lease is moved into the task so the final post-call
        // measurement always runs across every retry-loop exit; the freshest
        // token (initial or refreshed) is what `lease.finish` uses for its
        // post-call meter fetch.
        let pre_acquire_token = token.clone();
        tokio::spawn(async move {
            if tx
                .send(Ok(StreamEvent::ConnectionType {
                ...
                .is_err()
            {
                // Receiver dropped before we could even announce the
                // transport. Still finalize the lease against the initial
                // token so the OS lock is released.
                finalize_anthropic_lease(lease, &pre_acquire_token, &tx).await;
                return;
            }
            let final_token = run_stream_with_retries(
...
/// Marker prefix for guard result errors so orchestration can match on the
/// category and route the call to a non-Anthropic provider.
const ANTHROPIC_GUARD_DENIED_MARKER: &str = "anthropic OAuth subscription monitor denied";

/// Format an [`AdmissionError`] from `acquire_anthropic_subscription_guard` into
/// a clear provider error that tells orchestration to use a non-Anthropic
/// fallback, without leaking the bearer token.
fn anthropic_guard_provider_error(err: &AdmissionError) -> anyhow::Error {
    anyhow::anyhow!(
        "{ANTHROPIC_GUARD_DENIED_MARKER} this OAuth Anthropic call ({err}); \
         orchestration should fall back to a non-Anthropic provider for this turn."
    )
}

/// Acquire the per-call OAuth subscription lease. Non-OAuth callers always
/// bypass the guard outright (returning `Ok(None)`) so an API-key call never
/// hits the OAuth usage endpoint. OAuth callers with the guard disabled also
/// bypass (`Err(AdmissionError::Disabled)`); any denial cause surfaces as a
/// clear provider error so orchestration can fall back to a non-Anthropic
/// provider.
async fn acquire_anthropic_lease(
    is_oauth: bool,
    token: &str,
    api_model: &str,
    call_id: &str,
    ...
    if !is_oauth {
        // API-key path: never call into the OAuth guard. The guard hits the
        // OAuth usage endpoint, which would 401/403 with an API-key bearer.
        return Ok(None);
    }
    match acquire_anthropic_subscription_guard(token, api_model, call_id).await {
        Ok(lease) => Ok(Some(lease)),
        Err(AdmissionError::Disabled) => Ok(None),
        Err(err) => {
            jcode_base::logging::warn(&format!(
                "Anthropic OAuth subscription monitor denied call for model '{}': {err}",
                api_model
            ));
            Err(anthropic_guard_provider_error(&err))
        }
    }
}

/// Predict whether an internal retry-loop fallback model is allowed by the
/// guard. When the guard is disabled every model is allowed; when enabled,
/// only allowlisted models are accepted so an admitted Sonnet/Opus call can
/// never silently switch to Fable/Haiku via fallback.
fn oauth_fallback_model_allowed(model: &str) -> bool {
    let cfg = resolve_runtime_config();
    if !cfg.enabled {
        return true;
    }
    cfg.model_allowed(model)
}

/// Finalize the OAuth subscription monitor lease after the retry loop returns.
/// This is the single finalization path used by every `run_stream_with_retries`
/// exit (success, retry exhaustion, model fallback refusal, guard fallback
/// denial, terminal failures). The post-call meter is fetched against the
/// final token, after any forced refresh that may have happened inside the
/// retry loop.
///
/// Kill and surface errors are persisted/logged. We deliberately do NOT send
/// `StreamEvent::Error` from the finalize path because the streamed response
/// already delivered content to the consumer; turning that into an error would
/// trigger a duplicate cross-provider failover for content the user has
/// already seen. Instead we emit a `StatusDetail` warning so the user notices
/// the guard tripped without the consumer misreading the result as a failure.
///
/// Admission denial before transport (handled in `complete` /
/// `complete_split`) remains an `Err` because nothing has been streamed yet,
/// so orchestration can safely fall back to a non-Anthropic provider.
///
/// The bearer token is never logged or returned.
async fn finalize_anthropic_lease(
    lease: Option<Lease>,
    final_token: &str,
    ...
    match lease.finish(final_token).await {
        Ok(FinishOutcome::WithinBudget) => {}
        Ok(FinishOutcome::UnverifiedMeter) => {
            jcode_base::logging::warn(
                "Anthropic OAuth subscription monitor: post-call usage meter could not be verified; telemetry warning recorded.",
            );
            let _ = tx
                .send(Ok(StreamEvent::StatusDetail {
                    detail: "⚠ Claude subscription monitor could not verify the post-call usage meter. The completed response is preserved; inspect `jcode claude status` before starting more premium work if usage looks suspicious.".to_string(),
                }))
                .await;
        }
        Ok(FinishOutcome::Killed) => {
            jcode_base::logging::warn(
                "Anthropic OAuth subscription monitor: suspicious five-hour movement detected; future Claude calls require operator approval.",
            );
            let _ = tx
                .send(Ok(StreamEvent::StatusDetail {
                    detail: "⚠ Claude subscription movement reached the configured warning threshold. This response is preserved. Ask the operator for permission before more Claude work, then run `jcode claude approve`.".to_string(),
                }))
                .await;
        }
        Err(err) => {
            jcode_base::logging::warn(&format!(
                "Anthropic OAuth subscription monitor finalization failed: {err}"
            ));
            let _ = tx
                .send(Ok(StreamEvent::StatusDetail {
                    detail: format!(
                        "⚠ Claude subscription monitor finalization failed: {err}. The completed response is preserved; use `jcode claude status` before additional Claude work."
                    ),
                }))
                .await;
        }
...
/// Build a unique, non-secret call id for the guard's in-flight marker.
fn anthropic_guard_call_id(session_id: &str) -> String {
    format!("anthropic-{}-{}", session_id, Uuid::new_v4().simple())
}
    ...
    for attempt in 0..MAX_RETRIES {
        ...
        .await
        {
            Ok(()) => {
                ...
                return token; // Success - return the final token for guard finalization
            }
            Err(e) => {
                ...
                //
                // When subscription monitoring is enabled, the fallback model
                // target must also be on the allowlist: an admitted Sonnet/Opus
                // call must never silently switch to Fable/Haiku via fallback.
                if is_model_not_found_error(&error_str)
                    && !saw_output
                    && let Some(fallback) = anthropic_fallback_model(&tried_models, &error_str)
                {
                    if is_oauth && !oauth_fallback_model_allowed(&fallback) {
                        jcode_base::logging::warn(&format!(
                            "Anthropic OAuth subscription monitor refused fallback '{}' for unavailable model '{}'; refusing to substitute.",
                            fallback, model_name
                        ));
                        let _ = tx
                            .send(Err(anyhow::anyhow!(
                                "{ANTHROPIC_GUARD_DENIED_MARKER} model '{}' is unavailable and the guard forbid fallback to '{}'; \
                                 orchestration should fall back to a non-Anthropic provider.",
                ...
                //
                // Substitute Opus only when the guard allowlist permits it; an
                // admitted Fable call must never silently switch to an
                // unallowlisted model.
                if is_oauth
                    && !saw_output
                ...
                {
                    if !oauth_fallback_model_allowed(&fallback) {
                        jcode_base::logging::warn(&format!(
                            "Anthropic OAuth subscription monitor refused fallback '{}' for exhausted Fable weekly quota; refusing to substitute.",
                            fallback
                        ));
                        let _ = tx
                            .send(Err(anyhow::anyhow!(
                                "{ANTHROPIC_GUARD_DENIED_MARKER} Fable weekly quota is exhausted and the guard forbid fallback to '{}'; \
                                 orchestration should fall back to a non-Anthropic provider.",
                                strip_1m_suffix(&fallback),
                            )))
                            .await;
