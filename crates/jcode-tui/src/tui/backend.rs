// sonar-file-size: waive=350 reason=pre-existing legacy transport module; this mission adds one bounded nonblocking catalog refresh helper and full decomposition is out of scope
...
impl RemoteConnection {
    ...
    pub async fn connect_with_session(
        ...
        conn.send_request(Request::Subscribe {
            ...
            rlm_client: Some(crate::rlm::launcher_identity().is_rlm()),
    ...
    /// Request both the fresh and route-expanded model catalogs without
    /// blocking the TUI. `/model` can paint the cached/names-only picker
    /// immediately and rebuild it in place when these responses arrive.
    pub fn refresh_model_catalog_detached(&mut self) {
        let refresh_id = self.next_request_id;
        self.next_request_id += 1;
        self.send_request_detached(Request::RefreshModels { id: refresh_id }, "refresh_models");

        let catalog_id = self.next_request_id;
        self.next_request_id += 1;
