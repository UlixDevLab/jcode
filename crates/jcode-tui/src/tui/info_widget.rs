// sonar-file-size: waive=350 reason=pre-existing legacy widget renderer; this mission only replaces the context row with RLM status for the opt-in launcher
...
pub struct UsageInfo {
    ...
    /// Privacy-safe identity for the account that supplied subscription limits.
    /// This distinguishes otherwise identical provider windows when multiple
    /// OAuth accounts are configured.
...
pub struct InfoWidgetData {
    ...
    /// Latest privacy-safe RLM selection snapshot for the opt-in launcher.
    pub rlm_status: Option<crate::rlm::RlmStatusSnapshot>,
...
fn render_context_widget(data: &InfoWidgetData, inner: Rect) -> Vec<Line<'static>> {
    if let Some(status) = data.rlm_status.as_ref() {
        return vec![render_rlm_status_line(status, inner.width)];
    }
...
pub(crate) fn rlm_status_text(status: &crate::rlm::RlmStatusSnapshot) -> String {
    if status.waiting {
        return format!("RLM {} · waiting for first call", status.mode);
    }
    if status.mode == "observe" {
        return format!(
            "RLM observe · {} msgs · ~{}t",
            status.provider_message_count, status.token_estimate
        );
    }
    if status.selected {
        let reduction = status
            .baseline_message_count
            .saturating_sub(status.provider_message_count)
            .saturating_mul(100)
            / status.baseline_message_count.max(1);
        return format!(
            "RLM pilot · {}/{} msgs · ↓{}% · ~{}t",
            status.provider_message_count,
            status.baseline_message_count,
            reduction,
            status.token_estimate
        );
    }
    if status.mode == "off" {
        return "RLM off · transcript context".to_string();
    }
    format!(
        "RLM pilot · baseline {} msgs · ~{}t",
        status.provider_message_count, status.token_estimate
    )
}

fn render_rlm_status_line(status: &crate::rlm::RlmStatusSnapshot, width: u16) -> Line<'static> {
    let text = rlm_status_text(status);
    Line::from(vec![
        Span::styled("RLM ", Style::default().fg(rgb(170, 130, 255)).bold()),
        Span::styled(
            truncate_smart(
                text.strip_prefix("RLM ").unwrap_or(&text),
