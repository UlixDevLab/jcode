        ...
        _ => {
            ...
            if !label.is_empty() {
                ...
                lines.push(Line::from(vec![Span::styled(
                    format!("{}{} limits", label, account_suffix),
                    Style::default()
                        .fg(rgb(140, 140, 150))
...
mod tests {
    ...
    #[test]
    fn openai_usage_header_identifies_the_active_account() {
        let info = UsageInfo {
            provider: UsageProvider::OpenAI,
            account_label: Some("d***v@gmail.com".to_string()),
            primary_limit_label: Some("Weekly".to_string()),
            seven_day: 0.03,
            available: true,
            ..Default::default()
        };

        let text = render_usage_compact(&info, 48, false)
            .iter()
            .map(line_text)
            .collect::<Vec<_>>()
            .join("\n");

        assert!(text.contains("OpenAI · d***v@gmail.com limits"), "{text}");
    }
}
