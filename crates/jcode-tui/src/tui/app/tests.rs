// sonar-file-size: {"waive":350,"owner":"@jcode-maintainers","ticket":"JCODE-QUALITY-RATCHET","baseline_loc":1819,"max_loc":1819,"expires":"2026-11-30","removal_trigger":"Extract focused remote History test fixtures","reason":"Inherited oversized file changed for the bounded session activity protocol"}
...
#[test]
fn active_skill_allowed_tools_narrow_the_tui_request_surface() {
    let mut app = create_test_app();
    let temp = tempfile::tempdir().expect("tempdir");
    let skill_dir = temp.path().join(".jcode/skills/narrow-tools");
    std::fs::create_dir_all(&skill_dir).expect("create skill dir");
    std::fs::write(
        skill_dir.join("SKILL.md"),
        "---\nname: narrow-tools\ndescription: Narrow schema regression skill\nallowed-tools: read, shell_exec\n---\nUse it.\n",
    )
    .expect("write skill");
    app.session.working_dir = Some(temp.path().to_string_lossy().to_string());
    app.active_skill = Some("narrow-tools".to_string());

    assert_eq!(
        app.active_skill_allowed_tools(),
        Some(HashSet::from(["bash".to_string(), "read".to_string()])),
        "the TUI must send only canonical schemas declared by the active skill"
    );
}

#[test]
fn active_skill_allowed_tools_retain_selfdev_for_tui_canaries() {
    let mut app = create_test_app();
    let temp = tempfile::tempdir().expect("tempdir");
    let skill_dir = temp.path().join(".jcode/skills/narrow-canary-tools");
    std::fs::create_dir_all(&skill_dir).expect("create skill dir");
    std::fs::write(
        skill_dir.join("SKILL.md"),
        "---\nname: narrow-canary-tools\ndescription: Narrow canary schema regression skill\nallowed-tools: read\n---\nUse it.\n",
    )
    .expect("write skill");
    app.session.working_dir = Some(temp.path().to_string_lossy().to_string());
    app.session.is_canary = true;
    app.active_skill = Some("narrow-canary-tools".to_string());

    assert_eq!(
        app.active_skill_allowed_tools(),
        Some(HashSet::from(["read".to_string(), "selfdev".to_string()])),
        "a scoped TUI canary must retain the selfdev control tool"
    );
}
