// sonar-file-size: waive=350 reason=pre-existing legacy input module; this mission adds only consent approve-deny handling and full decomposition is out of scope
use super::*;
...
async fn handle_remote_key_internal(
    ...
    let mut code = input::normalize_modified_shortcut_code(code, modifiers);
    ...
    if let Some(request) = app.pending_consent_request.take() {
        let response = match code {
            KeyCode::Char('y' | 'Y') => "approve",
            KeyCode::Char('n' | 'N') | KeyCode::Esc => "deny",
            _ => {
                app.pending_consent_request = Some(request);
                return Ok(());
            }
        };
        remote
            .send_stdin_response(&request.request_id, response)
            .await?;
        app.set_status_notice(if response == "approve" {
    ...
    if app_mod::hotkey_feedback::is_shortcut_help_hotkey(code, modifiers) {
        app.open_shortcut_help_panel();
        return Ok(());
    }
    ...
    match code {
        ...
        KeyCode::Enter => {
            ...
            if !app.input.is_empty() {
                ...
                if trimmed == "/update_lite" {
                    let session_id = app
                        .remote_session_id
                        .clone()
                        .unwrap_or_else(|| app.session.id.clone());
                    app.request_lite_update(session_id);
                    return Ok(());
                }
...
pub(in crate::tui::app) fn open_remote_model_picker_and_refresh(
    app: &mut App,
    remote: &mut RemoteConnection,
) {
    // Paint from the bootstrap/cache immediately. The old path awaited two
    // socket writes before opening the picker, so a busy reconnect writer left
    // the UI apparently frozen and prevented model selection during startup.
    app.open_model_picker();
    app.set_status_notice("Updating model routes…");
