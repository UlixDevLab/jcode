// sonar-file-size: waive=350 reason=pre-existing legacy event module; this mission adds only structured consent request handling and full decomposition is out of scope
use super::*;
...
const CONSENT_PROMPT_PREFIX: &str = "JCODE_CONSENT_V1";

fn parse_consent_request(prompt: &str) -> Option<(String, String, String)> {
    let mut lines = prompt.lines();
    (lines.next()? == CONSENT_PROMPT_PREFIX).then_some(())?;
    let mut tool = None;
    let mut action = None;
    let mut target = None;
    for line in lines {
    ...
    Some((tool?, action?, target?))
}

#[cfg(test)]
mod consent_prompt_tests {
    use super::parse_consent_request;

    #[test]
    fn consent_prefix_is_required_and_fields_are_extracted() {
        assert_eq!(
        ...
        assert!(parse_consent_request("Password:").is_none());
        assert!(parse_consent_request("JCODE_CONSENT_V1\ntool=browser").is_none());
    }
}
...
pub(in crate::tui::app) fn handle_server_event(
    ...
    match event {
        ...
        ServerEvent::StdinRequest {
            ...
            if let Some((tool, action, target)) = parse_consent_request(&prompt) {
                app.pending_consent_request = Some(app_mod::PendingConsentRequest {
                    request_id,
                    tool,
                    action,
                    target,
                });
                app.set_status_notice("Approval required for protected operation");
            } else {
