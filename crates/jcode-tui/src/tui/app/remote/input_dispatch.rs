// sonar-file-size: waive=350 reason=pre-existing remote input dispatch coordinator; this mission adds one focused user-image echo fix and decomposition is out of scope
...
fn rendered_user_input_images(
    ...
    images
        ...
        .map(|(media_type, data)| crate::session::RenderedImage {
            ...
            anchor: Some(crate::session::RenderedImageAnchor::UserPrompt {
                ordinal: prompt_ordinal,
            }),
        })
        .collect()
}
...
pub(in crate::tui::app) async fn submit_prepared_remote_input(
    ...
    let prompt_ordinal = app.display_user_message_count;
    let live_images = rendered_user_input_images(&prepared.images, prompt_ordinal);
    ...
    app.append_live_inline_images(live_images);
    let _ = app
