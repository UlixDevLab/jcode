...
// sonar-file-size: {"waive":350,"owner":"@jcode-maintainers","ticket":"jcode-auto-poke-finalization","baseline_loc":4128,"max_loc":4128,"expires":"2026-11-30","removal_trigger":"Extract completion scheduling from the input controller","reason":"Pre-existing input controller; the hard gate is kept in the existing turn-end scheduler."}
...
/// Remove sentinel-marked reasoning lines and normalize resulting whitespace.
pub(super) fn strip_reasoning_lines(content: &str) -> String {
    ...
    // Collapse blank runs from removed reasoning blocks.
...
/// Recognize only an exact pasted CLI resume invocation, never ordinary prose.
fn parse_inline_jcode_resume(input: &str) -> Option<&str> {
    let mut words = input.split_whitespace();
    (words.next()? == "jcode" && words.next()? == "--resume").then_some(())?;
    let session_id = words.next()?;
    (session_id.starts_with("session_") && words.next().is_none()).then_some(session_id)
}
...
// Allows large logs without multi-megabyte submit-path hangs.
...
/// Normalize Caps Lock for Ctrl/Command shortcuts but preserve Alt+Shift.
pub(super) fn normalize_modified_shortcut_code(code: KeyCode, modifiers: KeyModifiers) -> KeyCode {
...
mod tests {
    ...
    use super::{
        ...
        is_clipboard_paste_shortcut, normalize_modified_shortcut_code, parse_dropped_paths,
        parse_inline_jcode_resume, preferred_wayland_text_type, read_clipboard_for_paste_with,
        shifted_printable_fallback, text_input_for_key,
    };
    use crossterm::event::{KeyCode, KeyModifiers};

    #[test]
    fn inline_jcode_resume_requires_an_exact_session_invocation() {
        assert_eq!(
            parse_inline_jcode_resume("jcode --resume session_duck_123"),
            Some("session_duck_123")
        );
        assert_eq!(parse_inline_jcode_resume("jcode --resume"), None);
        assert_eq!(
            parse_inline_jcode_resume("please run jcode --resume session_duck_123"),
            None
        );
        assert_eq!(
            parse_inline_jcode_resume("jcode --resume session_duck_123 --fresh-spawn"),
            None
        );
    }
    ...
    #[test]
    fn modified_ascii_shortcuts_ignore_caps_lock_case() {
        for modifiers in [
            KeyModifiers::CONTROL,
            KeyModifiers::SUPER,
            KeyModifiers::META,
            ...
            assert_eq!(
                normalize_modified_shortcut_code(KeyCode::Char('V'), modifiers),
                KeyCode::Char('v'),
                "{modifiers:?}+V should dispatch like lowercase v"
            );
        }

        assert_eq!(
            normalize_modified_shortcut_code(KeyCode::Char('V'), KeyModifiers::ALT),
            KeyCode::Char('V'),
            "Alt+uppercase keeps its implicit-Shift meaning"
        );
    }

    #[test]
    fn command_modified_unicode_remains_layout_text() {
        for c in ['ї', 'Ї', 'є', 'Є'] {
            assert_eq!(
                text_input_for_key(KeyCode::Char(c), KeyModifiers::SUPER),
                Some(c.to_string()),
                "Command-modified layout output {c:?} should remain typeable"
            );
        }

        assert_eq!(
            text_input_for_key(KeyCode::Char('u'), KeyModifiers::SUPER),
            None,
            "ASCII Command chords must remain shortcuts"
        );
    }
...
fn modifiers_allow_printable_text(c: char, modifiers: KeyModifiers) -> bool {
    if modifiers.intersects(KeyModifiers::SUPER | KeyModifiers::HYPER | KeyModifiers::META) {
        // Custom macOS layouts can use Command as a character layer. Terminals
        // that preserve the final Unicode character should be allowed to type
        // it; ASCII Command chords remain reserved for shortcuts. If a terminal
        // reports only the physical ASCII key, there is no layout-safe character
        // to synthesize here.
        return is_layout_modified_text_char(c);
    }
...
pub(super) fn is_next_prompt_new_session_hotkey(code: KeyCode, modifiers: KeyModifiers) -> bool {
    // Space cannot be part of a simultaneous terminal chord because it is a key,
    // not a modifier. Use a deliberately uncommon single event instead so fast
    // typing cannot accidentally arm a new session.
    code == KeyCode::Char('n') && modifiers == (KeyModifiers::CONTROL | KeyModifiers::ALT)
}
...
impl App {
    ...
    fn handle_key_core(
        ...
        let mut code = normalize_modified_shortcut_code(code, modifiers);
        let mut modifiers = modifiers;
        ctrl_bracket_fallback_to_esc(&mut code, &mut modifiers);
        ...
        if super::hotkey_feedback::is_shortcut_help_hotkey(code, modifiers) {
            self.open_shortcut_help_panel();
            return Ok(());
        }
    ...
    pub(super) fn submit_input(&mut self) {
        ...
        if let Some(session_id) = parse_inline_jcode_resume(&raw_input) {
            ...
            self.set_status_notice(format!("Switching → {}", session_id));
            self.push_display_message(DisplayMessage::system(format!(
                "Switching this terminal to {}. Use /resume to browse sessions.",
                session_id
            )));
            return;
        }
