// sonar-file-size: waive=350 reason=pre-existing tool transcript compaction table; this change preserves one aggregate count through that compaction boundary and splitting every tool rule is out of scope
use super::*;

const AUTHORED_TOOL_ARGUMENT_CHARS_FIELD: &str = "_jcode_authored_argument_chars";

pub(crate) fn authored_tool_argument_chars(tool: &crate::message::ToolCall) -> usize {
    if let Some(chars) = tool
        .input
        .get(AUTHORED_TOOL_ARGUMENT_CHARS_FIELD)
        .and_then(serde_json::Value::as_u64)
    {
        return usize::try_from(chars).unwrap_or(usize::MAX);
    }
    let input_chars = serde_json::to_string(&tool.input)
        .map(|input| input.chars().count())
        .unwrap_or(0);
    tool.name.chars().count().saturating_add(input_chars)
}
...
pub(crate) fn compact_display_message_tool_data(message: &mut DisplayMessage) {
    ...
    let authored_chars = authored_tool_argument_chars(tool);
    let mut compacted = compact_tool_input_for_display(tool.name.as_str(), &tool.input);
    if let Some(fields) = compacted.as_object_mut() {
        fields.insert(
            AUTHORED_TOOL_ARGUMENT_CHARS_FIELD.to_string(),
            serde_json::Value::from(u64::try_from(authored_chars).unwrap_or(u64::MAX)),
        );
    }
...
mod tests {
    ...
    #[test]
    fn compaction_preserves_full_authored_tool_argument_character_count() {
        let mut message = tool_message(
            "write",
            serde_json::json!({
                "file_path": "demo.rs",
                "content": "fn main() { println!(\"full generated payload\"); }",
            }),
        );
        let expected = authored_tool_argument_chars(
            message
                .tool_data
                .as_ref()
                .expect("tool data before compaction"),
        );
        ...
        let tool = message
            .tool_data
            .as_ref()
            .expect("tool data after compaction");
        assert_eq!(authored_tool_argument_chars(tool), expected);
        assert_eq!(
            tool.input.get("file_path").and_then(|value| value.as_str()),
            Some("demo.rs")
        );
        assert!(tool.input.get("content").is_none());
    }
