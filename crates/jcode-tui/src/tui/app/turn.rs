...
impl App {
    /// Narrow the model-visible tool surface to an active skill's declared
    /// `allowed-tools`. Skills without a declaration retain the normal full
    /// surface. `all` and `*` are explicit escape hatches.
    pub(super) fn active_skill_allowed_tools(&self) -> Option<HashSet<String>> {
        let skill_name = self.active_skill.as_deref()?;
        let skills = self.current_skills_snapshot();
        let skill = skills.get(skill_name)?;
        let declared = skill.allowed_tools.as_ref()?;
        if declared.iter().any(|name| {
            let name = name.trim();
            name == "*" || name.eq_ignore_ascii_case("all")
        }) {
            return None;
        }
        let mut allowed: HashSet<String> = declared
            .iter()
            .map(|name| Registry::resolve_tool_name(name.trim()).to_string())
            .filter(|name| !name.is_empty())
            .collect();
        if self.session.is_canary {
            allowed.insert("selfdev".to_string());
        }
        Some(allowed)
    }
    ...
    pub(super) async fn run_turn_interactive(
        ...
        'turn_loop: loop {
            ...
            let allowed_tools = self.active_skill_allowed_tools();
            crate::tool::set_session_tool_policy(
                &self.session.id,
                allowed_tools.clone(),
                HashSet::new(),
            );
            let tools = self.registry.definitions(allowed_tools.as_ref()).await;
