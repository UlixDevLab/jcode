// sonar-file-size: {"waive":350,"owner":"@jcode-maintainers","ticket":"jcode-directory-account-usage","baseline_loc":2448,"max_loc":2448,"expires":"2026-11-30","removal_trigger":"Extract info-widget account usage assembly","reason":"Pre-existing TUI state module; resolves quota data through the session's existing directory account binding."}
use super::*;
...
impl App {
    ...
    /// Best-known provider/model identity for remote reasoning-effort UI.
    ...
    fn widget_usage_info(
        ...
        match route.provider {
            ...
            WidgetProviderKind::OpenAI => {
                ...
                let openai_usage = crate::usage::get_openai_usage_for_working_dir_sync(
                    self.session.working_dir.as_deref(),
                );
                Some(crate::tui::info_widget::UsageInfo {
                    provider: crate::tui::info_widget::UsageProvider::OpenAI,
                    account_label: openai_usage.account_label.clone(),
...
impl crate::tui::TuiState for App {
    ...
    fn info_widget_data(&self) -> crate::tui::info_widget::InfoWidgetData {
        ...
        let rlm_status = crate::rlm::launcher_identity().is_rlm().then(|| {
            session_id
                .and_then(crate::rlm::latest_status_snapshot)
                .unwrap_or_else(|| {
                    crate::rlm::RlmStatusSnapshot::waiting(
                        crate::config::config().rlm.mode.as_str(),
                    )
                })
        });
    ...
    fn consent_request(&self) -> Option<(&str, &str, &str)> {
        self.pending_consent_request.as_ref().map(|request| {
            (
                request.tool.as_str(),
