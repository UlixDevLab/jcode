// sonar-file-size: waive=350 reason=pre-existing split TUI test fixture; this mission updates one auto-poke default regression and further decomposition is out of scope
use super::*;
use crate::bus::{
...
/// Auto-poke is opt-in unless the operator enables it.
#[test]
fn auto_poke_defaults_off_when_config_does_not_enable_it() {
    with_temp_jcode_home(|| {
        crate::config::invalidate_config_cache();
        let app = create_test_app();
        assert!(
            !app.auto_poke_incomplete_todos,
            "auto-poke must stay off by default so the report remains visible"
        );
    });
}
