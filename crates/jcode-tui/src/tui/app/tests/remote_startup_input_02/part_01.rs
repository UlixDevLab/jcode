// sonar-file-size: waive=350 reason=pre-existing split TUI test fixture; this mission updates auto-poke default and toggle regressions and further decomposition is out of scope
#[test]
...
#[test]
fn test_remote_antigravity_names_only_fallback_preserves_antigravity_route() {
    // Oversized AvailableModelsUpdated frames can arrive with model names but
    // without route expansion. When Antigravity is the active remote provider,
    // its known models must not fall through to an unknown or Gemini route.
    with_temp_jcode_home(|| {
        let mut app = create_test_app();
        app.is_remote = true;
        app.remote_provider_name = Some("Antigravity".to_string());
        ...
        assert!(
            ...
            "Antigravity names-only fallback must preserve provider identity: {routes:?}"
        );
    });
}
