...
#[test]
fn pasted_cli_resume_queues_current_terminal_switch_without_an_agent_turn() {
    let runtime = tokio::runtime::Runtime::new().expect("test runtime");
    let _guard = runtime.enter();
    let mut app = create_test_app();

    app.input = "jcode --resume session_duck_123".to_string();
    app.submit_input();
...
fn test_test_command_queues_layered_verification_prompt() {
    ...
    assert!(queued.contains("Summarize the result truthfully"));
}
...
/// The picker must report the account the session actually bills. It used to
/// read the process-global active label, which for a directory-bound session
/// The picker must report the account the session actually bills. It used to
/// read the process-global active label, which for a directory-bound session
/// can name a different subscription than the one being charged.
#[test]
fn account_picker_reports_the_directory_bound_openai_account() {
    with_temp_jcode_home(|| {
        let home = std::path::PathBuf::from(
            std::env::var("JCODE_HOME").expect("temp home"),
        );
        let now_ms = chrono::Utc::now().timestamp_millis() + 60_000;
        for (label, email) in [
            ("seed-a", "company@example.com"),
            ("seed-b", "personal@example.com"),
        ] {
            crate::auth::codex::upsert_account(crate::auth::codex::OpenAiAccount {
                label: label.to_string(),
                access_token: format!("acc-{label}"),
                refresh_token: format!("ref-{label}"),
                id_token: None,
        ...
        let work_dir = home.join("company-work");
        std::fs::create_dir_all(&work_dir).expect("work dir");
        std::fs::write(
            home.join("config.toml"),
            format!(
                "[accounts.openai]\ndefault = \"personal@example.com\"\n\n\
                 [[accounts.openai.rules]]\npath = \"{}\"\naccount = \"company@example.com\"\n",
                work_dir.display()
            ),
        )
        .expect("write config");
        ...
        // Global active account is the personal one; the directory says company.
        crate::auth::codex::set_active_account("openai-2").expect("set active");

        let mut app = create_test_app();
        app.session.working_dir = Some(work_dir.display().to_string());

        assert_eq!(
            app.directory_bound_openai_account().as_deref(),
            Some("openai-1"),
            "picker must follow the directory binding, not the global active account"
        );
    });
}
