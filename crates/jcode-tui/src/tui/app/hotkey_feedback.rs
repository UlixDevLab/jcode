// sonar-file-size: waive=350 reason=pre-existing hotkey registry and feedback module; this change extends its existing help panel statistics and a registry extraction is out of scope
...
const SHORTCUT_HELP_PAGE_ID: &str = "shortcut-help";
...
pub(super) fn build_registry(inputs: &RegistryInputs<'_>) -> Vec<KnownHotkey> {
    ...
    out.push(KnownHotkey::quiet(
        ...
        "shortcut_help",
        "open shortcuts and session stats",
    ));
...
pub(super) fn is_shortcut_help_hotkey(code: KeyCode, modifiers: KeyModifiers) -> bool {
    matches!(code, KeyCode::Char('h' | 'H'))
        && modifiers.contains(KeyModifiers::CONTROL | KeyModifiers::ALT)
        && !modifiers.intersects(KeyModifiers::SUPER | KeyModifiers::META | KeyModifiers::HYPER)
}

#[derive(Debug, Clone, Copy, Default, PartialEq, Eq)]
struct ConversationActivityChars {
    user_prompts: usize,
    assistant_text: usize,
    tool_arguments: usize,
}

impl ConversationActivityChars {
    fn jcode_authored(self) -> usize {
        self.assistant_text.saturating_add(self.tool_arguments)
    }
}

fn conversation_activity_chars(
    messages: &[jcode_tui_messages::DisplayMessage],
) -> ConversationActivityChars {
    messages.iter().fold(
        ConversationActivityChars::default(),
        |mut totals, message| {
            match message.role.as_str() {
                "user" => {
                    totals.user_prompts = totals
                        .user_prompts
                        .saturating_add(message.content.chars().count());
                }
                "assistant" => {
                    totals.assistant_text = totals
                        .assistant_text
                        .saturating_add(message.content.chars().count());
                }
                "tool" => {
                    if let Some(tool) = message.tool_data.as_ref() {
                        totals.tool_arguments = totals.tool_arguments.saturating_add(
                            super::state_ui_storage::authored_tool_argument_chars(tool),
                        );
                    }
                }
                _ => {}
...
fn playful_output_comparison(session_id: &str, assistant_chars: usize) -> String {
    let references = [
        ("The Little Prince", 80_000usize),
        ("Hamlet", 180_000usize),
        ("The Hobbit", 500_000usize),
        ("War and Peace", 3_100_000usize),
    ];
    let index = session_id
        .bytes()
        .fold(0usize, |acc, byte| acc.wrapping_add(byte as usize))
        % references.len();
    let (title, estimated_chars) = references[index];
    let copies = assistant_chars as f64 / estimated_chars as f64;
    format!("roughly {copies:.2}× *{title}* by character count")
}
...
impl App {
    ...
    pub(super) fn open_shortcut_help_panel(&mut self) {
        let registry = self.hotkey_registry(self.is_remote);
        let usage = self.hotkey_usage.get_or_insert_with(load_state);
        let listing = render_hotkeys_listing(&registry, usage, now_unix());
        let activity = conversation_activity_chars(&self.display_messages);
        let user_chars = activity.user_prompts;
        let jcode_chars = activity.jcode_authored();
        let session_id = self
        ...
        let tokens = self
            .remote_total_tokens
            .map(|(input, output)| {
                let input_display = usize::try_from(input).unwrap_or(usize::MAX);
                let output_display = usize::try_from(output).unwrap_or(usize::MAX);
                let total_display =
                    usize::try_from(input.saturating_add(output)).unwrap_or(usize::MAX);
                format!(
                    "- **Session tokens:** {} input · {} output · {} total\n",
                    crate::util::format_number(input_display),
                    crate::util::format_number(output_display),
                    crate::util::format_number(total_display)
                )
            })
            .unwrap_or_else(|| {
                "- **Session tokens:** not reported by this runtime yet\n".to_string()
            });
        let shared_tokens = crate::provider_activity::token_usage_snapshot()
            .map(|usage| {
                let week_total = usage
                    .week_input_tokens
                    .saturating_add(usage.week_output_tokens);
                let all_time_total = usage
                    .all_time_input_tokens
                    .saturating_add(usage.all_time_output_tokens);
                format!(
                    "- **All Jcode this week:** {} input · {} output · {} total\n- **All Jcode recorded:** {} input · {} output · {} total\n",
                    crate::util::format_number(
                        usize::try_from(usage.week_input_tokens).unwrap_or(usize::MAX),
                    ),
                    crate::util::format_number(
                        usize::try_from(usage.week_output_tokens).unwrap_or(usize::MAX),
                    ),
                    crate::util::format_number(
                        usize::try_from(week_total).unwrap_or(usize::MAX),
                    ),
                    crate::util::format_number(
                        usize::try_from(usage.all_time_input_tokens).unwrap_or(usize::MAX),
                    ),
                    crate::util::format_number(
                        usize::try_from(usage.all_time_output_tokens).unwrap_or(usize::MAX),
                    ),
                    crate::util::format_number(
                        usize::try_from(all_time_total).unwrap_or(usize::MAX),
                    ),
                )
            })
            .unwrap_or_else(|| {
                "- **All Jcode usage:** recording starts with this build\n".to_string()
            });
        let ratio = if user_chars == 0 {
            "n/a".to_string()
        } else {
            format!("{:.1}:1", jcode_chars as f64 / user_chars as f64)
        };
        let comparison = playful_output_comparison(session_id, jcode_chars);
        let content = format!(
            "# Shortcuts & stats\n\nOpen or refresh this panel with `Ctrl+Option+H` (`Ctrl+Alt+H` in terminal notation).\n\n{listing}\n---\n\n## Session pulse\n\n{tokens}{shared_tokens}- **Your prompts:** {} characters\n- **Jcode authored:** {} characters\n- **Tool arguments included:** {} characters\n- **Jcode / you:** {ratio}\n- **Bookish scale:** {comparison}\n\n> Jcode-authored characters include assistant text plus serialized tool arguments such as code, patches, file contents, and shell commands. Tool results and opaque encrypted provider state are excluded.\n",
            crate::util::format_number(user_chars),
            crate::util::format_number(jcode_chars),
            crate::util::format_number(activity.tool_arguments),
        );

        let mut snapshot = self.side_panel.clone();
        snapshot
            .pages
            .retain(|page| page.id != SHORTCUT_HELP_PAGE_ID);
        snapshot.pages.push(crate::side_panel::SidePanelPage {
            id: SHORTCUT_HELP_PAGE_ID.to_string(),
            pdf_data: None,
            title: "Shortcuts & stats".to_string(),
            file_path: String::new(),
            format: crate::side_panel::SidePanelPageFormat::Markdown,
            source: crate::side_panel::SidePanelPageSource::Ephemeral,
            content,
            updated_at_ms: now_unix().saturating_mul(1_000),
        });
        snapshot.focused_page_id = Some(SHORTCUT_HELP_PAGE_ID.to_string());
        self.side_panel_user_hidden = false;
        self.side_panel_explicit_hidden = false;
        self.apply_side_panel_snapshot(snapshot);
        self.set_status_notice("Shortcuts & stats");
    }
