// sonar-file-size: waive=350 reason=pre-existing legacy remote integration test module; this mission adds one focused startup model-picker regression and test decomposition is out of scope
use super::reconnect;
use super::{
...
#[test]
fn submitted_remote_user_image_is_echoed_inline_immediately() {
    const PNG_1X1: &str = "iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAQAAAC1HAwCAAAAC0lEQVR42mP8/x8AAusB9Y9ZlVQAAAAASUVORK5CYII=";
    let mut app = create_test_app();
    app.is_remote = true;
    app.runtime_mode = crate::tui::app::AppRuntimeMode::RemoteClient;
    app.pin_images = true;
    app.inline_images_visible = true;
    app.side_panel_user_hidden = false;
    ...
    rt.block_on(crate::tui::app::remote::submit_prepared_remote_input(
        ...
        crate::tui::app::input::PreparedInput {
            raw_input: "[image 1] describe this".to_string(),
            expanded: "[image 1] describe this".to_string(),
            images: vec![("image/png".to_string(), PNG_1X1.to_string())],
        },
    ))
    ...
    assert_eq!(
        ...
        1,
        "a submitted image must render with the locally echoed user prompt instead of waiting for a reconnect/history refresh"
    );
    let image = &app.remote_side_pane_images[0];
    assert_eq!(image.media_type, "image/png");
    assert_eq!(image.data, PNG_1X1);
    assert_eq!(image.source, crate::session::RenderedImageSource::UserInput);
    assert_eq!(
        image.anchor,
    ...
    terminal
        ...
        .expect("draw submitted image frame");
    ...
    let rendered = (0..area.height)
        ...
        .join("\n");
    assert!(
        rendered.contains("image/png") && rendered.contains("[I]"),
        "the locally echoed image label and visibility control must reach the real TUI frame:\n{rendered}"
    );
}
...
#[test]
fn remote_model_picker_opens_before_catalog_refresh_completes() {
    let rt = tokio::runtime::Runtime::new().expect("runtime");
    let _guard = rt.enter();
    let mut app = create_test_app();
    app.is_remote = true;
    app.remote_provider_name = Some("OpenAI".to_string());
    app.remote_provider_model = Some("gpt-current".to_string());
    let mut remote = crate::tui::backend::RemoteConnection::dummy();

    super::key_handling::open_remote_model_picker_and_refresh(&mut app, &mut remote);

    assert!(
        app.inline_interactive_state.is_some(),
        "the cached picker must be interactive before catalog responses arrive"
    );
    assert_eq!(
        app.status_notice.as_ref().map(|(text, _)| text.as_str()),
        Some("Updating model routes…")
    );
}
