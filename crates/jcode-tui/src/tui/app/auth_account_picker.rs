...
impl App {
    ...
    /// The OpenAI account this session actually bills, when a
    /// `[accounts.openai]` rule binds its working directory.
    ///
    /// The pickers otherwise report `active_account_label()`, which is
    /// process-global and can disagree with a directory-pinned session: it
    /// would mark `openai-2` active while the session really spends
    /// `openai-1`, which is precisely the confusion the binding exists to
    /// remove. `None` means no rule applies and the global label is correct.
    pub(crate) fn directory_bound_openai_account(&self) -> Option<String> {
        let dir = self.session.working_dir.clone().or_else(|| {
            std::env::current_dir()
                .ok()
                .map(|dir| dir.display().to_string())
        })?;
        crate::auth::account_binding::resolve_label_for_dir(
            "openai",
            &dir,
            &crate::auth::codex::account_identities(),
        )
    }
    ...
    fn build_all_inline_account_picker(&self) -> (Vec<crate::tui::PickerEntry>, usize) {
        ...
        let openai_bound = self.directory_bound_openai_account();
        let openai_active = openai_bound.clone().unwrap_or(openai_active);
        ...
        for account in &openai_accounts {
            ...
            models.push(crate::tui::PickerEntry {
                ...
                options: vec![crate::tui::PickerOption {
                    ...
                    available: true,
                    detail: if openai_bound.as_deref() == Some(account.label.as_str()) {
                        // Say *why* it is active, so a manual switch that is
                        // re-pinned on the next session is not baffling.
                        format!("{} - {} - bound to this directory", email, status)
                    } else {
                        format!("{} - {} - acct {}", email, status, account_id)
                    },
                    estimated_reference_cost_micros: None,
                }],
                action: crate::tui::PickerAction::Account(
    ...
    fn build_openai_inline_account_picker(&self) -> (Vec<crate::tui::PickerEntry>, usize) {
        ...
        let bound_label = self.directory_bound_openai_account();
        let active_label = bound_label.clone().unwrap_or_else(|| {
        ...
        for (index, account) in accounts.iter().enumerate() {
            ...
            models.push(crate::tui::PickerEntry {
                ...
                options: vec![crate::tui::PickerOption {
                    ...
                    available: true,
                    detail: if bound_label.as_deref() == Some(account.label.as_str()) {
                        format!("{} - {} - bound to this directory", email, status)
                    } else {
                        format!("{} - {} - acct {}", email, status, account_id)
                    },
