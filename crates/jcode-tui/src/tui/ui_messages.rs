...
fn todo_goal_assessment_spans(goal: &crate::todo::TodoGoal) -> Vec<Span<'static>> {
    ...
    if let Some(state) = goal.closed_feedback_loop {
        ...
        states.push(("Closed feedback loop", state.as_str().to_string(), color));
    }
    if let Some(state) = goal.feedback_loop_relevance {
        ...
        states.push(("Relevance", state.as_str().to_string(), color));
    }
    if let Some(state) = goal.feedback_loop_coverage {
        ...
        states.push(("Coverage", state.as_str().to_string(), color));
    }
    if let Some(state) = goal.feedback_loop_traceability {
        ...
        states.push(("Traceability", state.as_str().to_string(), color));
    }
    ...
    // Delivery is progress toward the outcome, not an approval decision.
...
fn push_todo_goal_details(
    ...
    let scores = todo_goal_assessment_spans(goal);
    if !scores.is_empty() {
        ...
        let score_count = usize::from(goal.closed_feedback_loop.is_some())
            ...
            + usize::from(goal.feedback_loop_traceability.is_some())
        ...
        if score_width > inner_width.saturating_sub(2) && score_count > 1 {
            ...
            if let Some(state) = goal.feedback_loop_traceability {
                states.push(("Traceability", state.as_str().to_string()));
            }
            if let Some(state) = goal.delivery_state {
...
/// intent change first, then any per-goal assessment updates.
