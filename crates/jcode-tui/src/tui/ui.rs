...
// sonar-file-size: waive=350 reason=pre-existing legacy renderer; this mission adds only consent overlay dispatch and full decomposition is out of scope
...
fn draw_inner(frame: &mut Frame, app: &dyn TuiState) {
    ...
    if let Some((tool, action, target)) = app.consent_request() {
        overlays::draw_consent_overlay(frame, area, tool, action, target);
