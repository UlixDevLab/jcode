...
fn idle_donut_active_with_policy(
    ...
    // The donut is an empty-screen decoration, not composer chrome. Keeping its
    // 14-row reservation while a draft or inline picker is visible over-constrains
    // short terminals (an 84x24 `/model` picker can be reduced to no useful rows)
    // and leaves animation-only repaints competing with typed text. Yield the
    // screen as soon as the user starts composing or an inline workflow opens.
    if !state.input().is_empty() || state.inline_ui_state().is_some() {
        return false;
    }
