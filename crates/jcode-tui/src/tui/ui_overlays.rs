// sonar-file-size: waive=350 reason=pre-existing legacy overlay module; this mission adds only the consent popup renderer and full decomposition is out of scope
use super::{
...
pub(super) fn draw_consent_overlay(
    frame: &mut Frame,
    area: Rect,
    tool: &str,
    action: &str,
    ...
    let height = area.height.min(11).max(1);
    let popup = Rect {
    ...
    let block = Block::default()
        .title(Span::styled(
            " Protected operation approval ",
            Style::default()
                .fg(Color::Yellow)
                .add_modifier(Modifier::BOLD),
        ))
        .title_bottom(Line::from(Span::styled(
            " y approve once · n / Esc deny ",
    ...
    let text = vec![
        Line::from(Span::styled(
            "Jcode needs your approval before this live operation.",
        ...
        Line::from(""),
        Line::from("This approval applies only to the current call."),
    ];
    frame.render_widget(block, popup);
    frame.render_widget(
        Paragraph::new(text).wrap(ratatui::widgets::Wrap { trim: true }),
        Block::default().inner(popup),
    );
}
