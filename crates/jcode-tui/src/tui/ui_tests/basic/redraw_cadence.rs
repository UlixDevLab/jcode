...
/// A draft left sitting in the composer is no longer an idle screen. Keeping
/// the donut alive here wastes most of a short terminal and lets animation-only
/// repaints compete with the cursor and typed text.
#[test]
fn a_paused_draft_hides_the_decorative_animation() {
    let _idle_animation = IdleAnimationEnvGuard::enable();
    crate::perf::pin_full_profile_for_tests();
    let policy = full_tier_policy();
    ...
    assert!(
        !crate::tui::idle_donut_active(&paused),
        "a non-empty composer must hide the idle decoration"
    );
    assert!(
        interval >= crate::tui::REDRAW_IDLE,
        "a paused draft must not retain animation cadence (got {interval:?})"
    );
}

/// The model picker owns the vertical space above the composer. On an 84x24
/// terminal its 20-row budget plus the old 14-row donut reservation was
/// over-constrained, leaving the picker state alive but its rows effectively
/// invisible. Any inline UI must therefore displace the decorative animation.
#[test]
fn an_inline_model_picker_hides_the_decorative_animation() {
    let _idle_animation = IdleAnimationEnvGuard::enable();
    crate::perf::pin_full_profile_for_tests();
    let picker = TestState {
    ...
    assert!(
        !crate::tui::idle_donut_active(&picker),
        "an inline picker must own the screen without the idle decoration"
    );
}
