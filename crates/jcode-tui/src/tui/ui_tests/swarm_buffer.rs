...
/// The account badge answers "whose subscription is this spending?". Without
/// it the provider line reads just "OpenAI", which is identical for the
/// company and personal subscriptions, making cross-billing invisible.
#[test]
fn right_fact_stack_names_the_billed_subscription() {
    let _lock = viewport_snapshot_test_lock();
    clear_flicker_frame_history_for_tests();
    let state = fact_test_state(String::new(), true);
    let backend = TestBackend::new(120, 18);
    ...
    terminal
        .draw(|frame| crate::tui::ui::draw(frame, &state))
        .expect("account badge frame");

    let rows = buffer_rows(&terminal);
    let y = row_containing(&rows, "OpenAI · OAuth");
    // Rendered against the developer's real account store, so assert on the
    // shape (provider qualified by an account) rather than a specific email.
    let row = &rows[y];
    let tail = row
        .split("OpenAI · OAuth")
        .nth(1)
        .expect("text after the provider/auth pair");
    assert!(
        tail.trim_end().starts_with(" · "),
        "provider line should be followed by the billed account, got {row:?}"
    );
}
