// sonar-file-size: waive=350 reason=pre-existing legacy input renderer; this mission only swaps one context fact row for compact RLM diagnostics
...
mod tests {
    ...
    #[test]
    fn rlm_status_shows_selected_context_reduction() {
        let status = crate::rlm::RlmStatusSnapshot {
            mode: "pilot".to_string(),
            provider_message_count: 14,
            baseline_message_count: 26,
            token_estimate: 962,
            selected: true,
            waiting: false,
        };
        assert_eq!(
            crate::tui::info_widget::rlm_status_text(&status),
            "RLM pilot · 14/26 msgs · ↓46% · ~962t"
        );
    }

    #[test]
    fn rlm_status_distinguishes_waiting_and_baseline() {
        assert_eq!(
            crate::tui::info_widget::rlm_status_text(&crate::rlm::RlmStatusSnapshot::waiting(
                "pilot"
            )),
            "RLM pilot · waiting for first call"
        );
        let baseline = crate::rlm::RlmStatusSnapshot {
            mode: "pilot".to_string(),
            provider_message_count: 26,
            baseline_message_count: 26,
            token_estimate: 8_100,
            selected: false,
            waiting: false,
        };
        assert_eq!(
            crate::tui::info_widget::rlm_status_text(&baseline),
            "RLM pilot · baseline 26 msgs · ~8100t"
        );
    }
...
pub(super) fn draw_overscroll_status(frame: &mut Frame, app: &dyn TuiState, area: Rect) {
    ...
    // Subscription account, when more than one is logged in and a
    // per-directory binding decides which one bills. Sits next to the provider
    // because it qualifies it: "openai" alone does not say whose quota is being
    // spent, and confusing company with personal spend is the failure this is
    // meant to make impossible to miss.
    if provider.to_ascii_lowercase().contains("openai")
        && let Some(dir) = data.working_dir.clone().or_else(|| {
            std::env::current_dir()
                .ok()
                .map(|d| d.display().to_string())
        })
        && let Some(account) = crate::auth::account_binding::bound_account_display("openai", &dir)
    {
        if !spans.is_empty() {
            spans.push(sep());
        }
        spans.push(Span::styled(
            account,
            Style::default().fg(rgb(200, 160, 255)),
        ));
    }
    ...
    if let Some(status) = data.rlm_status.as_ref() {
        let text = super::info_widget::rlm_status_text(status);
        if !spans.is_empty() {
            spans.push(sep());
        }
        spans.push(Span::styled("RLM ", Style::default().fg(ai_color()).bold()));
        spans.push(Span::styled(
            text.strip_prefix("RLM ").unwrap_or(&text).to_string(),
            Style::default().fg(rgb(140, 140, 150)),
        ));
    } else if let Some((used, limit)) = overscroll_context_usage(&data) {
        if !spans.is_empty() {
            spans.push(sep());
        }
        spans.push(Span::styled(
...
fn right_fact_lines(app: &dyn TuiState) -> Vec<RightFactLine> {
    ...
    // Which subscription this session bills, when more than one is logged in.
    // Qualifies the provider: "OpenAI" alone does not say whose quota is being
    // spent, and silently mixing company and personal spend is exactly the
    // failure the per-directory binding exists to prevent.
    if provider.to_ascii_lowercase().contains("openai")
        && let Some(dir) = app.working_dir().or_else(|| {
            // Local sessions do not always record a working dir; fall back to
            // the process cwd so the badge cannot silently vanish exactly when
            // the user needs it.
            std::env::current_dir()
                .ok()
        ...
        && let Some(account) = crate::auth::account_binding::bound_account_display("openai", &dir)
    {
        ...
        access.push(Span::styled(account, right_fact_neutral_style()));
    }
    ...
    if let Some(status) = data.rlm_status.as_ref() {
        let text = super::info_widget::rlm_status_text(status);
        let spans = vec![
            Span::styled("RLM ", Style::default().fg(ai_color()).bold()),
            Span::styled(
                text.strip_prefix("RLM ").unwrap_or(&text).to_string(),
                right_fact_neutral_style(),
            ),
