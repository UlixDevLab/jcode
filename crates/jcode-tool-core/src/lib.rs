...
/// A candidate implementation state bound to one Todo cycle.
///
/// The native structural-review tool derives this from the local candidate.
/// It is never decoded from the model request, a prompt token, or a socket
/// session id.
#[derive(Clone, Debug, PartialEq, Eq, Serialize, Deserialize)]
pub struct StructuralReviewCandidate {
...
#[derive(Clone, Debug, PartialEq, Eq, Serialize, Deserialize)]
pub struct StructuralReviewOpen {
    pub owner_session_id: String,
    pub reviewer_session_id: String,
    pub candidate: StructuralReviewCandidate,
}

#[derive(Clone, Copy, Debug, PartialEq, Eq, Serialize, Deserialize)]
#[serde(rename_all = "SCREAMING_SNAKE_CASE")]
pub enum StructuralReviewDisposition {
    Cohesive,
    RefactorRequired,
}

#[derive(Clone, Debug, PartialEq, Eq, Serialize, Deserialize)]
pub struct StructuralReviewSubmission {
    pub request_id: String,
    pub disposition: StructuralReviewDisposition,
    pub refactor_obligations: Vec<String>,
}

#[derive(Clone, Debug, PartialEq, Eq, Serialize, Deserialize)]
pub struct StructuralReviewReceipt {
    pub request_id: String,
    pub owner_session_id: String,
    pub reviewer_session_id: String,
    pub candidate: StructuralReviewCandidate,
...
/// Server-installed, actor-bound structural-review authority.
///
/// Implementations derive the actor from the live runtime when this capability
/// is installed. Tool arguments deliberately contain neither actor identity nor
/// a role claim, so model input cannot self-authorize a verdict.
#[async_trait::async_trait]
pub trait StructuralReviewAuthority: Send + Sync {
    async fn open_structural_review(
        &self,
        request: StructuralReviewOpen,
    ) -> anyhow::Result<StructuralReviewReceipt>;
...
pub struct ToolContext {
    ...
    /// Opaque runtime capability. It is not serialized, described in a tool
    /// schema, or accepted from model input.
    pub structural_review_authority: Option<Arc<dyn StructuralReviewAuthority>>,
...
/// A user-visible description of one operation that needs current-user consent.
///
/// Tool implementations must construct this from safe metadata only. In
/// particular, it must not include arguments, commands, headers, environment
/// values, or credential material.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct NativeConsentRequirement {
    pub tool: String,
    pub action: String,
    pub target_summary: String,
}

impl NativeConsentRequirement {
    pub fn new(
...
pub trait Tool: Send + Sync {
    ...
    /// Describe an operation that needs native current-user consent.
    ///
    /// The default is deliberately safe for existing and dynamically loaded
    /// tools: no consent is requested until a tool explicitly classifies one of
    /// its calls as protected.
