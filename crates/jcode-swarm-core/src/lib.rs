...
pub struct SwarmMemberRecord {
    ...
    /// Model this member actually ran, captured from its live runtime.
    ///
    /// The coordinator chooses a model per worker, but nothing recorded which
    /// one was used, so a completed swarm could not be audited: every member
    /// read as "inherited" once its session ended. That makes the routing
    /// rules in `swarm-prompt.md` unenforceable after the fact, and the
    /// specific failure it hides is the expensive one, a broad or ambiguous
    /// task dispatched to a model meant only for mechanical work.
    ///
    /// `None` means "not observed yet" (the worker had not reported a turn),
    /// never "inherited from the parent". Absent rather than guessed, so a
    /// missing value is not mistaken for a routing claim.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub model: Option<String>,
    /// Provider that served this member, e.g. `openai` or `anthropic`.
    ///
    /// A model name alone cannot distinguish a subscription route from a
    /// metered API route, which is the distinction that carries billing
    /// consequences.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub provider: Option<String>,
    /// Reasoning effort this member ran at.
    ///
    /// Effort is a first-order cost multiplier, so a correct model at the
    /// wrong effort is still a routing error worth seeing.
    #[serde(default, skip_serializing_if = "Option::is_none")]
...
/// Idempotency marker for [`append_worker_budget_notice`].
pub const SWARM_TOKEN_BUDGET_MARKER: &str = "SWARM TOKEN BUDGET";

/// Tell a worker the spend budget it is running under.
///
/// The budget is enforced server-side and kills the turn when exceeded, but
/// nothing ever told the worker it existed. A worker that cannot see the limit
/// cannot pace itself: it gets cut off mid-thought, and its node fails with no
/// partial result. Naming the budget lets it scope the work, and report
/// something useful if it is running out.
///
/// `limit` is in *new* tokens (output plus freshly-read input, cache hits
/// excluded), matching what the server actually charges. `None` or zero means
/// uncapped, in which case no notice is added. Idempotent via
/// [`SWARM_TOKEN_BUDGET_MARKER`].
pub fn append_worker_budget_notice(message: &str, limit: Option<u64>) -> String {
    let Some(limit) = limit.filter(|limit| *limit > 0) else {
        return message.to_string();
    };
    if message.contains(SWARM_TOKEN_BUDGET_MARKER) {
        return message.to_string();
    }
    ...
    out.push_str("<system-reminder>\n");
    out.push_str(SWARM_TOKEN_BUDGET_MARKER);
    out.push_str(&format!(
        "\nThis session is capped at about {millions:.1}M new tokens (your output plus input \
read fresh; cached context re-reads are not charged, so re-reading the same files is cheap \
while generating new output is not). Exceeding it ends your turn immediately and your work \
is lost. Scope accordingly: prefer targeted reads over dumping whole trees, do not re-derive \
what you already established, and if you find yourself approaching the limit, stop and submit \
your completion report with what you have plus what remains, rather than being cut off \
mid-task.\n"
    ));
    out.push_str("</system-reminder>");
    out
...
#[cfg(test)]
mod worker_budget_notice_tests {
    use super::*;

    #[test]
    fn notice_states_the_limit_and_is_idempotent() {
        let prompt = "Review the diff.";
        let once = append_worker_budget_notice(prompt, Some(5_000_000));
        assert!(once.starts_with(prompt));
        assert!(once.contains(SWARM_TOKEN_BUDGET_MARKER));
        assert!(once.contains("5.0M new tokens"));
        // The worker must be told cached re-reads are cheap, otherwise the
        // notice would discourage exactly the behaviour we want.
        assert!(once.contains("cached context re-reads are not charged"));
        assert_eq!(append_worker_budget_notice(&once, Some(5_000_000)), once);
    }

    #[test]
    fn uncapped_workers_get_no_notice() {
        // MiniMax routes resolve to a 0 cap; inventing a limit for them would
        // make the worker throttle itself for no reason.
        let prompt = "Do the work.";
        assert_eq!(append_worker_budget_notice(prompt, Some(0)), prompt);
        assert_eq!(append_worker_budget_notice(prompt, None), prompt);
    }

    #[test]
    fn notice_composes_after_the_completion_report_contract() {
        let with_report = append_swarm_completion_report_instructions("Task.");
        let both = append_worker_budget_notice(&with_report, Some(5_000_000));
        assert!(both.contains(SWARM_COMPLETION_REPORT_MARKER));
        assert!(both.contains(SWARM_TOKEN_BUDGET_MARKER));
    }
}
