// sonar-file-size: waive=350 reason=pre-existing compaction core module; this mission adds one provider-accounting helper beside the existing context heuristic and decomposition is out of scope
...
pub fn effective_context_tokens_from_usage(
    ...
    effective_billed_input_tokens_from_usage(
        provider_name,
        input_tokens,
...
/// Provider-normalized billed input tokens from one usage report.
///
/// Unlike [`effective_context_tokens_from_usage`], cache-only Anthropic-style
/// reports remain non-zero. Use this for telemetry and usage budgets, not for
/// deciding whether a provider supplied a usable context-size observation.
pub fn effective_billed_input_tokens_from_usage(
    provider_name: &str,
    input_tokens: u64,
    cache_read_input_tokens: Option<u64>,
    cache_creation_input_tokens: Option<u64>,
...
mod tests {
    ...
    #[test]
    fn effective_billed_input_keeps_cache_only_split_usage() {
        assert_eq!(
            effective_billed_input_tokens_from_usage("anthropic", 0, Some(300_000), Some(5_000),),
            305_000
        );
        assert_eq!(
            effective_billed_input_tokens_from_usage("openai", 400_000, Some(390_000), None),
            400_000
        );
    }
