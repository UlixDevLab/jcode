// sonar-file-size: {"waive":350,"owner":"jcode-maintainers","ticket":"Jcode-control-plane-policy","baseline_loc":1623,"max_loc":1623,"expires":"2026-12-01","removal_trigger":"module reaches 350 LOC","reason":"Policy extraction keeps inherited module at or below mission-base LOC."}
use serde::{Deserialize, Serialize};
pub mod accounts;
pub use accounts::{
    AccountResolution, AccountRule, AccountsConfig, ProviderAccountsConfig, resolve_account_for_dir,
};
mod display;
pub use display::DisplayConfig;
mod consent;
pub use consent::{ConsentConfig, ConsentNavigationPolicy};
pub mod keybindings;
mod rlm;
mod serde_lenient;
mod tool_policy;
...
pub use rlm::{RlmConfig, RlmManifestRetention, RlmMode};
pub use tool_policy::SessionToolPolicyMode;
/// Compaction mode
...
pub struct AgentsConfig {
    /// Tool surface for newly created interactive root/coordinator sessions.
    /// Swarm workers are always created with `Normal`, independent of this
    /// setting. Existing sessions retain their persisted value on resume.
    pub root_tool_policy_mode: SessionToolPolicyMode,
    ...
    /// Optional pool of model identifiers used by the `pool` / `rotate`
    /// sentinel for swarm model selection. When `swarm spawn --model pool`
    /// (or `swarm_model = "pool"`) is used, the resolution picks the next
    /// non-blank entry from this list using a process-global atomic
    /// round-robin counter. Blank entries are skipped. Empty pool with the
    /// sentinel fails clearly instead of silently inheriting. Unset or empty
    /// means no pool is configured.
    #[serde(default, skip_serializing_if = "Vec::is_empty")]
    pub swarm_model_pool: Vec<String>,
...
impl Default for AgentsConfig {
    fn default() -> Self {
        Self {
            root_tool_policy_mode: SessionToolPolicyMode::default(),
            swarm_model: None,
            swarm_model_pool: Vec::new(),
            swarm_effort: None,
...
pub struct HooksConfig {
    ...
    /// Runs when the agent BLOCKS on the current user for consent to a
    /// protected operation, and again when that wait resolves. This is the
    /// only point where jcode is genuinely stuck waiting on a human, so it is
    /// the honest "needs attention" signal for a supervisor: unlike `pre_tool`
    /// (which runs before EVERY tool call, approved or not), it fires only
    /// when a prompt is actually on screen.
    /// Fields: STATE ("waiting"/"resolved"), TOOL_NAME, ACTION, TARGET,
    /// and on resolve OUTCOME ("approved"/"denied"/"timeout"/"disconnected")
    /// plus WAITED_MS. Env override: JCODE_HOOK_PERMISSION.
    pub permission: Option<HookCommands>,
...
pub struct FeatureConfig {
    ...
    /// (default: false). Keeping this opt-in prevents verification continuations
    /// from starting after a user-facing report and scrolling that report away.
    pub auto_poke: bool,
...
pub struct ProviderConfig {
    ...
    /// Providers to keep out of automatic failover entirely, by label
    /// ("anthropic"/"claude", "openai", "copilot", "gemini", "cursor",
    /// "bedrock", "openrouter").
    ///
    /// A disabled provider is skipped as a failover candidate even when it is
    /// fully authenticated, so work lands on the routes you actually want to
    /// spend. This is for steering cost, not for safety: explicitly selecting a
    /// disabled provider with `/model` still works, so you are never locked out
    /// of a login you are paying for.
    ...
    /// Anthropic OAuth subscription guard: enable a cross-process exclusive
    /// lease that bounds subscription utilization movement between calls.
    /// Disabled by default so upstream users see no behavior change. Set to
    /// `true` to monitor subscription-backed Claude calls across sessions and
    /// pause future calls for operator approval after suspicious 5-hour usage
    /// movement. Overridable via `JCODE_ANTHROPIC_SUBSCRIPTION_GUARD_ENABLED`.
    pub anthropic_subscription_guard_enabled: bool,
    /// Anthropic OAuth subscription monitor: request operator approval before
    /// future calls when observed 5-hour utilization moves by at least this
    /// many percentage points (0-100). The response that observes the movement
    /// is preserved. Default: 5.0. Overridable via
    /// `JCODE_ANTHROPIC_SUBSCRIPTION_GUARD_THRESHOLD_PERCENT`.
    pub anthropic_subscription_guard_threshold_percent: f32,
    /// Anthropic OAuth subscription monitor: preflight ceiling for 5-hour
    /// utilization. New calls require operator approval when utilization is at
    /// or above this percentage (0-100). Default: 95.0.
    /// Overridable via `JCODE_ANTHROPIC_SUBSCRIPTION_GUARD_MAX_PREFLIGHT_PERCENT`.
    pub anthropic_subscription_guard_max_preflight_percent: f32,
    /// Compatibility setting retained for existing config files. The
    /// concurrent monitor now records meter failures as warnings and does not
    /// create a durable pause from an unverified sample.
    pub anthropic_subscription_guard_fail_closed: bool,
    /// Optional allowlist of subscription-backed Claude model ids. Empty or
    /// `claude-*` admits the supported Claude family, including Haiku and older
    /// capable models. Fable is admitted only when listed by its exact id, so
    /// neither an empty list nor `claude-*` selects it by accident.
    /// Comma-separated env override:
    /// `JCODE_ANTHROPIC_SUBSCRIPTION_GUARD_ALLOWED_MODELS=foo,bar`.
    pub anthropic_subscription_guard_allowed_models: Vec<String>,
...
impl Default for ProviderConfig {
    fn default() -> Self {
        Self {
            ...
            anthropic_subscription_guard_allowed_models: Vec::new(),
            max_retries: 8,
