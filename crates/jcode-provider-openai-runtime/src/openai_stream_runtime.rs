use super::*;
use jcode_base::usage_journal::transport as usage_transport;
...
use self::openai_rate_limit_format::{
    LONG_RESET_MARKER, format_rate_limit_error, parse_rate_limit_reset,
};
...
pub(super) async fn stream_response(
    ...
    let usage_identity = if is_chatgpt_mode {
        usage_transport::subscription_identity(
            creds.account_id.as_deref(),
            creds
                .id_token
                .as_deref()
                .and_then(jcode_base::auth::codex::extract_email)
                .as_deref(),
            "https",
        )
    } else {
        usage_transport::TransportObservation {
            transport: "https".into(),
            billing_route: Some("openai_api".into()),
            ..Default::default()
        }
    };
    drop(creds);
    let usage_observation = usage_transport::begin(usage_identity);
    ...
    if !response.status().is_success() {
        ...
        if status == StatusCode::TOO_MANY_REQUESTS {
            // OpenAI reports subscription usage-limit resets in the JSON body,
            // not in `Retry-After`. Honor a short reset as a retry hint, and
            // treat a long one as terminal for this route so the turn fails
            // over to another account/provider immediately instead of burning
            // its whole retry budget against a limit that cannot clear.
            let reset = parse_rate_limit_reset(&body);
            let mut msg = format_rate_limit_error(&body, retry_after.map(|hint| hint.remaining()));
            let long_reset = reset
                .is_some_and(|reset| reset > jcode_provider_core::retry_after::MAX_RETRY_AFTER);
            if long_reset {
                msg.push_str(&format!(" This {LONG_RESET_MARKER}."));
            }
            let hint = retry_after.or_else(|| {
                reset
                    .filter(|_| !long_reset)
                    .map(jcode_provider_core::retry_after::retry_after_from_duration)
            });
            return Err(OpenAIStreamFailure::Other(
                jcode_provider_core::retry_after::error_with_retry_after(msg, hint),
            ));
        }
        return Err(OpenAIStreamFailure::Other(
...
async fn continue_persistent_ws_locked(
    ...
    let mut usage_identity = state.usage_identity.clone();
    usage_identity.transport = "websocket_continuation".into();
    let usage_observation = usage_transport::begin(usage_identity);
...
pub(super) async fn stream_response_websocket_persistent(
    ...
    let usage_identity = if OpenAIProvider::is_chatgpt_mode(&creds) {
        usage_transport::subscription_identity(
            creds.account_id.as_deref(),
            creds
                .id_token
    ...
    drop(creds);
    let usage_observation = usage_transport::begin(usage_identity.clone());
...
pub(super) fn is_retryable_error(error_str: &str) -> bool {
    ...
    jcode_provider_core::is_transient_transport_error(error_str)
        ...
        // other providers (Anthropic/Copilot) which already retry these. A
        // usage limit whose provider-reported reset is hours or days away is
        // the exception: retrying cannot clear it, and spending the retry
        // budget here only delays account/provider failover.
        || ((error_str.contains("429 too many requests")
            || error_str.contains("rate limit")
            || error_str.contains("rate_limit"))
            && !error_str.contains(LONG_RESET_MARKER))
...
mod stream_runtime_tests {
    ...
    #[test]
    fn short_rate_limits_stay_retryable_but_long_usage_limits_do_not() {
        assert!(is_retryable_error("429 too many requests"));
        assert!(is_retryable_error(
            "rate limited: please slow down. resets in 30s."
        ));
        // A usage limit whose reset is hours or days out cannot clear inside
        // the retry budget; failing fast lets account/provider failover run.
        assert!(!is_retryable_error(&format!(
            "rate limited: the usage limit has been reached. resets in 30d 4h. this {LONG_RESET_MARKER}."
        )));
    }
