...
pub struct SwarmTaskProgress {
    ...
    /// session was dead (failed/stopped/crashed/gone) or a transient worker
    /// watchdog ended the turn. Caps automatic re-dispatch so a node whose
    /// workers keep dying or hanging cannot spawn workers forever; past the cap,
    /// explicit `retry`/`assign_task` remain the recovery paths.
    #[serde(default, skip_serializing_if = "Option::is_none")]
...
mod tests {
    ...
    #[test]
    fn the_documented_plan_item_shape_deserializes() {
        // The swarm tool's schema now tells callers to send exactly this shape.
        // A schema assertion only proves the schema says the field exists; this
        // proves the type on the receiving end actually accepts it. Those can
        // drift apart, and when they do the caller is told to send a field that
        // is then silently dropped, which is worse than never advertising it.
        let raw = r#"{
            "id": "t1",
        ...
        let item: PlanItem = serde_json::from_str(raw).expect("documented shape must deserialize");

        assert_eq!(item.subsystem.as_deref(), Some("parsing"));
        assert_eq!(item.file_scope, vec!["src/parse.rs", "src/lex.rs"]);
        assert_eq!(item.blocked_by, vec!["t0"]);
        assert_eq!(item.assigned_to.as_deref(), Some("session-abc"));
    }

    #[test]
    fn omitting_the_optional_plan_fields_still_deserializes() {
        // The four fields are advertised but optional. A caller that ignores
        // them must keep working, or naming them in the schema would be a
        // breaking change disguised as documentation.
        let raw = r#"{
            "id": "t1",
            "content": "small fix",
            "status": "pending",
        ...
        let item: PlanItem = serde_json::from_str(raw).expect("minimal shape must deserialize");

        assert!(item.subsystem.is_none());
        assert!(item.file_scope.is_empty());
        assert!(
            item.blocked_by.is_empty(),
            "no blockers means ready to start"
        );
