// sonar-file-size: waive=350 reason=pre-existing protocol round-trip suite; P3a adds backward-compatible subscribe identity coverage, and test-suite decomposition is out of scope
#[test]
...
fn test_subscribe_request_roundtrip_preserves_session_takeover_flags() -> Result<()> {
    ...
    assert_eq!(rlm_client, Some(true));
