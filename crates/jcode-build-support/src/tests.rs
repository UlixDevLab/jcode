...
fn build_info_fixture(version: &str) -> BuildInfo {
    BuildInfo {
        hash: version.to_string(),
        full_hash: format!("{version}-full"),
        built_at: chrono::Utc::now(),
        commit_message: Some(format!("build {version}")),
        dirty: false,
...
fn install_version_fixture(version: &str) {
    let binary = version_binary_path(version).expect("version path");
    std::fs::create_dir_all(binary.parent().unwrap()).expect("version dir");
    std::fs::write(binary, format!("fixture {version}")).expect("fixture binary");
}
...
#[test]
fn prune_unreferenced_versions_keeps_channels_and_filters_manifest_history() {
    with_temp_jcode_home(|| {
        let stale = "stale-version";
        let current = "current-version";
        let shared = "shared-version";
        for version in [stale, current, shared] {
            install_version_fixture(version);
        }
        update_current_symlink(current).expect("current channel");
        update_shared_server_symlink(shared).expect("shared channel");

        let mut manifest = BuildManifest {
            history: vec![
                {
                    let mut legacy = build_info_fixture("legacy-hash");
                    legacy.version_label = None;
                    legacy
                },
                build_info_fixture(stale),
                build_info_fixture(current),
                build_info_fixture(shared),
            ],
            ..Default::default()
        };
        manifest.save().expect("save manifest");

        let report = prune_unreferenced_versions().expect("prune versions");

        assert_eq!(report.removed, vec![stale.to_string()]);
        assert!(report.failures.is_empty());
        assert!(!version_binary_path(stale).unwrap().exists());
        assert!(version_binary_path(current).unwrap().exists());
        assert!(version_binary_path(shared).unwrap().exists());

        manifest = BuildManifest::load().expect("reload manifest");
        let retained: Vec<_> = manifest
            .history
            .iter()
            .filter_map(|info| info.version_label.as_deref())
            .collect();
        assert_eq!(retained, vec![current, shared]);
        assert!(
            manifest
                .history
                .iter()
                .any(|info| info.hash == "legacy-hash" && info.version_label.is_none()),
            "pruning must preserve history written before version labels existed"
        );
    });
}

#[test]
fn prune_unreferenced_versions_keeps_pending_rollback_versions() {
    with_temp_jcode_home(|| {
        let previous_current = "previous-current";
        let previous_shared = "previous-shared";
        let pending = "pending-version";
        let stale = "stale-version";
        for version in [previous_current, previous_shared, pending, stale] {
            install_version_fixture(version);
        }

        BuildManifest {
            canary: Some(pending.to_string()),
            pending_activation: Some(PendingActivation {
                session_id: "session-prune".to_string(),
                new_version: pending.to_string(),
                previous_current_version: Some(previous_current.to_string()),
                previous_shared_server_version: Some(previous_shared.to_string()),
                source_fingerprint: None,
                requested_at: chrono::Utc::now(),
            }),
            history: [previous_current, previous_shared, pending, stale]
                .into_iter()
                .map(build_info_fixture)
                .collect(),
            ..Default::default()
        }
        .save()
        ...
        assert_eq!(report.removed, vec![stale.to_string()]);
        for version in [previous_current, previous_shared, pending] {
            assert!(
                version_binary_path(version).unwrap().exists(),
                "pending activation must retain {version}"
            );
        }
    });
}
...
#[test]
#[ignore = "helper process for running-version retention test"]
fn running_executable_fixture_process() {
    std::thread::sleep(std::time::Duration::from_secs(30));
}

#[cfg(any(target_os = "macos", target_os = "linux"))]
#[test]
fn prune_unreferenced_versions_keeps_a_running_version_until_exit() {
    with_temp_jcode_home(|| {
        use std::process::{Command, Stdio};
        use std::thread;
        use std::time::Duration;

        let running = "running-version";
        let binary = version_binary_path(running).expect("version path");
        std::fs::create_dir_all(binary.parent().unwrap()).expect("version dir");
        std::fs::copy(
            std::env::current_exe().expect("current test executable"),
            &binary,
        )
        .expect("copy test executable fixture");
        crate::platform_support::set_permissions_executable(&binary).expect("make executable");
        BuildManifest {
            history: vec![build_info_fixture(running)],
            ..Default::default()
        }
        .save()
        .expect("save manifest");
        ...
        let mut child = Command::new(&binary)
            .args([
                "--ignored",
                "--exact",
                "tests::running_executable_fixture_process",
            ])
            .stdin(Stdio::null())
            .stdout(Stdio::null())
            .stderr(Stdio::null())
            .spawn()
            .expect("spawn running version");
        thread::sleep(Duration::from_millis(100));

        let first = prune_unreferenced_versions().expect("prune while running");
        child.kill().expect("kill fixture process");
        child.wait().expect("wait fixture process");

        assert!(first.removed.is_empty());
        assert!(binary.exists(), "running version must remain installed");
        let second = prune_unreferenced_versions().expect("prune after exit");
        assert_eq!(second.removed, vec![running.to_string()]);
...
fn update_launcher_symlink_stays_inside_sandbox_home() {
    with_temp_jcode_home(|| {
        let previous_version = "sandbox-previous";
        install_version_fixture(previous_version);
        update_current_symlink(previous_version).expect("publish previous current version");
        ...
        let current = current_binary_path().expect("current binary path");
        let staging = current.parent().unwrap().join(format!(
            ".{}-current-{}",
            binary_stem(),
            std::process::id()
        ));
        assert!(!staging.exists(), "atomic swap must clean its staging path");
    });
}
...
#[test]
fn generated_ops_freshness_does_not_dirty_source_state() {
    let repo = create_git_repo_fixture();
    let generated_dir = repo.path().join(".jcode");
    let generated = generated_dir.join("ops-freshness.json");
    std::fs::create_dir_all(&generated_dir).expect("generated dir");
    std::fs::write(&generated, "{\"fresh\":true}\n").expect("generated fixture");
    std::process::Command::new("git")
        .args(["add", ".jcode/ops-freshness.json"])
        .current_dir(repo.path())
        .output()
        .expect("git add generated fixture");
    std::process::Command::new("git")
        .args(["commit", "-m", "track generated fixture"])
        .current_dir(repo.path())
        .output()
        .expect("git commit generated fixture");

    let clean = current_source_state(repo.path()).expect("clean source state");
    std::fs::write(&generated, "{\"fresh\":false}\n").expect("modify generated fixture");
    let generated_only = current_source_state(repo.path()).expect("generated-only source state");

    assert!(!generated_only.dirty);
    assert_eq!(generated_only.changed_paths, 0);
    assert_eq!(generated_only.fingerprint, clean.fingerprint);
    ...
    let source_changed = current_source_state(repo.path()).expect("dirty source state");
    assert!(source_changed.dirty);
    assert_eq!(source_changed.changed_paths, 1);
    assert_ne!(source_changed.fingerprint, clean.fingerprint);
}
