...
const SOURCE_STATE_EXCLUDES: &[&str] = &[":(exclude).jcode/ops-freshness.json"];
...
fn git_source_state_output_bytes(repo_dir: &Path, args: &[&str]) -> Result<Vec<u8>> {
    let mut scoped_args = Vec::with_capacity(args.len() + SOURCE_STATE_EXCLUDES.len() + 2);
    scoped_args.extend_from_slice(args);
    scoped_args.extend(["--", "."]);
    scoped_args.extend_from_slice(SOURCE_STATE_EXCLUDES);
    git_output_bytes(repo_dir, &scoped_args)
}
...
pub fn current_source_state(repo_dir: &Path) -> Result<SourceState> {
    ...
    let status = git_source_state_output_bytes(
        repo_dir,
        &["status", "--porcelain=v1", "-z", "--untracked-files=all"],
    )?;
    let diff = git_source_state_output_bytes(repo_dir, &["diff", "--binary", "HEAD"])?;
    let untracked = git_source_state_output_bytes(
        repo_dir,
