...
/// Result of removing immutable builds that no active channel, pending reload,
/// or running process still needs.
...
fn installed_version_labels(versions_dir: &Path) -> Result<BTreeSet<String>> {
...
/// Remove immutable version directories that are not reachable from an active
/// channel, needed for a pending rollback, or loaded by a running process.
///
/// On Windows a loaded executable is retained by the operating system's sharing
/// violation even though process-path enumeration is unnecessary there.
pub fn prune_unreferenced_versions() -> Result<VersionPruneReport> {
    let builds = builds_dir()?;
    let versions_dir = builds.join("versions");
    if !versions_dir.exists() {
        return Ok(VersionPruneReport::default());
    }

    let mut manifest = BuildManifest::load()?;
    let mut protected = BTreeSet::new();
    insert_version(&mut protected, manifest.stable.as_deref());
    insert_version(&mut protected, manifest.canary.as_deref());
    insert_version(&mut protected, read_stable_version()?.as_deref());
    insert_version(&mut protected, read_current_version()?.as_deref());
    insert_version(&mut protected, read_shared_server_version()?.as_deref());
    if let Some(pending) = manifest.pending_activation.as_ref() {
        insert_version(&mut protected, Some(&pending.new_version));
        insert_version(&mut protected, pending.previous_current_version.as_deref());
        insert_version(
            &mut protected,
    ...
    for channel in ["stable", "current", "shared-server", "canary"] {
        let binary = builds.join(channel).join(binary_name());
        if let Some(version) = version_for_binary_path(&binary, &versions_dir) {
            protected.insert(version);
        }
    }
    for path in crate::platform_support::running_executable_paths()? {
        if let Some(version) = version_for_binary_path(&path, &versions_dir) {
            protected.insert(version);
        }
    }

    let installed = installed_version_labels(&versions_dir)?;
    ...
    for version in installed {
        ...
        match std::fs::remove_dir_all(&path) {
            Ok(()) => report.removed.push(version),
            Err(error) if error.kind() == std::io::ErrorKind::NotFound => {
                report.removed.push(version)
            }
            Err(error) => report.failures.push((version, error.to_string())),
        }
    }

    let retained = installed_version_labels(&versions_dir)?;
    let history_len = manifest.history.len();
    manifest.history.retain(|info| {
        info.version_label
            .as_ref()
            .is_none_or(|version| retained.contains(version))
    });
    if manifest.history.len() != history_len {
        manifest.save()?;
    }
    Ok(report)
}
