...
pub const CLIENT_TERMINAL_ENV_VARS: &[&str] = &[
    // Per-client lifecycle observers. A long-lived shared jcode server cannot
    // inherit these from every terminal that later connects, so clients carry
    // the exact hook command and its Cate correlation context with Subscribe.
...
mod tests {
    ...
    fn snapshot_client_terminal_env_captures_set_vars_only() {
        ...
        let previous_hook = std::env::var_os("JCODE_HOOK_TURN_END");
        let previous_endpoint = std::env::var_os("CATE_HOOK_ENDPOINT");
        let previous_token = std::env::var_os("CATE_HOOK_TOKEN");
        let previous_terminal = std::env::var_os("CATE_TERMINAL_ID");
        unsafe {
            std::env::set_var("ZELLIJ_SESSION_NAME", "snapshot-test");
            std::env::remove_var("TMUX");
            std::env::set_var("JCODE_HOOK_TURN_END", "/tmp/cate-hook-bridge-jcode");
            std::env::set_var("CATE_HOOK_ENDPOINT", "http://127.0.0.1:4567");
            std::env::set_var("CATE_HOOK_TOKEN", "terminal-token");
            std::env::set_var("CATE_TERMINAL_ID", "rpty-cate");
        }
        ...
        assert!(
            snapshot
                .iter()
                .any(|(k, v)| { k == "JCODE_HOOK_TURN_END" && v == "/tmp/cate-hook-bridge-jcode" })
        );
        assert!(
            snapshot
                .iter()
                .any(|(k, v)| k == "CATE_HOOK_ENDPOINT" && v == "http://127.0.0.1:4567")
        );
        assert!(
            snapshot
                .iter()
                .any(|(k, v)| k == "CATE_HOOK_TOKEN" && v == "terminal-token")
        );
        assert!(
            snapshot
                .iter()
                .any(|(k, v)| k == "CATE_TERMINAL_ID" && v == "rpty-cate")
        );
        unsafe {
            std::env::remove_var("ZELLIJ_SESSION_NAME");
            match previous_hook {
                Some(value) => std::env::set_var("JCODE_HOOK_TURN_END", value),
                None => std::env::remove_var("JCODE_HOOK_TURN_END"),
            }
            match previous_endpoint {
                Some(value) => std::env::set_var("CATE_HOOK_ENDPOINT", value),
                None => std::env::remove_var("CATE_HOOK_ENDPOINT"),
            }
            match previous_token {
                Some(value) => std::env::set_var("CATE_HOOK_TOKEN", value),
                None => std::env::remove_var("CATE_HOOK_TOKEN"),
            }
            match previous_terminal {
                Some(value) => std::env::set_var("CATE_TERMINAL_ID", value),
                None => std::env::remove_var("CATE_TERMINAL_ID"),
            }
        }
    }
