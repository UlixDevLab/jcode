...
| `input_tokens` / `output_tokens` | `12345` / `678` | Raw session-level provider-reported token usage totals |
| `cache_read_input_tokens` / `cache_creation_input_tokens` | `9000` / `1200` | Raw provider-reported prompt-cache token breakdown when available |
| `total_tokens` | `14223` | Provider-normalized effective input plus output. Split-cache providers add cache counters to uncached input; subset-cache providers do not double-count cached input |
