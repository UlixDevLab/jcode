...
fn run_main() -> Result<()> {
    ...
    // This read-only command is deliberately intercepted before normal CLI
    // startup so it cannot initialize providers, daemon state, or a session.
    if let Some(result) = jcode::rlm::run_manifest_command(std::env::args().skip(1)) {
        return result;
    }
