...
#[test]
fn claude_monitor_subcommands_parse() {
    let args = Args::try_parse_from(["jcode", "claude", "status", "--json"]).unwrap();
    match args.command {
        Some(Command::Claude {
            action: ClaudeCommand::Status { json },
        }) => assert!(json),
        other => panic!("unexpected command: {:?}", other),
    }

    for (name, expected) in [("on", "on"), ("off", "off"), ("approve", "approve")] {
        let args = Args::try_parse_from(["jcode", "claude", name]).unwrap();
        let actual = match args.command {
            Some(Command::Claude {
                action: ClaudeCommand::On,
            }) => "on",
            Some(Command::Claude {
                action: ClaudeCommand::Off,
            }) => "off",
            Some(Command::Claude {
                action: ClaudeCommand::Approve,
            }) => "approve",
            other => panic!("unexpected command: {:?}", other),
        };
