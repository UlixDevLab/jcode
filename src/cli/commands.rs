// sonar-file-size: waive=350 reason=pre-existing CLI command monolith; this fix only propagates launcher identity into headless run and decomposition is out of scope
...
pub(crate) fn run_claude_monitor_command(action: super::args::ClaudeCommand) -> Result<()> {
    use crate::usage::{ClaudeMonitorCommand, claude_monitor_status, control_claude_monitor};

    let (state, emit_json, action_label) = match action {
        super::args::ClaudeCommand::Status { json } => (claude_monitor_status()?, json, "status"),
        super::args::ClaudeCommand::On => (
            control_claude_monitor(ClaudeMonitorCommand::On)?,
            false,
            "on",
        ),
        super::args::ClaudeCommand::Off => (
            control_claude_monitor(ClaudeMonitorCommand::Off)?,
            false,
            "off",
        ),
        super::args::ClaudeCommand::Approve => (
            control_claude_monitor(ClaudeMonitorCommand::Approve)?,
            false,
            "approve",
        ),
    };
    ...
    println!("Claude subscription monitor: {action_label}");
    println!(
        "  new calls: {}",
    ...
    println!(
        "  five-hour usage: latest={} baseline={} movement={:.2}pp",
        state
            .latest_five_hour_pct
        ...
        state
            ...
            .map(|value| format!("{value:.2}%"))
            .unwrap_or_else(|| "unknown".to_string()),
        state.aggregate_observed_movement_pct,
    );
    let (started_calls, finished_calls, meter_failures) = state.sessions.values().fold(
        (0_u64, 0_u64, 0_u64),
        |(started, finished, failures), telemetry| {
            (
                started + telemetry.started_calls,
                finished + telemetry.finished_calls,
                failures + telemetry.meter_failures,
            )
        },
    );
    println!("  active calls: {}", state.active_calls.len());
    println!(
        "  observed sessions: {} (started={} finished={} meter_failures={})",
        state.sessions.len(),
        started_calls,
        finished_calls,
        meter_failures,
    );
    if let Some(warning) = state.last_warning.as_deref() {
...
pub async fn run_server_reload_command(force: bool, emit_json: bool) -> Result<()> {
    ...
    // Bound the entire normal request path. A socket can remain connectable
    // while one daemon request handler is stalled, so an unbounded Subscribe,
    // Reload, or event read would otherwise prevent the safely-attested
    // SIGUSR1 fallback from ever being considered.
    let normal_attempt = tokio::time::timeout(Duration::from_secs(3), async {
        ...
        // This repair belongs only to the explicit normal upgrade request. The
        // stalled-path fallback below is current-executable-only and never
        // activates a shared-server channel candidate.
        ...
        // A disconnect mid-reload is the expected handoff only after the
        // server has explicitly announced it. Otherwise this remains a normal
        // request failure and may use the safe stalled-path fallback.
        loop {
            match client.read_event().await {
                Ok(ServerEvent::Ack { id }) if id == request_id => {}
                Ok(ServerEvent::Reloading { .. }) => reloading = true,
                Ok(ServerEvent::ReloadProgress { step, .. }) if step == "skip" => skipped = true,
                ...
                Ok(_) => {}
                Err(_error) if reloading => break,
                Err(error) => return Err(error),
            }
        }
        Ok::<_, anyhow::Error>((reloading, skipped))
    })
    .await;

    let (reloading, skipped) = match normal_attempt {
        Ok(Ok(outcome)) => outcome,
        outcome => {
            let normal_error = match outcome {
                Ok(Err(error)) => error,
                Err(_) => anyhow::anyhow!("server reload request handler exceeded 3s deadline"),
                Ok(Ok(_)) => unreachable!(),
            };

            // Do not signal a registry PID. Unix-only fallback first performs a
            // live nonce-bound control exchange tied to this exact primary
            // socket, then signals the attested handler and confirms a marker.
            #[cfg(unix)]
            {
                if let Err(fallback_error) = crate::server::signal_stalled_reload(&socket).await {
                    return Err(normal_error.context(format!(
                        "normal reload failed and safe SIGUSR1 fallback was declined: {fallback_error}"
                    )));
                }
                let handoff =
                    crate::server::await_reload_handoff(&socket, Duration::from_secs(30)).await;
                if handoff != crate::server::ReloadWaitStatus::Ready {
                    return Err(normal_error.context(format!(
                        "attested SIGUSR1 fallback did not complete its same-build handoff: {handoff:?}"
                    )));
                }
                return emit(ServerReloadReport {
                    socket: socket.display().to_string(),
                    ...
                    already_current: false,
                    handoff_ready: true,
                    detail: "jcode server accepted a same-build idle recovery after the normal reload path stalled.".to_string(),
                });
            }
...
pub async fn run_single_message_command(
    ...
    agent.set_rlm_client_identity(crate::rlm::launcher_identity());
