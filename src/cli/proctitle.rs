...
pub(crate) fn initial_title(args: &Args) -> String {
    match &args.command {
        ...
        Some(Command::GateTelemetry) => "jcode gate-telemetry".to_string(),
        Some(Command::Claude { .. }) => "jcode claude".to_string(),
        ...
        Some(Command::Replay(_)) => "jcode replay".to_string(),
        Some(Command::Model(_)) => "jcode model".to_string(),
        Some(Command::ProviderTestCoverage { .. }) => "jcode provider-test-coverage".to_string(),
        Some(Command::ProviderDoctor(_)) => "jcode provider-doctor".to_string(),
        Some(Command::AuthTest(_)) => "jcode auth-test".to_string(),
...
mod tests {
    ...
    #[test]
    fn initial_title_labels_gate_telemetry_command() {
        with_selfdev_env_removed(|| {
            let args = Args::parse_from(["jcode", "gate-telemetry"]);
            assert_eq!(initial_title(&args), "jcode gate-telemetry");
        });
    }
