...
pub fn register_external_provider_runtimes() {
    ...
    crate::provider::external::register_external_provider_fallible(
        ...
        || {
            // A session bound to a specific subscription pins its account label
            // for the duration of this (synchronous) construction. Without the
            // pin the runtime would load whichever account is globally active,
            // which is exactly the cross-billing this binding prevents.
            let pinned = crate::auth::account_binding::pinned_account_for_construction("openai");
            let credentials = match pinned.as_deref() {
                Some(label) => crate::auth::codex::load_credentials_for_label(label)
                    .inspect(|_| {
                        crate::logging::info(&format!(
                            "Loaded OpenAI credentials for pinned account '{label}'"
                        ));
                    })
                    .inspect_err(|error| {
                        crate::logging::warn(&format!(
                            "Could not load pinned OpenAI account '{label}': {error}. Falling back to the active account."
                        ));
                    })
                    .or_else(|_| crate::auth::codex::load_credentials()),
                None => crate::auth::codex::load_credentials(),
            };
