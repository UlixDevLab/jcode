...
use super::args::{
    AmbientCommand, Args, AuthCommand, AuthTestCommand, CloudCommand, CloudSessionsCommand,
    Command, MemoryCommand, ModelCommand, ProviderCommand, ProviderDoctorCommand, ReplayCommand,
...
pub(crate) async fn run_main(mut args: Args) -> Result<()> {
    ...
    match args.command {
        ...
        Some(Command::Usage { json }) => commands::run_usage_command(json).await?,
        Some(Command::GateTelemetry) => super::gate_telemetry::print_last_seven_days_report()?,
        Some(Command::Claude { action }) => {
            commands::run_claude_monitor_command(action)?;
        }
        ...
        Some(Command::ProviderDoctor(ProviderDoctorCommand {
            provider,
            tier,
            json,
        })) => {
        ...
        Some(Command::AuthTest(AuthTestCommand {
            login,
            all_configured,
            no_smoke,
            no_tool_smoke,
