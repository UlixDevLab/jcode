...
#[test]
fn run_auto_poke_followup_targets_only_incomplete_todos() {
    let todos = vec![
        test_todo("completed", "completed", "high"),
        test_todo("cancelled", "cancelled", "low"),
        test_todo("working", "in_progress", "medium"),
    ];
...
fn run_auto_poke_followup_prioritizes_incomplete_todos() {
    ...
    match followup {
        ...
        other => panic!("expected incomplete-todo follow-up, got {other:?}"),
    }
}
...
fn run_auto_poke_treats_completion_synonyms_and_case_as_finished() {
    for status in ["done", "finished", "complete", "Completed", " DONE "] {
        let todos = vec![test_todo("a", status, "high")];
        assert!(
            build_run_auto_poke_follow_up_from_todos(&todos).is_none(),
