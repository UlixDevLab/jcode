...
pub async fn run_self_dev(should_build: bool, resume_session: Option<String>) -> Result<()> {
    ...
    if should_build {
        ...
        if let Err(error) = build::prune_unreferenced_versions() {
            logging::warn(&format!(
                "self-dev build version pruning was skipped: {error}"
            ));
        }
    ...
    if server_running && let Err(error) = build::prune_unreferenced_versions() {
        logging::warn(&format!(
            "self-dev resume version pruning was skipped: {error}"
        ));
    }
