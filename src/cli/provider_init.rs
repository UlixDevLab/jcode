...
async fn init_provider_with_options(
    ...
    // Bind this process to the subscription configured for its working
    // directory. `jcode run` and other CLI entry points construct their
    // provider here rather than through the server session lifecycle, so the
    // pin has to be applied on both paths. Held for the whole function because
    // provider construction below is what loads credentials.
    let _account_pin = std::env::current_dir().ok().and_then(|dir| {
        auth::account_binding::pin_for_new_session("openai", &dir.to_string_lossy())
    });
