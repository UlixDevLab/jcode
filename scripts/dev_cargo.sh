#!/usr/bin/env bash
# sonar-file-size: {"waive":350,"owner":"jcode-maintainers","ticket":"Jcode-disk-cargo-gate","baseline_loc":1088,"max_loc":1088,"expires":"2026-12-01","removal_trigger":"Extract remaining build policy domains into focused scripts","reason":"Inherited build wrapper changed while extracting host-wide Cargo gate responsibilities; it remains below its mission-start size."}
set -euo pipefail
...
# shellcheck source=scripts/cargo_gate.sh
source "$repo_root/scripts/cargo_gate.sh"
...
finish_cargo_action() {
  ...
  record_rust_action_log "$exit_code" || true
  exit "$exit_code"
}
